...
# ThinkFleetBot Auth Widget for Termux
...
SERVER="${THINKFLEETBOT_SERVER:-l36}"

# Check auth status
termux-toast "Checking ThinkFleetBot auth..."

STATUS=$(ssh "$SERVER" '$HOME/thinkfleetbot/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/thinkfleetbot/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|THINKFLEETBOT_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/thinkfleetbot/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|THINKFLEETBOT_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: ThinkFleetBot won't work until re-auth"
                ;;
        esac
        ;;
