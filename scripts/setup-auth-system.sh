#!/bin/bash
# Setup ThinkFleetBot Auth Management System
...
echo "=== ThinkFleetBot Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'thinkfleetbot-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  ThinkFleetBot message: Send warning via ThinkFleetBot itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/thinkfleetbot-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/thinkfleetbot-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/thinkfleetbot-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now thinkfleetbot-auth-monitor.timer
...
echo "   scp $SCRIPT_DIR/termux-quick-auth.sh phone:~/.shortcuts/TFAuth"
echo "   scp $SCRIPT_DIR/termux-auth-widget.sh phone:~/.shortcuts/TFAuth-Full"
echo ""
echo "4. Make them executable on phone:"
echo "   ssh phone 'chmod +x ~/.shortcuts/TF*'"
echo ""
echo "5. Add Termux:Widget to your home screen"
echo "6. Tap the widget to see your auth scripts"
echo ""
echo "The quick widget (TFAuth) shows status and opens auth URL if needed."
echo "The full widget (TFAuth-Full) provides guided re-auth flow."
echo ""
...
echo "  - Auth monitor: systemctl --user status thinkfleetbot-auth-monitor.timer"
echo ""
echo "Quick commands:"
