...
ZIP=${1:?"Usage: $0 ThinkFleetBot-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/thinkfleetbot/thinkfleetbot/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^ThinkFleetBot-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/thinkfleetbot/thinkfleet-engine/releases/download/v${VERSION}/"}
