...
set_image_model() {
  ...
  local candidate
  for candidate in "$@"; do
    if thinkfleet --profile "$profile" models set-image "$candidate" >/dev/null 2>&1; then
      echo "$candidate"
      return 0
    fi
  done
...
set_agent_model() {
  ...
  shift
  for candidate in "$@"; do
    if thinkfleet --profile "$profile" models set "$candidate" >/dev/null 2>&1; then
      echo "$candidate"
      return 0
    fi
  done
...
run_agent_turn() {
  ...
  local prompt="$3"
  local out_json="$4"
  thinkfleet --profile "$profile" agent \
...
run_profile() {
  ...
  local agent_model_provider="$4" # "openai"|"anthropic"
	  ...
	  if [[ "$agent_model_provider" == "openai" ]]; then
	    thinkfleet --profile "$profile" onboard \
	      --non-interactive \
	      --accept-risk \
	      --flow quickstart \
  ...
  SESSION_JSONL="/root/.thinkfleet-${profile}/agents/main/sessions/${SESSION_ID}.jsonl"
  ...
  thinkfleet --profile "$profile" gateway --port "$port" --bind loopback >"$GATEWAY_LOG" 2>&1 &
  GATEWAY_PID="$!"
  cleanup_profile() {
  ...
  for _ in $(seq 1 60); do
    if thinkfleet --profile "$profile" health --timeout 2000 --json >/dev/null 2>&1; then
      break
    fi
    sleep 0.25
  done
  thinkfleet --profile "$profile" health --timeout 10000 --json >/dev/null
  ...
  if [[ ! -f "$SESSION_JSONL" ]]; then
    ...
    ls -la "/root/.thinkfleet-${profile}/agents/main/sessions" >&2 || true
    exit 1
  fi
