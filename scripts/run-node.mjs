...
const compiler = env.THINKFLEETBOT_TS_COMPILER === "tsc" ? "tsc" : "tsgo";
...
const shouldBuild = () => {
  if (env.THINKFLEETBOT_FORCE_BUILD === "1") return true;
...
const logRunner = (message) => {
  if (env.THINKFLEETBOT_RUNNER_LOG === "0") return;
  process.stderr.write(`[thinkfleetbot] ${message}\n`);
};

const runNode = () => {
  const nodeProcess = spawn(process.execPath, ["thinkfleetbot.mjs", ...args], {
    cwd,
    env,
    stdio: "inherit",
  });
