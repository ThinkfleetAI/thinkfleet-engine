...
BASE_IMAGE="${BASE_IMAGE:-thinkfleetbot-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-thinkfleetbot-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=thinkfleetbot.sandbox=1)
NOTE
