...
# ThinkFleet Logging Utility
# Simplifies access to ThinkFleet logs using macOS unified logging system

set -euo pipefail

# Configuration
SUBSYSTEM="ai.thinkfleet"
DEFAULT_LEVEL="info"
...
handle_sudo_error() {
    ...
    echo -e "${RED}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}\n"
    echo -e "tflog needs to use sudo to show complete log data (Apple hides sensitive info by default)."
...
show_usage() {
    cat << EOF
tflog - ThinkFleetBot Logging Utility

USAGE:
    tflog [OPTIONS]

DESCRIPTION:
    View ThinkFleetBot logs with full details (bypasses Apple's privacy redaction).
    Requires sudo access configured for /usr/bin/log command.

LOG FLOW ARCHITECTURE:
    ThinkFleetBot logs flow through the macOS unified log (subsystem: ai.thinkfleet).
...
QUICK START:
    tflog -n 100             Show last 100 lines from all components
    tflog -f                 Follow logs in real-time
    tflog -e                 Show only errors
    tflog -c ServerManager   Show logs from ServerManager only
...
EXAMPLES:
    tflog                   Show last 50 lines from past 5 minutes (default)
    tflog -f                Stream logs continuously
    tflog -n 100            Show last 100 lines
    tflog -e                Show only recent errors
    tflog -l 30m -n 200     Show last 200 lines from past 30 minutes
    tflog -c ServerManager  Show recent ServerManager logs
    tflog -s "fail"         Search for "fail" in recent logs
    tflog --server -e       Show recent server errors
    tflog -f -d             Stream debug logs continuously
...
list_categories() {
    echo -e "${BLUE}Fetching ThinkFleet log categories from the last hour...${NC}\n"
...
if [[ "$STREAM_MODE" == true ]]; then
    ...
    echo -e "${GREEN}Streaming ThinkFleet logs continuously...${NC}"
