...
async function main() {
  ...
  const baseDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "thinkfleetbot-zai-fallback-"),
  );
  const stateDir = path.join(baseDir, "state");
  const configPath = path.join(baseDir, "thinkfleetbot.json");
  ...
  const sessionId =
    process.env.THINKFLEETBOT_ZAI_FALLBACK_SESSION_ID ?? randomUUID();
