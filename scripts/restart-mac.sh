#!/usr/bin/env bash
# Reset ThinkFleetBot like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${THINKFLEETBOT_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="ThinkFleetBot.app/Contents/MacOS/ThinkFleetBot"
...
WAIT_FOR_LOCK=0
LOG_PATH="${THINKFLEETBOT_RESTART_LOG:-/tmp/thinkfleetbot-restart.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${THINKFLEETBOT_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.thinkfleetbot/disable-launchagent"
ATTACH_ONLY=1
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log ""
      log "Env:"
      log "  THINKFLEETBOT_GATEWAY_WAIT_SECONDS=0  Wait time before gateway port check (unsigned only)"
      log ""
      log "Unsigned recovery:"
      log "  node thinkfleetbot.mjs daemon install --force --runtime node"
      log "  node thinkfleetbot.mjs daemon restart"
      log ""
      log "Reset unsigned overrides:"
      log "  rm ~/.thinkfleetbot/disable-launchagent"
      log ""
...
log "==> Killing existing ThinkFleetBot instances"
kill_all_thinkfleetbot
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product ThinkFleetBot"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/ThinkFleetBot.app" ]]; then
    APP_BUNDLE="/Applications/ThinkFleetBot.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/ThinkFleetBot.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/ThinkFleetBot.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/ThinkFleetBot.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set THINKFLEETBOT_APP_BUNDLE to your installed ThinkFleetBot.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node thinkfleetbot.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node thinkfleetbot.mjs daemon restart"
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".thinkfleetbot", "thinkfleetbot.json"), "utf8");
...
if pgrep -f "${APP_PROCESS_PATTERN}" >/dev/null 2>&1; then
  log "OK: ThinkFleetBot is running."
else
