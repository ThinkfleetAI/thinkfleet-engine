...
IMAGE_NAME="thinkfleetbot-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="thinkfleetbot-net-e2e-$$"
GW_NAME="thinkfleetbot-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "THINKFLEETBOT_GATEWAY_TOKEN=$TOKEN" \
	  -e "THINKFLEETBOT_SKIP_CHANNELS=1" \
	  -e "THINKFLEETBOT_SKIP_GMAIL_WATCHER=1" \
	  -e "THINKFLEETBOT_SKIP_CRON=1" \
	  -e "THINKFLEETBOT_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
