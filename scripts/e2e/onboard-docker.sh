...
IMAGE_NAME="thinkfleetbot-onboard-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  cat > /tmp/thinkfleetbot-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  chmod +x /tmp/thinkfleetbot-bin/trash
  ...
  run_wizard_cmd() {
    ...
    input_fifo="$(mktemp -u "/tmp/thinkfleetbot-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/thinkfleetbot-onboard-${case_name}.log"
  ...
  make_home() {
    mktemp -d "/tmp/thinkfleetbot-e2e-$1.XXXXXX"
  }
...
NODE
  ...
  run_case_reset() {
    ...
    cat > "$HOME/.thinkfleetbot/thinkfleetbot.json" <<'"'"'JSON'"'"'
{
