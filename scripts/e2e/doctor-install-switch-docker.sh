...
IMAGE_NAME="thinkfleetbot-doctor-install-switch-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/thinkfleetbot-bin:$PATH"
  mkdir -p /tmp/thinkfleetbot-bin

  cat > /tmp/thinkfleetbot-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/thinkfleetbot-bin/systemctl

  cat > /tmp/thinkfleetbot-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
exit 0
LOGINCTL
  chmod +x /tmp/thinkfleetbot-bin/loginctl
  ...
  npm_bin="/tmp/npm-prefix/bin/thinkfleetbot"
  npm_entry="/tmp/npm-prefix/lib/node_modules/thinkfleetbot/thinkfleetbot.mjs"
  git_entry="/app/thinkfleetbot.mjs"
  ...
  run_flow() {
    ...
    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/thinkfleetbot-switch-${name}.XXXXXX")
    export HOME="$home_dir"
    export USER="testuser"

    eval "$install_cmd"

    unit_path="$HOME/.config/systemd/user/thinkfleetbot-gateway.service"
    if [ ! -f "$unit_path" ]; then
