...
IMAGE_NAME="thinkfleetbot-plugins-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  home_dir=$(mktemp -d "/tmp/thinkfleetbot-plugins-e2e.XXXXXX")
  export HOME="$home_dir"
  mkdir -p "$HOME/.thinkfleetbot/extensions"

  cat > "$HOME/.thinkfleetbot/extensions/demo-plugin.js" <<'"'"'JS'"'"'
module.exports = {
  id: "demo-plugin",
  name: "Demo Plugin",
...
NODE
  ...
  pack_dir="$(mktemp -d "/tmp/thinkfleetbot-plugin-pack.XXXXXX")"
  mkdir -p "$pack_dir/package"
  cat > "$pack_dir/package/package.json" <<'"'"'JSON'"'"'
{
  "name": "@thinkfleetbot/demo-plugin-tgz",
  "version": "0.0.1",
  "thinkfleetbot": { "extensions": ["./index.js"] }
}
JSON
...
NODE
  ...
  dir_plugin="$(mktemp -d "/tmp/thinkfleetbot-plugin-dir.XXXXXX")"
  cat > "$dir_plugin/package.json" <<'"'"'JSON'"'"'
{
  "name": "@thinkfleetbot/demo-plugin-dir",
  "version": "0.0.1",
  "thinkfleetbot": { "extensions": ["./index.js"] }
}
JSON
...
NODE
  ...
  file_pack_dir="$(mktemp -d "/tmp/thinkfleetbot-plugin-filepack.XXXXXX")"
  mkdir -p "$file_pack_dir/package"
  cat > "$file_pack_dir/package/package.json" <<'"'"'JSON'"'"'
{
  "name": "@thinkfleetbot/demo-plugin-file",
  "version": "0.0.1",
  "thinkfleetbot": { "extensions": ["./index.js"] }
}
JSON
