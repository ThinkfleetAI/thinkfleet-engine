...
# Build and bundle ThinkFleetBot into a minimal .app we can open.
# Outputs to dist/ThinkFleetBot.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/ThinkFleetBot.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="ThinkFleetBot"
BUNDLE_ID="${BUNDLE_ID:-ai.thinkfleet.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/thinkfleetbot/thinkfleetbot/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/ThinkFleetBot/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :ThinkFleetBotBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :ThinkFleetBotGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/ThinkFleetBot"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/ThinkFleetBot"
fi
chmod +x "$APP_ROOT/Contents/MacOS/ThinkFleetBot"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/ThinkFleetBot" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/ThinkFleetBot/Resources/ThinkFleetBot.icns" "$APP_ROOT/Contents/Resources/ThinkFleetBot.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/ThinkFleetBot/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying ThinkFleetBotKit resources"
THINKFLEETBOTKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/ThinkFleetBotKit_ThinkFleetBotKit.bundle"
if [ -d "$THINKFLEETBOTKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/ThinkFleetBotKit_ThinkFleetBotKit.bundle"
  cp -R "$THINKFLEETBOTKIT_BUNDLE" "$APP_ROOT/Contents/Resources/ThinkFleetBotKit_ThinkFleetBotKit.bundle"
else
  echo "WARN: ThinkFleetBotKit resource bundle not found at $THINKFLEETBOTKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running ThinkFleetBot"
killall -q ThinkFleetBot 2>/dev/null || true
