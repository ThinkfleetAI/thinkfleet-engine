...
#
# Env:
#   DMG_VOLUME_NAME        default: CFBundleName (or "ThinkFleetBot")
...
APP_NAME=$(/usr/libexec/PlistBuddy -c "Print CFBundleName" "$APP_PATH/Contents/Info.plist" 2>/dev/null || echo "ThinkFleetBot")
...
DMG_TEMP="$(mktemp -d /tmp/thinkfleetbot-dmg.XXXXXX)"
...
if [[ "${SKIP_DMG_STYLE:-0}" != "1" ]]; then
  ...
  ICON_SRC="$ROOT_DIR/apps/macos/Sources/ThinkFleetBot/Resources/ThinkFleetBot.icns"
  if [[ -f "$ICON_SRC" ]]; then
...
hdiutil resize -limits "$DMG_RW_PATH" >/tmp/thinkfleetbot-dmg-limits.txt 2>/dev/null || true
MIN_SECTORS="$(tail -n 1 /tmp/thinkfleetbot-dmg-limits.txt 2>/dev/null | awk '{print $1}')"
rm -f /tmp/thinkfleetbot-dmg-limits.txt
