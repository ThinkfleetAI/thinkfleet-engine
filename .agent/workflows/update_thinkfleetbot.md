---
description: Update ThinkFleetBot from upstream when branch has diverged (ahead/behind)
---

# ThinkFleetBot Upstream Sync Workflow
...
```bash
# Kill running app
pkill -x "ThinkFleetBot" || true

# Move old version
mv /Applications/ThinkFleetBot.app /tmp/ThinkFleetBot-backup.app

# Install new build
cp -R dist/ThinkFleetBot.app /Applications/

# Launch
open /Applications/ThinkFleetBot.app
```
...
pnpm thinkfleetbot health

# Verify no zombie processes
ps aux | grep -E "(thinkfleetbot|gateway)" | grep -v grep

# Test agent functionality by sending a verification message
pnpm thinkfleetbot agent --message "Verification: macOS app rebuild successful - agent is responding." --session-id YOUR_TELEGRAM_SESSION_ID
...
# Update thinkfleetbot.json with fallback chains
...
pnpm thinkfleetbot agent --message "Verification: Upstream sync and macOS rebuild completed successfully." --session-id YOUR_TELEGRAM_SESSION_ID || echo "Warning: Agent test failed - check Telegram for verification message"
