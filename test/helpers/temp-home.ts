...
function snapshotEnv(): EnvSnapshot {
  return {
    ...
    stateDir: process.env.THINKFLEETBOT_STATE_DIR,
  };
}
...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("THINKFLEETBOT_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env.THINKFLEETBOT_STATE_DIR = path.join(base, ".thinkfleetbot");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "thinkfleetbot-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".thinkfleetbot", "agents", "main", "sessions"), {
    recursive: true,
  });
  if (opts.env) {
