...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("THINKFLEET_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env.THINKFLEET_STATE_DIR = path.join(base, ".thinkfleet");
...
export async function withTempHome<T>(
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".thinkfleet", "agents", "main", "sessions"), {
    recursive: true,
  });
  if (opts.env) {
