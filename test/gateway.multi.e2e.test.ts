...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `thinkfleetbot-e2e-${name}-`));
  const configDir = path.join(homeDir, ".thinkfleetbot");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          HOME: homeDir,
          THINKFLEETBOT_CONFIG_PATH: configPath,
          THINKFLEETBOT_STATE_DIR: stateDir,
          THINKFLEETBOT_GATEWAY_TOKEN: "",
          THINKFLEETBOT_GATEWAY_PASSWORD: "",
          THINKFLEETBOT_SKIP_CHANNELS: "1",
          THINKFLEETBOT_SKIP_BROWSER_CONTROL_SERVER: "1",
...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        THINKFLEETBOT_GATEWAY_TOKEN: inst.gatewayToken,
        THINKFLEETBOT_GATEWAY_PASSWORD: "",
      },
    )) as NodeListPayload;
    ...
    async () => {
      ...
      const [healthA, healthB] = (await Promise.all([
        runCliJson(["health", "--json", "--timeout", "10000"], {
          THINKFLEETBOT_GATEWAY_PORT: String(gwA.port),
          THINKFLEETBOT_GATEWAY_TOKEN: gwA.gatewayToken,
          THINKFLEETBOT_GATEWAY_PASSWORD: "",
        }),
        runCliJson(["health", "--json", "--timeout", "10000"], {
          THINKFLEETBOT_GATEWAY_PORT: String(gwB.port),
          THINKFLEETBOT_GATEWAY_TOKEN: gwB.gatewayToken,
          THINKFLEETBOT_GATEWAY_PASSWORD: "",
        }),
