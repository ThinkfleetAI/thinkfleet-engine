...
import type { ThinkFleetBotConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("thinkfleetbot-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("thinkfleetbot-bin-sherpa-");
      const modelDir = await makeTempDir("thinkfleetbot-sherpa-model-");
      ...
      const cfg: ThinkFleetBotConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("thinkfleetbot-bin-whispercpp-");
      const modelDir = await makeTempDir("thinkfleetbot-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("thinkfleetbot-bin-gemini-");
      ...
      const cfg: ThinkFleetBotConfig = { tools: { media: { image: {} } } };
