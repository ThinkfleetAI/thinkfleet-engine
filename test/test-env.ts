...
export function installTestEnv(): { cleanup: () => void; tempHome: string } {
  const live =
    process.env.LIVE === "1" ||
    process.env.THINKFLEETBOT_LIVE_TEST === "1" ||
    process.env.THINKFLEETBOT_LIVE_GATEWAY === "1";
  ...
  const restore: RestoreEntry[] = [
    { key: "THINKFLEETBOT_TEST_FAST", value: process.env.THINKFLEETBOT_TEST_FAST },
    ...
    { key: "THINKFLEETBOT_STATE_DIR", value: process.env.THINKFLEETBOT_STATE_DIR },
    { key: "THINKFLEETBOT_CONFIG_PATH", value: process.env.THINKFLEETBOT_CONFIG_PATH },
    { key: "THINKFLEETBOT_GATEWAY_PORT", value: process.env.THINKFLEETBOT_GATEWAY_PORT },
    { key: "THINKFLEETBOT_BRIDGE_ENABLED", value: process.env.THINKFLEETBOT_BRIDGE_ENABLED },
    { key: "THINKFLEETBOT_BRIDGE_HOST", value: process.env.THINKFLEETBOT_BRIDGE_HOST },
    { key: "THINKFLEETBOT_BRIDGE_PORT", value: process.env.THINKFLEETBOT_BRIDGE_PORT },
    { key: "THINKFLEETBOT_CANVAS_HOST_PORT", value: process.env.THINKFLEETBOT_CANVAS_HOST_PORT },
    { key: "THINKFLEETBOT_TEST_HOME", value: process.env.THINKFLEETBOT_TEST_HOME },
  ...
  const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "thinkfleetbot-test-home-"));

  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.THINKFLEETBOT_TEST_HOME = tempHome;
  process.env.THINKFLEETBOT_TEST_FAST = "1";

  // Ensure test runs never touch the developer's real config/state, even if they have overrides set.
  delete process.env.THINKFLEETBOT_CONFIG_PATH;
  // Prefer deriving state dir from HOME so nested tests that change HOME also isolate correctly.
  delete process.env.THINKFLEETBOT_STATE_DIR;
  // Prefer test-controlled ports over developer overrides (avoid port collisions across tests/workers).
  delete process.env.THINKFLEETBOT_GATEWAY_PORT;
  delete process.env.THINKFLEETBOT_BRIDGE_ENABLED;
  delete process.env.THINKFLEETBOT_BRIDGE_HOST;
  delete process.env.THINKFLEETBOT_BRIDGE_PORT;
  delete process.env.THINKFLEETBOT_CANVAS_HOST_PORT;
  ...
  if (process.platform === "win32") {
    process.env.THINKFLEETBOT_STATE_DIR = path.join(tempHome, ".thinkfleetbot");
  }
