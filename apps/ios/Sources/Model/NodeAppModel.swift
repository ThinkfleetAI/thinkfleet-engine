...
final class NodeAppModel {
    ...
    private func handleCanvasA2UIAction(body: [String: Any]) async {
        ...
        guard let name = ThinkFleetBotCanvasA2UIAction.extractActionName(userAction) else { return }
        let actionId: String = {
        ...
        let contextJSON = ThinkFleetBotCanvasA2UIAction.compactJSON(userAction["context"])
        let sessionKey = self.mainSessionKey

        let messageContext = ThinkFleetBotCanvasA2UIAction.AgentMessageContext(
            actionName: name,
            session: .init(key: sessionKey, surfaceId: surfaceId),
            component: .init(id: sourceComponentId, host: host, instanceId: instanceId),
            contextJSON: contextJSON)
        let message = ThinkFleetBotCanvasA2UIAction.formatAgentMessage(messageContext)
        ...
        let js = ThinkFleetBotCanvasA2UIAction.jsDispatchA2UIActionStatus(actionId: actionId, ok: ok, error: errorText)
        do {
    ...
    private func resolveA2UIHostURL() async -> String? {
        ...
        return base.appendingPathComponent("__thinkfleetbot__/a2ui/").absoluteString + "?platform=ios"
    }
    ...
    func requestLocationPermissions(mode: ThinkFleetBotLocationMode) async -> Bool {
    ...
    private func sendAgentRequest(link: AgentDeepLink) async throws {
        ...
        // (Key-based unattended auth is handled on macOS for thinkfleetbot:// links.)
    ...
    private func handleInvoke(_ req: BridgeInvokeRequest) async -> BridgeInvokeResponse {
        ...
        do {
            switch command {
            case ThinkFleetBotLocationCommand.get.rawValue:
                return try await self.handleLocationInvoke(req)
            case ThinkFleetBotCanvasCommand.present.rawValue,
                 ThinkFleetBotCanvasCommand.hide.rawValue,
                 ThinkFleetBotCanvasCommand.navigate.rawValue,
                 ThinkFleetBotCanvasCommand.evalJS.rawValue,
                 ThinkFleetBotCanvasCommand.snapshot.rawValue:
                return try await self.handleCanvasInvoke(req)
            case ThinkFleetBotCanvasA2UICommand.reset.rawValue,
                 ThinkFleetBotCanvasA2UICommand.push.rawValue,
                 ThinkFleetBotCanvasA2UICommand.pushJSONL.rawValue:
                return try await self.handleCanvasA2UIInvoke(req)
            case ThinkFleetBotCameraCommand.list.rawValue,
                 ThinkFleetBotCameraCommand.snap.rawValue,
                 ThinkFleetBotCameraCommand.clip.rawValue:
            ...
            default:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: ThinkFleetBotNodeError(code: .invalidRequest, message: "INVALID_REQUEST: unknown command"))
            }
        } catch {
            ...
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: ThinkFleetBotNodeError(code: .unavailable, message: error.localizedDescription))
        }
    }
    ...
    private func handleLocationInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        let params = (try? Self.decodeParams(ThinkFleetBotLocationGetParams.self, from: req.paramsJSON)) ??
            ThinkFleetBotLocationGetParams()
        ...
        let payload = ThinkFleetBotLocationPayload(
    ...
    private func handleCanvasInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case ThinkFleetBotCanvasCommand.present.rawValue:
            let params = (try? Self.decodeParams(ThinkFleetBotCanvasPresentParams.self, from: req.paramsJSON)) ??
                ThinkFleetBotCanvasPresentParams()
        ...
        case ThinkFleetBotCanvasCommand.hide.rawValue:
            return BridgeInvokeResponse(id: req.id, ok: true)
        case ThinkFleetBotCanvasCommand.navigate.rawValue:
            let params = try Self.decodeParams(ThinkFleetBotCanvasNavigateParams.self, from: req.paramsJSON)
            self.screen.navigate(to: params.url)
            return BridgeInvokeResponse(id: req.id, ok: true)
        case ThinkFleetBotCanvasCommand.evalJS.rawValue:
            let params = try Self.decodeParams(ThinkFleetBotCanvasEvalParams.self, from: req.paramsJSON)
            let result = try await self.screen.eval(javaScript: params.javaScript)
            let payload = try Self.encodePayload(["result": result])
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case ThinkFleetBotCanvasCommand.snapshot.rawValue:
            let params = try? Self.decodeParams(ThinkFleetBotCanvasSnapshotParams.self, from: req.paramsJSON)
    ...
    private func handleCanvasA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case ThinkFleetBotCanvasA2UICommand.reset.rawValue:
            ...
            (() => {
              if (!globalThis.thinkfleetbotA2UI) return JSON.stringify({ ok: false, error: "missing thinkfleetbotA2UI" });
              return JSON.stringify(globalThis.thinkfleetbotA2UI.reset());
            })()
            """)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case ThinkFleetBotCanvasA2UICommand.push.rawValue, ThinkFleetBotCanvasA2UICommand.pushJSONL.rawValue:
            let messages: [AnyCodable]
            if command == ThinkFleetBotCanvasA2UICommand.pushJSONL.rawValue {
                let params = try Self.decodeParams(ThinkFleetBotCanvasA2UIPushJSONLParams.self, from: req.paramsJSON)
                messages = try ThinkFleetBotCanvasA2UIJSONL.decodeMessagesFromJSONL(params.jsonl)
            } else {
                do {
                    let params = try Self.decodeParams(ThinkFleetBotCanvasA2UIPushParams.self, from: req.paramsJSON)
            ...
            let messagesJSON = try ThinkFleetBotCanvasA2UIJSONL.encodeMessagesJSONArray(messages)
            let js = """
            (() => {
              try {
                if (!globalThis.thinkfleetbotA2UI) return JSON.stringify({ ok: false, error: "missing thinkfleetbotA2UI" });
                const messages = \(messagesJSON);
                return JSON.stringify(globalThis.thinkfleetbotA2UI.applyMessages(messages));
              } catch (e) {
    ...
    private func handleCameraInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case ThinkFleetBotCameraCommand.list.rawValue:
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case ThinkFleetBotCameraCommand.snap.rawValue:
            self.showCameraHUD(text: "Taking photo…", kind: .photo)
            self.triggerCameraFlash()
            let params = (try? Self.decodeParams(ThinkFleetBotCameraSnapParams.self, from: req.paramsJSON)) ??
                ThinkFleetBotCameraSnapParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case ThinkFleetBotCameraCommand.clip.rawValue:
            let params = (try? Self.decodeParams(ThinkFleetBotCameraClipParams.self, from: req.paramsJSON)) ??
                ThinkFleetBotCameraClipParams()
    ...
    private func handleScreenRecordInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(ThinkFleetBotScreenRecordParams.self, from: req.paramsJSON)) ??
            ThinkFleetBotScreenRecordParams()
...
private extension NodeAppModel {
    func locationMode() -> ThinkFleetBotLocationMode {
        let raw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        return ThinkFleetBotLocationMode(rawValue: raw) ?? .off
    }
