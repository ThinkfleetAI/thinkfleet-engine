...
class NodeForegroundService : Service() {
  ...
  override fun onCreate() {
    super.onCreate()
    ensureChannel()
    val initial = buildNotification(title = "ThinkFleetBot Node", text = "Starting…")
    ...
    notificationJob =
      scope.launch {
        combine(
          ...
          val title = if (connected) "ThinkFleetBot Node · Connected" else "ThinkFleetBot Node"
          val voiceSuffix =
  ...
  private fun ensureChannel() {
    ...
    val channel =
      NotificationChannel(
        ...
        description = "ThinkFleetBot node connection status"
        setShowBadge(false)
      }
