...
export function App() {
  ...
  const auth = useAuth();
  const agentsHook = useAgents(auth.isAuthenticated);
  ...
  useEffect(() => {
    ...
    api.settings.get("localAgentEnabled").then((val) => {
      setLocalAgentEnabled(val === true);
    });
  }, []);
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100vh", background: "#0f0f1a" }}>
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 8, paddingLeft: 64 }}>
          <svg width="16" height="16" viewBox="0 0 82 82" fill="#3875c8">
      ...
      <div style={{ flex: 1, overflow: "hidden", display: "flex" }}>
        {auth.isAuthenticated && (
          <AgentSidebar
            ...
            onSelect={agentsHook.selectAgent}
        ...
        <div style={{ flex: 1, overflow: "hidden" }}>
          {view === "chat" ? (
            <ChatWindow gateway={gateway} agentId={agentsHook.selectedAgentId} />
          ) : (
      ...
      <div style={{ height: 24, display: "flex", alignItems: "center", padding: "0 12px", background: "#161625", borderTop: "1px solid #2a2a4a", fontSize: 10, color: "#64748b", gap: 16 }}>
        ...
        <span>{agentsHook.agents.find((a) => a.agentId === agentsHook.selectedAgentId)?.name || (auth.agentMode === "saas" ? "SaaS" : "Standalone")}</span>
