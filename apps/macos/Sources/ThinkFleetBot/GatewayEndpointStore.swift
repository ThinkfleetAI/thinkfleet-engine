...
actor GatewayEndpointStore {
    ...
    {
        let raw = env["THINKFLEETBOT_GATEWAY_PASSWORD"] ?? ""
        ...
        if !trimmed.isEmpty {
               ...
               !configPassword.isEmpty
            {
                self.warnEnvOverrideOnce(
                    kind: .password,
                    envVar: "THINKFLEETBOT_GATEWAY_PASSWORD",
    ...
    private static func resolveGatewayToken(
        ...
        launchdSnapshot: LaunchAgentPlistSnapshot?) -> String?
    {
        let raw = env["THINKFLEETBOT_GATEWAY_TOKEN"] ?? ""
        let trimmed = raw.trimmingCharacters(in: .whitespacesAndNewlines)
        if !trimmed.isEmpty {
               ...
               configToken != trimmed
            {
                self.warnEnvOverrideOnce(
                    kind: .token,
                    envVar: "THINKFLEETBOT_GATEWAY_TOKEN",
    ...
    init(deps: Deps = .live) {
        ...
        if let modeRaw {
            ...
            let seen = UserDefaults.standard.bool(forKey: "thinkfleetbot.onboardingSeen")
            initialMode = seen ? .local : .unconfigured
        }

        let port = deps.localPort()
        let bind = GatewayEndpointStore.resolveGatewayBindMode(
            root: ThinkFleetBotConfigFile.loadDict(),
            env: ProcessInfo.processInfo.environment)
        let customBindHost = GatewayEndpointStore.resolveGatewayCustomBindHost(root: ThinkFleetBotConfigFile.loadDict())
    ...
    {
        if let envBind = env["THINKFLEETBOT_GATEWAY_BIND"] {
    ...
    private static func resolveGatewayScheme(
        ...
        env: [String: String]) -> String
    {
        if let envValue = env["THINKFLEETBOT_GATEWAY_TLS"]?.trimmingCharacters(in: .whitespacesAndNewlines),
           !envValue.isEmpty
        {
