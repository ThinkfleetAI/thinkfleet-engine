...
struct DebugSettings: View {
    ...
    private var launchdSection: some View {
        GroupBox("Gateway startup") {
            VStack(alignment: .leading, spacing: 8) {
                ...
                Text(
                    "When enabled, ThinkFleetBot won't install or manage \(gatewayLaunchdLabel). " +
            ...
            VStack(alignment: .leading, spacing: 10) {
                ...
                HStack(spacing: 8) {
                    ...
                    Button("Copy sample URL") {
                        ...
                        let url = "thinkfleetbot://agent?message=\(encoded)&key=\(key)"
                        NSPasteboard.general.clearContents()
                ...
                Text("Deep links (thinkfleetbot://…) are always enabled; the key controls unattended runs.")
                    .font(.caption2)
                    .foregroundStyle(.secondary)
                ...
                GridRow {
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        Toggle("Write rolling diagnostics log (JSONL)", isOn: self.$diagnosticsFileLogEnabled)
                            ...
                            .help(
                                "Writes a rotating, local-only log under ~/Library/Logs/ThinkFleetBot/. " +
    ...
    private var pathsSection: some View {
        GroupBox("Paths") {
            VStack(alignment: .leading, spacing: 12) {
                VStack(alignment: .leading, spacing: 6) {
                    Text("ThinkFleetBot project root")
                        .font(.caption.weight(.semibold))
                    HStack(spacing: 8) {
                        TextField("Path to thinkfleetbot repo", text: self.$gatewayRootInput)
                        ...
                        Button("Reset") {
                            let def = FileManager().homeDirectoryForCurrentUser
                                .appendingPathComponent("Projects/thinkfleetbot").path
                    ...
                    GridRow {
                        ...
                        VStack(alignment: .leading, spacing: 6) {
                            ...
                            if let sessionStoreSaveError {
                                ...
                                Text("Used by the CLI session loader; stored in ~/.thinkfleetbot/thinkfleetbot.json.")
                                    .font(.footnote)
                                    .foregroundStyle(.secondary)
                            }
                        }
    ...
    private var quickActionsSection: some View {
        GroupBox("Quick actions") {
            VStack(alignment: .leading, spacing: 10) {
                ...
                VStack(alignment: .leading, spacing: 6) {
                    Text(
                        "Note: macOS may require restarting ThinkFleetBot after enabling Accessibility or Screen Recording.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                        .fixedSize(horizontal: false, vertical: true)

                    Button {
                        LaunchdManager.startThinkFleetBot()
                    } label: {
                        Label("Restart ThinkFleetBot", systemImage: "arrow.counterclockwise")
                    }
                    .buttonStyle(.bordered)
                    .controlSize(.small)
                }
    ...
    private func configURL() -> URL {
        FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent(".thinkfleetbot")
...
extension DebugSettings {
    static func exerciseForTesting() async {
        ...
        view.gatewayRootInput = "/tmp/thinkfleetbot"
