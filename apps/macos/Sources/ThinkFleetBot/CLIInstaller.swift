...
enum CLIInstaller {
        ...
        fileManager: FileManager) -> String?
    {
        for basePath in searchPaths {
            let candidate = URL(fileURLWithPath: basePath).appendingPathComponent("thinkfleetbot").path
    ...
    static func install(statusHandler: @escaping @MainActor @Sendable (String) async -> Void) async {
        ...
        await statusHandler("Installing thinkfleetbot CLI…")
        ...
        if response.success {
            ...
            let summary = installedVersion.map { "Installed thinkfleetbot \($0)." } ?? "Installed thinkfleetbot."
