        ...
        ScrollView(.vertical) {
            VStack(alignment: .leading, spacing: 18) {
                VStack(alignment: .leading, spacing: 12) {
                    SettingsToggleRow(
                        title: "ThinkFleetBot active",
                        subtitle: "Pause to stop the ThinkFleetBot gateway; no messages will be processed.",
                    ...
                    SettingsToggleRow(
                        ...
                        subtitle: "Automatically start ThinkFleetBot after you sign in.",
                        binding: self.$state.launchAtLogin)

                    SettingsToggleRow(
                        title: "Show Dock icon",
                        subtitle: "Keep ThinkFleetBot visible in the Dock instead of menu-bar-only mode.",
                ...
                HStack {
                    Spacer()
                    Button("Quit ThinkFleetBot") { NSApp.terminate(nil) }
                ...
                DisclosureGroup(isExpanded: self.$showRemoteAdvanced) {
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        LabeledContent("CLI path") {
                            TextField("/Applications/ThinkFleetBot.app/.../thinkfleetbot", text: self.$state.remoteCliPath)
                                .textFieldStyle(.roundedBorder)
                                .frame(width: 280)
                        }
                    }
...
extension GeneralSettings {
    ...
    private func revealLogs() {
        ...
        Looked for thinkfleetbot logs in /tmp/thinkfleetbot/.
    ...
    private func applyDiscoveredGateway(_ gateway: GatewayDiscoveryModel.DiscoveredGateway) {
        ...
        if self.state.remoteTransport == .direct {
            ...
            ThinkFleetBotConfigFile.setRemoteGatewayUrl(host: host, port: gateway.gatewayPort)
        }
    }
}
...
extension GeneralSettings {
    static func exerciseForTesting() {
        ...
        state.remoteProjectRoot = "/tmp/thinkfleetbot"
        state.remoteCliPath = "/tmp/thinkfleetbot"
