...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: ThinkFleetBotProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private func friendlyGatewayMessage(_ error: Error) -> String {
           ...
           urlErr.code == .dataNotAllowed // used for WS close 1008 auth failures
        {
            ...
            let tokenKey = CommandResolver.connectionModeIsRemote()
                ...
                : "gateway.auth.token"
            return
                "Gateway rejected token; set \(tokenKey) (or THINKFLEETBOT_GATEWAY_TOKEN) " +
    ...
    private static func bridgeToProtocolArgs(
        _ value: ThinkFleetBotProtocol.AnyCodable?) -> [String: ThinkFleetBotProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: ThinkFleetBotProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: ThinkFleetKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: ThinkFleetBotProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("thinkfleetbot.control.heartbeat")
    static let controlAgentEvent = Notification.Name("thinkfleetbot.control.agent")
}
