...
struct MacGatewayChatTransport: ThinkFleetBotChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> ThinkFleetBotChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(ThinkFleetBotChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> ThinkFleetBotChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                ThinkFleetBotGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "chat":
                ...
                guard let chat = try? JSONDecoder().decode(
                    ThinkFleetBotChatEventPayload.self,
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    ThinkFleetBotAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<ThinkFleetBotChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any ThinkFleetBotChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = ThinkFleetBotChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: ThinkFleetBotChatView(
            viewModel: vm,
        ...
        contentViewController: NSViewController) -> NSWindow
    {
        ...
        case .window:
            let window = NSWindow(
                ...
                backing: .buffered,
                defer: false)
            window.title = "ThinkFleetBot Chat"
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<ThinkFleetBotChatView>) -> NSViewController
    {
