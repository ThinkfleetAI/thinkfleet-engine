...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.thinkfleetbot/thinkfleetbot.json. Prefer THINKFLEETBOT_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.thinkfleetbot/thinkfleetbot.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.thinkfleetbot/thinkfleetbot.json. Restart the gateway to apply."
        }
