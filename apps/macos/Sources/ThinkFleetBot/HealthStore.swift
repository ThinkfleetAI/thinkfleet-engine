    ...
    var summaryLine: String {
        ...
        if link.summary.linked != true {
            if let fallback = self.resolveFallbackChannel(snap, excluding: link.id) {
                ...
                return "\(fallbackLabel) \(fallbackState) · Not linked — run thinkfleetbot login"
            }
            return "Not linked — run thinkfleetbot login"
        }
    ...
    var detailLine: String? {
        if let error = self.lastError, !error.isEmpty {
            ...
            if lower.contains("connection refused") {
                ...
                return "The gateway control port (\(host)) isn’t listening — restart ThinkFleetBot to bring it back."
            }
