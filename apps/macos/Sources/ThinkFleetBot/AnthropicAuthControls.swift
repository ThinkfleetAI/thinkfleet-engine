...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: ThinkFleetBotOAuthStore.AnthropicOAuthStatus = ThinkFleetBotOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: ThinkFleetBotOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = ThinkFleetBotOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = ThinkFleetBotOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try ThinkFleetBotOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. ThinkFleetBot can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: ThinkFleetBotOAuthStore.AnthropicOAuthStatus,
