...
struct PermissionsSettings: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            Text("Allow these so ThinkFleetBot can notify and capture when needed.")
                .padding(.top, 4)
...
private struct LocationAccessSettings: View {
    @AppStorage(locationModeKey) private var locationModeRaw: String = ThinkFleetBotLocationMode.off.rawValue
    @AppStorage(locationPreciseKey) private var locationPreciseEnabled: Bool = true
    @State private var lastLocationModeRaw: String = ThinkFleetBotLocationMode.off.rawValue
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            Picker("", selection: self.$locationModeRaw) {
                Text("Off").tag(ThinkFleetBotLocationMode.off.rawValue)
                Text("While Using").tag(ThinkFleetBotLocationMode.whileUsing.rawValue)
                Text("Always").tag(ThinkFleetBotLocationMode.always.rawValue)
            }
            .labelsHidden()
            .pickerStyle(.menu)
        ...
        .onChange(of: self.locationModeRaw) { _, newValue in
            ...
            guard let mode = ThinkFleetBotLocationMode(rawValue: newValue) else { return }
            Task {
    ...
    private var locationMode: ThinkFleetBotLocationMode {
        ThinkFleetBotLocationMode(rawValue: self.locationModeRaw) ?? .off
    }

    private func requestLocationAuthorization(mode: ThinkFleetBotLocationMode) async -> Bool {
