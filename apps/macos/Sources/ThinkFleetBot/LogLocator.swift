...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["THINKFLEETBOT_LOG_DIR"], !override.isEmpty {
            return URL(fileURLWithPath: override)
        }

        return URL(fileURLWithPath: "/tmp/thinkfleetbot")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("thinkfleetbot-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("thinkfleetbot-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/thinkfleetbot/ (rolling or stdout), or nil if none exist.
    static func bestLogFile() -> URL? {
        ...
        return files
            .filter { $0.lastPathComponent.hasPrefix("thinkfleetbot") && $0.pathExtension == "log" }
            .max { lhs, rhs in
