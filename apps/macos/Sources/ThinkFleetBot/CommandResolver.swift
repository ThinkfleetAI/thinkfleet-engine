...
enum CommandResolver {
    private static let projectRootDefaultsKey = "thinkfleetbot.gatewayProjectRootPath"
    private static let helperName = "thinkfleetbot"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let binEntry = root.appendingPathComponent("bin/thinkfleetbot.js").path
    ...
    static func errorCommand(with message: String) -> [String] {
        let script = """
        cat <<'__THINKFLEETBOT_ERR__' >&2
        \(message)
        __THINKFLEETBOT_ERR__
        exit 1
        """
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/thinkfleetbot")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let thinkfleetbotPaths = self.thinkfleetbotManagedPaths(home: home)
        if !thinkfleetbotPaths.isEmpty {
            extras.insert(contentsOf: thinkfleetbotPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + thinkfleetbotPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func thinkfleetbotManagedPaths(home: URL) -> [String] {
        let base = home.appendingPathComponent(".thinkfleetbot")
    ...
    static func thinkfleetbotExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectThinkFleetBotExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        let candidate = self.projectRoot().appendingPathComponent("bin/thinkfleetbot.js").path
        return FileManager().isReadableFile(atPath: candidate) ? candidate : nil
    }

    static func hasAnyThinkFleetBotInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.thinkfleetbotExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func thinkfleetbotNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            ...
            if let thinkfleetbotPath = self.projectThinkFleetBotExecutable(projectRoot: root) {
                return [thinkfleetbotPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "thinkfleetbot", subcommand] + extraArgs
            }
            if let thinkfleetbotPath = self.thinkfleetbotExecutable(searchPaths: searchPaths) {
                return [thinkfleetbotPath, subcommand] + extraArgs
            }

            let missingEntry = """
            thinkfleetbot entrypoint missing (looked for dist/index.js or bin/thinkfleetbot.js); run pnpm build.
            """
    ...
    // Existing callers still refer to thinkfleetbotCommand; keep it as node alias.
    static func thinkfleetbotCommand(
        subcommand: String,
        extraArgs: [String] = [],
        defaults: UserDefaults = .standard,
        configRoot: [String: Any]? = nil,
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real thinkfleetbot CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/thinkfleetbot"
        ...
        \(cliSection)
        \(projectSection)
        if command -v thinkfleetbot >/dev/null 2>&1; then
          CLI="$(command -v thinkfleetbot)"
          thinkfleetbot \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/thinkfleetbot.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/thinkfleetbot.js"
            node "$PRJ/bin/thinkfleetbot.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent thinkfleetbot"
          pnpm --silent thinkfleetbot \(quotedArgs);
        else
          echo "thinkfleetbot CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? ThinkFleetBotConfigFile.loadDict()
