...
enum ThinkFleetBotLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = ThinkFleetBotOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = ThinkFleetBotFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        ThinkFleetBotLogging.bootstrapIfNeeded()
        let label = ThinkFleetBotLogging.makeLabel(subsystem: subsystem, category: category)
...
struct ThinkFleetBotOSLogHandler: LogHandler {
...
struct ThinkFleetBotFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = ThinkFleetBotLogging.parseLabel(self.label)
