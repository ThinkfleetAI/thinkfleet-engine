...
actor PortGuardian {
    ...
    private nonisolated static let appSupportDir: URL = {
        ...
        return base.appendingPathComponent("ThinkFleetBot", isDirectory: true)
    }()
        ...
        tunnelHealthy: Bool?) -> PortReport
    {
        ...
        let expectedCommands = ["node", "thinkfleetbot", "tsx", "pnpm", "bun"]
    ...
    private func isExpected(_ listener: Listener, port: Int, mode: AppState.ConnectionMode) -> Bool {
        ...
        case .remote:
            ...
            return false
        case .local:
            // The gateway daemon may listen as `thinkfleetbot` or as its runtime (`node`, `bun`, etc).
            if full.contains("gateway-daemon") { return true }
            // If args are unavailable, treat a thinkfleetbot listener as expected.
            if cmd.contains("thinkfleetbot"), full == cmd { return true }
            return false
        case .unconfigured:
            return false
        }
