...
extension OnboardingView {
    ...
    func welcomePage() -> some View {
        self.onboardingPage {
            VStack(spacing: 22) {
                Text("Welcome to ThinkFleetBot")
                    .font(.largeTitle.weight(.semibold))
                Text("ThinkFleetBot is a powerful personal AI assistant that can connect to WhatsApp or Telegram.")
                    .font(.body)
                        ...
                        VStack(alignment: .leading, spacing: 6) {
                            ...
                            Text(
                                "The connected AI agent (e.g. Claude) can trigger powerful actions on your Mac, " +
                                    ...
                                    "Only enable ThinkFleetBot if you understand the risks and trust the prompts and " +
    ...
    func connectionPage() -> some View {
        self.onboardingPage {
            Text("Choose your Gateway")
                .font(.largeTitle.weight(.semibold))
            Text(
                "ThinkFleetBot uses a single Gateway that stays running. Pick this Mac, " +
                                ...
                                if self.state.remoteTransport == .ssh {
                                    ...
                                    GridRow {
                                        Text("Project root")
                                            .font(.callout.weight(.semibold))
                                            .frame(width: labelWidth, alignment: .leading)
                                        TextField("/home/you/Projects/thinkfleetbot", text: self.$state.remoteProjectRoot)
                                            .textFieldStyle(.roundedBorder)
                                            .frame(width: fieldWidth)
                                    }
                                    GridRow {
                                        Text("CLI path")
                                            ...
                                            .frame(width: labelWidth, alignment: .leading)
                                        TextField(
                                            "/Applications/ThinkFleetBot.app/.../thinkfleetbot",
    ...
    func anthropicAuthPage() -> some View {
        self.onboardingPage {
            ...
            Text("Give your model the token it needs!")
                ...
                .frame(maxWidth: 540)
                .fixedSize(horizontal: false, vertical: true)
            Text("ThinkFleetBot supports any model — we strongly recommend Opus 4.5 for the best experience.")
                .font(.callout)
                .foregroundStyle(.secondary)
                .multilineTextAlignment(.center)
                .frame(maxWidth: 540)
            ...
            self.onboardingCard(spacing: 12, padding: 16) {
                ...
                Text(
                    "This lets ThinkFleetBot use Claude immediately. Credentials are stored at " +
                        "`~/.thinkfleetbot/credentials/oauth.json` (owner-only).")
                    .font(.subheadline)
                    .foregroundStyle(.secondary)
                    .fixedSize(horizontal: false, vertical: true)

                HStack(spacing: 12) {
                    Text(ThinkFleetBotOAuthStore.oauthURL().path)
                        .font(.caption)
                        .foregroundStyle(.secondary)
                        .lineLimit(1)
                        .truncationMode(.middle)
                    ...
                    Button("Reveal") {
                        NSWorkspace.shared.activateFileViewerSelecting([ThinkFleetBotOAuthStore.oauthURL()])
                    }
    ...
    func permissionsPage() -> some View {
        self.onboardingPage {
            Text("Grant permissions")
                .font(.largeTitle.weight(.semibold))
            Text("These macOS permissions let ThinkFleetBot automate apps and capture context on this Mac.")
                .font(.body)
                .foregroundStyle(.secondary)
                .multilineTextAlignment(.center)
                .frame(maxWidth: 520)
    ...
    func cliPage() -> some View {
        self.onboardingPage {
            Text("Install the CLI")
                .font(.largeTitle.weight(.semibold))
            Text("Required for local mode: installs `thinkfleetbot` so launchd can run the gateway.")
                .font(.body)
                .foregroundStyle(.secondary)
                .multilineTextAlignment(.center)
                .frame(maxWidth: 520)
    ...
    func workspacePage() -> some View {
        self.onboardingPage {
            Text("Agent workspace")
                .font(.largeTitle.weight(.semibold))
            Text(
                "ThinkFleetBot runs the agent from a dedicated workspace so it can load `AGENTS.md` " +
                ...
                if self.state.connectionMode == .remote {
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        Text("Workspace folder")
                            .font(.headline)
                        TextField(
                            AgentWorkspace.displayPath(for: ThinkFleetBotConfigFile.defaultWorkspaceURL()),
                                ...
                                Task {
                                    ...
                                    if saved {
                                        self.workspaceStatus =
                                            "Saved to ~/.thinkfleetbot/thinkfleetbot.json (agents.defaults.workspace)"
                                    }
                                }
                            }
                            .buttonStyle(.bordered)
    ...
    func onboardingChatPage() -> some View {
        VStack(spacing: 16) {
            ...
            self.onboardingGlassCard(padding: 8) {
                ThinkFleetBotChatView(viewModel: self.onboardingChatModel, style: .onboarding)
            ...
            self.onboardingCard {
                ...
                if self.state.connectionMode == .remote {
                    self.featureRow(
                        ...
                        subtitle: """
                        On your gateway host: install/update the `thinkfleetbot` package and make sure credentials exist
                        (typically `~/.thinkfleetbot/credentials/oauth.json`). Then connect again if needed.
                        """,
                        systemImage: "network")
                    Divider()
                        .padding(.vertical, 6)
                ...
                self.featureRow(
                    ...
                    subtitle: "Click the ThinkFleetBot menu bar icon for quick chat and status.",
