...
enum GatewayEnvironment {
    ...
    static func gatewayPort() -> Int {
        if let raw = ProcessInfo.processInfo.environment["THINKFLEETBOT_GATEWAY_PORT"] {
            let trimmed = raw.trimmingCharacters(in: .whitespacesAndNewlines)
            if let parsed = Int(trimmed), parsed > 0 { return parsed }
        }
        if let configPort = ThinkFleetBotConfigFile.gatewayPort(), configPort > 0 {
            return configPort
        }
    ...
    static func check() -> GatewayEnvironmentStatus {
        ...
        case let .success(runtime):
            let gatewayBin = CommandResolver.thinkfleetbotExecutable()
            ...
            if gatewayBin == nil, projectEntrypoint == nil {
                return GatewayEnvironmentStatus(
                    ...
                    gatewayVersion: nil,
                    requiredGateway: expectedString,
                    message: "thinkfleetbot CLI not found in PATH; install the CLI.")
            }
    ...
    private static func preferredGatewayBind() -> String? {
        ...
        if let env = ProcessInfo.processInfo.environment["THINKFLEETBOT_GATEWAY_BIND"] {
    ...
    static func installGlobal(versionString: String?, statusHandler: @escaping @Sendable (String) -> Void) async {
        ...
        let (label, cmd): (String, [String]) =
            if let npm {
                ("npm", [npm, "install", "-g", "thinkfleetbot@\(target)"])
            } else if let pnpm {
                ("pnpm", [pnpm, "add", "-g", "thinkfleetbot@\(target)"])
            } else if let bun {
                ("bun", [bun, "add", "-g", "thinkfleetbot@\(target)"])
            } else {
                ("npm", ["npm", "install", "-g", "thinkfleetbot@\(target)"])
            }

        statusHandler("Installing thinkfleetbot@\(target) via \(label)…")
        ...
        if response.success {
            statusHandler("Installed thinkfleetbot@\(target)")
        } else {
