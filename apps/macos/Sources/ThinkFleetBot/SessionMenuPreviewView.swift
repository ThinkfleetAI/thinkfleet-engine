...
enum SessionMenuPreviewLoader {
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> ThinkFleetBotSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: ThinkFleetBotSessionPreviewEntry,
    ...
    private static func cache(payload: ThinkFleetBotSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: ThinkFleetBotChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
    ...
    private static func decodeMessages(_ raw: [ThinkFleetKit.AnyCodable]) -> [ThinkFleetBotChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(ThinkFleetBotChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: ThinkFleetBotChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: ThinkFleetBotChatMessage) -> Bool {
    ...
    private static func toolNames(for message: ThinkFleetBotChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: ThinkFleetBotChatMessage) -> String? {
