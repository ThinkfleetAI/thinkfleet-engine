...
struct ConnectOptions {
    ...
    var clientId: String = "thinkfleetbot-macos"
...
func runConnect(_ args: [String]) async {
    ...
    if opts.help {
        print("""
        thinkfleetbot-mac connect

        Usage:
          thinkfleetbot-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
        ...
        Options:
          ...
          --json             Emit JSON
          --client-id <id>   Override client id (default: thinkfleetbot-macos)
    ...
    do {
        ...
        let displayName = opts.displayName ?? Host.current().localizedName ?? "ThinkFleetBot macOS Debug CLI"
...
private func printConnectOutput(_ output: ConnectOutput, json: Bool) {
    ...
    print("ThinkFleetBot macOS Gateway Connect")
...
private func resolvedToken(opts: ConnectOptions, mode: String, config: GatewayConfig) -> String? {
    ...
    if let token = ProcessInfo.processInfo.environment["THINKFLEETBOT_GATEWAY_TOKEN"], !token.isEmpty {
        return token
    }
    if mode == "remote" {
...
private func resolvedPassword(opts: ConnectOptions, mode: String, config: GatewayConfig) -> String? {
    ...
    if let password = ProcessInfo.processInfo.environment["THINKFLEETBOT_GATEWAY_PASSWORD"], !password.isEmpty {
        return password
    }
    if mode == "remote" {
