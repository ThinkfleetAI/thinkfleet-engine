...
@Suite(.serialized) struct CommandResolverTests {
    ...
    @Test func prefersThinkFleetBotBinary() async throws {
        ...
        let thinkfleetbotPath = tmp.appendingPathComponent("node_modules/.bin/thinkfleetbot")
        try self.makeExec(at: thinkfleetbotPath)

        let cmd = CommandResolver.thinkfleetbotCommand(subcommand: "gateway", defaults: defaults, configRoot: [:])
        #expect(cmd.prefix(2).elementsEqual([thinkfleetbotPath.path, "gateway"]))
    }
    ...
    @Test func fallsBackToNodeAndScript() async throws {
        ...
        let scriptPath = tmp.appendingPathComponent("bin/thinkfleetbot.js")
        try self.makeExec(at: nodePath)
        try "#!/bin/sh\necho v22.0.0\n".write(to: nodePath, atomically: true, encoding: .utf8)
        try FileManager().setAttributes([.posixPermissions: 0o755], ofItemAtPath: nodePath.path)
        try self.makeExec(at: scriptPath)

        let cmd = CommandResolver.thinkfleetbotCommand(
            subcommand: "rpc",
            defaults: defaults,
            configRoot: [:],
    ...
    @Test func fallsBackToPnpm() async throws {
        ...
        let cmd = CommandResolver.thinkfleetbotCommand(subcommand: "rpc", defaults: defaults, configRoot: [:])

        #expect(cmd.prefix(4).elementsEqual([pnpmPath.path, "--silent", "thinkfleetbot", "rpc"]))
    }
    ...
    @Test func pnpmKeepsExtraArgsAfterSubcommand() async throws {
        ...
        #expect(cmd.prefix(5).elementsEqual([pnpmPath.path, "--silent", "thinkfleetbot", "health", "--json"]))
    ...
    @Test func buildsSSHCommandForRemoteMode() async throws {
        ...
        defaults.set("/srv/thinkfleetbot", forKey: remoteProjectRootKey)
        ...
        if let script = cmd.last {
            #expect(script.contains("PRJ='/srv/thinkfleetbot'"))
            #expect(script.contains("cd \"$PRJ\""))
            #expect(script.contains("thinkfleetbot"))
    ...
    @Test func configRootLocalOverridesRemoteDefaults() async throws {
        ...
        #expect(cmd.first == thinkfleetbotPath.path)
        #expect(cmd.count >= 2)
        if cmd.count >= 2 {
            #expect(cmd[1] == "daemon")
        }
