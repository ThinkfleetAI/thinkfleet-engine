...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: ThinkFleetBotProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: ThinkFleetBotProtocol.AnyCodable] = [
            "presence": .init([ThinkFleetBotProtocol.AnyCodable(entry)]),
        ]
        let payload = ThinkFleetBotProtocol.AnyCodable(payloadMap)
