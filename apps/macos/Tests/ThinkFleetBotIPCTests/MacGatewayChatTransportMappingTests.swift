...
@Suite struct MacGatewayChatTransportMappingTests {
    @Test func snapshotMapsToHealth() {
        let snapshot = Snapshot(
            presence: [],
            health: ThinkFleetBotProtocol.AnyCodable(["ok": ThinkFleetBotProtocol.AnyCodable(false)]),
    ...
    @Test func healthEventMapsToHealth() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: ThinkFleetBotProtocol.AnyCodable(["ok": ThinkFleetBotProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func chatEventMapsToChat() {
        let payload = ThinkFleetBotProtocol.AnyCodable([
            "runId": ThinkFleetBotProtocol.AnyCodable("run-1"),
            "sessionKey": ThinkFleetBotProtocol.AnyCodable("main"),
            "state": ThinkFleetBotProtocol.AnyCodable("final"),
        ])
    ...
    @Test func unknownEventMapsToNil() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: ThinkFleetBotProtocol.AnyCodable(["a": ThinkFleetBotProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
