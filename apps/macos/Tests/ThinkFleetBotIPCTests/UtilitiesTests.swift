...
@Suite(.serialized) struct UtilitiesTests {
    ...
    @Test func gatewayEntrypointPrefersDistOverBin() throws {
        ...
        let bin = tmp.appendingPathComponent("bin/thinkfleetbot.js")
    ...
    @Test func logLocatorPicksNewestLogFile() throws {
        let fm = FileManager()
        let dir = URL(fileURLWithPath: "/tmp/thinkfleetbot", isDirectory: true)
        try? fm.createDirectory(at: dir, withIntermediateDirectories: true)

        let older = dir.appendingPathComponent("thinkfleetbot-old-\(UUID().uuidString).log")
        let newer = dir.appendingPathComponent("thinkfleetbot-new-\(UUID().uuidString).log")
