...
import Foundation
import Testing
@testable import ThinkFleetChatUI
...
private final class TestChatTransport: @unchecked Sendable, ThinkFleetBotChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [ThinkFleetBotChatHistoryPayload]
    private let sessionsResponses: [ThinkFleetBotChatSessionsListResponse]

    private let stream: AsyncStream<ThinkFleetBotChatTransportEvent>
    private let continuation: AsyncStream<ThinkFleetBotChatTransportEvent>.Continuation

    init(
        historyResponses: [ThinkFleetBotChatHistoryPayload],
        sessionsResponses: [ThinkFleetBotChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<ThinkFleetBotChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> ThinkFleetBotChatHistoryPayload {
        ...
        return self.historyResponses.last ?? ThinkFleetBotChatHistoryPayload(
    ...
    func sendMessage(
        ...
        attachments _: [ThinkFleetBotChatAttachmentPayload]) async throws -> ThinkFleetBotChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return ThinkFleetBotChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> ThinkFleetBotChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? ThinkFleetBotChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: ThinkFleetBotChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = ThinkFleetBotChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = ThinkFleetBotChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { ThinkFleetBotChatViewModel(sessionKey: "main", transport: transport) }
        ...
        transport.emit(
            .agent(
                ThinkFleetBotAgentEventPayload(
                    runId: sessionId,
                    seq: 1,
                    stream: "assistant",
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = ThinkFleetBotChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = ThinkFleetBotChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = ThinkFleetBotChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { ThinkFleetBotChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
