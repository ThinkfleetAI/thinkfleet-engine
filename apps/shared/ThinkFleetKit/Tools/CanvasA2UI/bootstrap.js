...
import "@a2ui/lit/ui";
import { themeContext } from "@thinkfleetbot/a2ui-theme-context";
...
class ThinkFleetBotA2UIHost extends LitElement {
  static properties = {
    surfaces: { state: true },
  ...
  #themeProvider = new ContextProvider(this, {
    ...
    initialValue: thinkfleetbotTheme,
  });
  ...
  static styles = css`
    :host {
      ...
      padding:
        var(--thinkfleetbot-a2ui-inset-top, 0px)
        var(--thinkfleetbot-a2ui-inset-right, 0px)
        var(--thinkfleetbot-a2ui-inset-bottom, 0px)
        var(--thinkfleetbot-a2ui-inset-left, 0px);
    }
    ...
    #surfaces {
      ...
      height: 100%;
      overflow: auto;
      padding-bottom: var(--thinkfleetbot-a2ui-scroll-pad-bottom, 0px);
    }
    ...
    .status {
      ...
      top: var(--thinkfleetbot-a2ui-status-top, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .toast {
      ...
      left: 50%;
      transform: translateX(-50%);
      bottom: var(--thinkfleetbot-a2ui-toast-bottom, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .empty {
      ...
      left: 50%;
      transform: translateX(-50%);
      top: var(--thinkfleetbot-a2ui-empty-top, var(--thinkfleetbot-a2ui-status-top, 12px));
      text-align: center;
      opacity: 0.8;
      padding: 10px 12px;
      pointer-events: none;
  ...
  connectedCallback() {
    ...
    globalThis.thinkfleetbotA2UI = api;
    globalThis.thinkfleetbotA2UI = api;
    this.addEventListener("a2uiaction", (evt) => this.#handleA2UIAction(evt));
    this.#statusListener = (evt) => this.#handleActionStatus(evt);
    globalThis.addEventListener("thinkfleetbot:a2ui-action-status", this.#statusListener);
    this.#syncSurfaces();
  }
  ...
  disconnectedCallback() {
    ...
    if (this.#statusListener) {
      globalThis.removeEventListener("thinkfleetbot:a2ui-action-status", this.#statusListener);
  ...
  #handleA2UIAction(evt) {
    ...
    globalThis.__thinkfleetbotLastA2UIAction = userAction;

    const handler =
      globalThis.webkit?.messageHandlers?.thinkfleetbotCanvasA2UIAction ??
      globalThis.webkit?.messageHandlers?.thinkfleetbotCanvasA2UIAction ??
      globalThis.thinkfleetbotCanvasA2UIAction ??
      globalThis.thinkfleetbotCanvasA2UIAction;
    if (handler?.postMessage) {
      try {
        // WebKit message handlers support structured objects; Android's JS interface expects strings.
        if (
          handler === globalThis.thinkfleetbotCanvasA2UIAction ||
          handler === globalThis.thinkfleetbotCanvasA2UIAction
        ) {
...
customElements.define("thinkfleetbot-a2ui-host", ThinkFleetBotA2UIHost);
