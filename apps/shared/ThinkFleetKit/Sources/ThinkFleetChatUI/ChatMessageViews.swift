...
struct ChatMessageBubble: View {
    let message: ThinkFleetBotChatMessage
    let style: ThinkFleetBotChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? ThinkFleetBotChatTheme.userText : ThinkFleetBotChatTheme.assistantText
    ...
    private var inlineAttachments: [ThinkFleetBotChatMessageContent] {
    ...
    private var toolCalls: [ThinkFleetBotChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [ThinkFleetBotChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? ThinkFleetBotChatTheme.userBubble
        }
        if self.style == .onboarding {
            return ThinkFleetBotChatTheme.onboardingAssistantBubble
        }
        return ThinkFleetBotChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return ThinkFleetBotChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: ThinkFleetBotChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? ThinkFleetBotChatTheme.userText : ThinkFleetBotChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: ThinkFleetBotChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(ThinkFleetBotChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(ThinkFleetBotChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [ThinkFleetBotChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: ThinkFleetBotChatTheme.assistantText)
            }
        }
    }
}
