...
@MainActor
struct ThinkFleetBotChatComposer: View {
    @Bindable var viewModel: ThinkFleetBotChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(ThinkFleetBotChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(ThinkFleetBotChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \ThinkFleetBotPendingAttachment.id)
                { (att: ThinkFleetBotPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            ThinkFleetBotPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Rectangle()
                .fill(ThinkFleetBotChatTheme.divider)
                .frame(height: 1)
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(ThinkFleetBotChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(ThinkFleetBotChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(ThinkFleetBotChatTheme.subtleCard)
        .clipShape(Capsule())
    }
