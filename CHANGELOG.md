...
Docs: https://docs.thinkfleet.dev

## 2026.1.27-beta.1
Status: beta.

### Changes
- Rebrand: rename the npm package/CLI to `thinkfleetbot`, add a `thinkfleetbot` compatibility shim, and move extensions to the `@thinkfleetbot/*` scope.
- Commands: group /help and /commands output with Telegram paging. (#2504) Thanks @hougangdev.
- macOS: limit project-local `node_modules/.bin` PATH preference to debug builds (reduce PATH hijacking risk).
- macOS: finish ThinkFleetBot app rename for macOS sources, bundle identifiers, and shared kit paths. (#2844) Thanks @fal3.
- Branding: update launchd labels, mobile bundle IDs, and logging subsystems to bot.molt (legacy com.thinkfleetbot migrations). Thanks @thewilloftheshadow.
...
- Docs: update exe.dev install instructions. (#https://github.com/thinkfleetbot/thinkfleet-engine/pull/3047) Thanks @zackerthescar.
...
### Highlights
- Providers: Ollama discovery + docs; Venice guide upgrades + cross-links. (#1606) Thanks @abhaymundhara. https://docs.thinkfleet.dev/providers/ollama https://docs.thinkfleet.dev/providers/venice
- Channels: LINE plugin (Messaging API) with rich replies + quick replies. (#1630) Thanks @plum-dawg.
- TTS: Edge fallback (keyless) + `/tts` auto modes. (#1668, #1667) Thanks @steipete, @sebslight. https://docs.thinkfleet.dev/tts
- Exec approvals: approve in-chat via `/approve` across all channels (including plugins). (#1621) Thanks @czekaj. https://docs.thinkfleet.dev/tools/exec-approvals https://docs.thinkfleet.dev/tools/slash-commands
- Telegram: DM topics as separate sessions + outbound link preview toggle. (#1597, #1700) Thanks @rohannagpal, @zerone0x. https://docs.thinkfleet.dev/channels/telegram

### Changes
- Channels: add LINE plugin (Messaging API) with rich replies, quick replies, and plugin HTTP registry. (#1630) Thanks @plum-dawg.
- TTS: add Edge TTS provider fallback, defaulting to keyless Edge with MP3 retry on format failures. (#1668) Thanks @steipete. https://docs.thinkfleet.dev/tts
- TTS: add auto mode enum (off/always/inbound/tagged) with per-session `/tts` override. (#1667) Thanks @sebslight. https://docs.thinkfleet.dev/tts
- Telegram: treat DM topics as separate sessions and keep DM history limits stable with thread suffixes. (#1597) Thanks @rohannagpal.
- Telegram: add `channels.telegram.linkPreview` to toggle outbound link previews. (#1700) Thanks @zerone0x. https://docs.thinkfleet.dev/channels/telegram
- Web search: add Brave freshness filter parameter for time-scoped results. (#1688) Thanks @JonUleis. https://docs.thinkfleet.dev/tools/web
- UI: refresh Control UI dashboard design system (colors, icons, typography). (#1745, #1786) Thanks @EnzeD, @mousberg.
- Exec approvals: forward approval prompts to chat with `/approve` for all channels (including plugins). (#1621) Thanks @czekaj. https://docs.thinkfleet.dev/tools/exec-approvals https://docs.thinkfleet.dev/tools/slash-commands
- Gateway: expose config.patch in the gateway tool with safe partial updates + restart sentinel. (#1653) Thanks @Glucksberg.
- Diagnostics: add diagnostic flags for targeted debug logs (config + env override). https://docs.thinkfleet.dev/diagnostics/flags
- Docs: expand FAQ (migration, scheduling, concurrency, model recommendations, OpenAI subscription auth, Pi sizing, hackable install, docs SSL workaround).
- Docs: add verbose installer troubleshooting guidance.
- Docs: add macOS VM guide with local/hosted options + VPS/nodes guidance. (#1693) Thanks @f-trycua.
- Docs: add Bedrock EC2 instance role setup + IAM steps. (#1625) Thanks @sergical. https://docs.thinkfleet.dev/bedrock
...
- BlueBubbles: route phone-number targets to DMs, avoid leaking routing IDs, and auto-create missing DMs (Private API required). (#1751) Thanks @tyler6204. https://docs.thinkfleet.dev/channels/bluebubbles
...
- Signal: add configurable signal-cli startup timeout + external daemon mode docs. (#1677) https://docs.thinkfleet.dev/channels/signal
...
### Highlights
- TTS: move Telegram TTS into core + enable model-driven TTS tags by default for expressive audio replies. (#1559) Thanks @Glucksberg. https://docs.thinkfleet.dev/tts
- Gateway: add `/tools/invoke` HTTP endpoint for direct tool calls (auth + tool policy enforced). (#1575) Thanks @vignesh07. https://docs.thinkfleet.dev/gateway/tools-invoke-http-api
- Heartbeat: per-channel visibility controls (OK/alerts/indicator). (#1452) Thanks @dlauer. https://docs.thinkfleet.dev/gateway/heartbeat
- Deploy: add Fly.io deployment support + guide. (#1570) https://docs.thinkfleet.dev/platforms/fly
- Channels: add Tlon/Urbit channel plugin (DMs, group mentions, thread replies). (#1544) Thanks @wca4a. https://docs.thinkfleet.dev/channels/tlon

### Changes
- Channels: allow per-group tool allow/deny policies across built-in + plugin channels. (#1546) Thanks @adam91holt. https://docs.thinkfleet.dev/multi-agent-sandbox-tools
- Agents: add Bedrock auto-discovery defaults + config overrides. (#1553) Thanks @fal3. https://docs.thinkfleet.dev/bedrock
- CLI: add `thinkfleetbot system` for system events + heartbeat controls; remove standalone `wake`. (commit 71203829d) https://docs.thinkfleet.dev/cli/system
- CLI: add live auth probes to `thinkfleetbot models status` for per-profile verification. (commit 40181afde) https://docs.thinkfleet.dev/cli/models
- CLI: restart the gateway by default after `thinkfleetbot update`; add `--no-restart` to skip it. (commit 2c85b1b40)
- Browser: add node-host proxy auto-routing for remote gateways (configurable per gateway/node). (commit c3cb26f7c)
- Plugins: add optional `llm-task` JSON-only tool for workflows. (#1498) Thanks @vignesh07. https://docs.thinkfleet.dev/tools/llm-task
...
- Docs: add cron vs heartbeat decision guide (with Lobster workflow notes). (#1533) Thanks @JustYannicc. https://docs.thinkfleet.dev/automation/cron-vs-heartbeat
- Docs: clarify HEARTBEAT.md empty file skips heartbeats, missing file still runs. (#1535) Thanks @JustYannicc. https://docs.thinkfleet.dev/gateway/heartbeat
...
- CLI: auth probe output polish (table output, inline errors, reduced noise, and wrap fixes in `thinkfleetbot models status`). (commit da3f2b489, commit 00ae21bed, commit 31e59cd58, commit f7dc27f2d, commit 438e782f8, commit 886752217, commit aabe0bed3, commit 81535d512, commit c63144ab1)
...
- Doctor: honor THINKFLEETBOT_GATEWAY_TOKEN for auth checks and security audit token reuse. (#1448) Thanks @azade-c.
...
### Fixes
- Control UI: ignore bootstrap identity placeholder text for avatar values and fall back to the default avatar. https://docs.thinkfleet.dev/cli/agents https://docs.thinkfleet.dev/web/control-ui
- Slack: remove deprecated `filetype` field from `files.uploadV2` to eliminate API warnings. (#1447)

## 2026.1.21

### Changes
- Highlight: Lobster optional plugin tool for typed workflows + approval gates. https://docs.thinkfleet.dev/tools/lobster
- Lobster: allow workflow file args via `argsJson` in the plugin tool. https://docs.thinkfleet.dev/tools/lobster
...
- CLI: flatten node service commands under `thinkfleetbot node` and remove `service node` docs.
- CLI: move gateway service commands under `thinkfleetbot gateway` and add `gateway probe` for reachability.
- Sessions: add per-channel reset overrides via `session.resetByChannel`. (#1353) Thanks @cash-echo-bot.
- Agents: add identity avatar config support and Control UI avatar rendering. (#1329, #1424) Thanks @dlauer.
- UI: show per-session assistant identity in the Control UI. (#1420) Thanks @robbyczgw-cla.
- CLI: add `thinkfleetbot update wizard` for interactive channel selection and restart prompts. https://docs.thinkfleet.dev/cli/update
- Signal: add typing indicators and DM read receipts via signal-cli.
- MSTeams: add file uploads, adaptive cards, and attachment handling improvements. (#1410) Thanks @Evizero.
- Onboarding: remove the run setup-token auth option (paste setup-token or reuse CLI creds instead).
- Docs: add troubleshooting entry for gateway.mode blocking gateway start. https://docs.thinkfleet.dev/gateway/troubleshooting
- Docs: add /model allowlist troubleshooting note. (#1405)
- Docs: add per-message Gmail search example for gog. (#1220) Thanks @mbelinky.

### Breaking
- **BREAKING:** Control UI now rejects insecure HTTP without device identity by default. Use HTTPS (Tailscale Serve) or set `gateway.controlUi.allowInsecureAuth: true` to allow token-only auth. https://docs.thinkfleet.dev/web/control-ui#insecure-http
...
### Changes
- Control UI: add copy-as-markdown with error feedback. (#1345) https://docs.thinkfleet.dev/web/control-ui
- Control UI: drop the legacy list view. (#1345) https://docs.thinkfleet.dev/web/control-ui
- TUI: add syntax highlighting for code blocks. (#1200) https://docs.thinkfleet.dev/tui
- TUI: session picker shows derived titles, fuzzy search, relative times, and last message preview. (#1271) https://docs.thinkfleet.dev/tui
- TUI: add a searchable model picker for quicker model selection. (#1198) https://docs.thinkfleet.dev/tui
- TUI: add input history (up/down) for submitted messages. (#1348) https://docs.thinkfleet.dev/tui
- ACP: add `thinkfleetbot acp` for IDE integrations. https://docs.thinkfleet.dev/cli/acp
- ACP: add `thinkfleetbot acp client` interactive harness for debugging. https://docs.thinkfleet.dev/cli/acp
- Skills: add download installs with OS-filtered options. https://docs.thinkfleet.dev/tools/skills
- Skills: add the local sherpa-onnx-tts skill. https://docs.thinkfleet.dev/tools/skills
- Memory: add hybrid BM25 + vector search (FTS5) with weighted merging and fallback. https://docs.thinkfleet.dev/concepts/memory
- Memory: add SQLite embedding cache to speed up reindexing and frequent updates. https://docs.thinkfleet.dev/concepts/memory
- Memory: add OpenAI batch indexing for embeddings when configured. https://docs.thinkfleet.dev/concepts/memory
- Memory: enable OpenAI batch indexing by default for OpenAI embeddings. https://docs.thinkfleet.dev/concepts/memory
- Memory: allow parallel OpenAI batch indexing jobs (default concurrency: 2). https://docs.thinkfleet.dev/concepts/memory
- Memory: render progress immediately, color batch statuses in verbose logs, and poll OpenAI batch status every 2s by default. https://docs.thinkfleet.dev/concepts/memory
- Memory: add `--verbose` logging for memory status + batch indexing details. https://docs.thinkfleet.dev/concepts/memory
- Memory: add native Gemini embeddings provider for memory search. (#1151) https://docs.thinkfleet.dev/concepts/memory
- Browser: allow config defaults for efficient snapshots in the tool/CLI. (#1336) https://docs.thinkfleet.dev/tools/browser
- Nostr: add the Nostr channel plugin with profile management + onboarding defaults. (#1323) https://docs.thinkfleet.dev/channels/nostr
- Matrix: migrate to matrix-bot-sdk with E2EE support, location handling, and group allowlist upgrades. (#1298) https://docs.thinkfleet.dev/channels/matrix
- Slack: add HTTP webhook mode via Bolt HTTP receiver. (#1143) https://docs.thinkfleet.dev/channels/slack
- Telegram: enrich forwarded-message context with normalized origin details + legacy fallback. (#1090) https://docs.thinkfleet.dev/channels/telegram
- Discord: fall back to `/skill` when native command limits are exceeded. (#1287)
- Discord: expose `/skill` globally. (#1287)
- Zalouser: add channel dock metadata, config schema, setup wiring, probe, and status issues. (#1219) https://docs.thinkfleet.dev/plugins/zalouser
- Plugins: require manifest-embedded config schemas with preflight validation warnings. (#1272) https://docs.thinkfleet.dev/plugins/manifest
- Plugins: move channel catalog metadata into plugin manifests. (#1290) https://docs.thinkfleet.dev/plugins/manifest
- Plugins: align Nextcloud Talk policy helpers with core patterns. (#1290) https://docs.thinkfleet.dev/plugins/manifest
- Plugins/UI: let channel plugin metadata drive UI labels/icons and cron channel options. (#1306) https://docs.thinkfleet.dev/web/control-ui
- Agents/UI: add agent avatar support in identity config, IDENTITY.md, and the Control UI. (#1329) https://docs.thinkfleet.dev/gateway/configuration
- Plugins: add plugin slots with a dedicated memory slot selector. https://docs.thinkfleet.dev/plugins/agent-tools
- Plugins: ship the bundled BlueBubbles channel plugin (disabled by default). https://docs.thinkfleet.dev/channels/bluebubbles
- Plugins: migrate bundled messaging extensions to the plugin SDK and resolve plugin-sdk imports in the loader.
- Plugins: migrate the Zalo plugin to the shared plugin SDK runtime. https://docs.thinkfleet.dev/channels/zalo
- Plugins: migrate the Zalo Personal plugin to the shared plugin SDK runtime. https://docs.thinkfleet.dev/plugins/zalouser
- Plugins: allow optional agent tools with explicit allowlists and add the plugin tool authoring guide. https://docs.thinkfleet.dev/plugins/agent-tools
- Plugins: auto-enable bundled channel/provider plugins when configuration is present.
- Plugins: sync plugin sources on channel switches and update npm-installed plugins during `thinkfleetbot update`.
- Plugins: share npm plugin update logic between `thinkfleetbot update` and `thinkfleetbot plugins update`.
...
- Usage: add `/usage cost` summaries and macOS menu cost charts. https://docs.thinkfleet.dev/reference/api-usage-costs
- Security: warn when <=300B models run without sandboxing while web tools are enabled. https://docs.thinkfleet.dev/cli/security
- Exec: add host/security/ask routing for gateway + node exec. https://docs.thinkfleet.dev/tools/exec
- Exec: add `/exec` directive for per-session exec defaults (host/security/ask/node). https://docs.thinkfleet.dev/tools/exec
- Exec approvals: migrate approvals to `~/.thinkfleetbot/exec-approvals.json` with per-agent allowlists + skill auto-allow toggle, and add approvals UI + node exec lifecycle events. https://docs.thinkfleet.dev/tools/exec-approvals
- Nodes: add headless node host (`thinkfleetbot node start`) for `system.run`/`system.which`. https://docs.thinkfleet.dev/cli/node
- Nodes: add node daemon service install/status/start/stop/restart. https://docs.thinkfleet.dev/cli/node
- Bridge: add `skills.bins` RPC to support node host auto-allow skill bins.
- Sessions: add daily reset policy with per-type overrides and idle windows (default 4am local), preserving legacy idle-only configs. (#1146) https://docs.thinkfleet.dev/concepts/session
- Sessions: allow `sessions_spawn` to override thinking level for sub-agent runs. https://docs.thinkfleet.dev/tools/subagents
- Channels: unify thread/topic allowlist matching + command/mention gating helpers across core providers. https://docs.thinkfleet.dev/concepts/groups
- Models: add Qwen Portal OAuth provider support. (#1120) https://docs.thinkfleet.dev/providers/qwen
- Onboarding: add allowlist prompts and username-to-id resolution across core and extension channels. https://docs.thinkfleet.dev/start/onboarding
- Docs: clarify allowlist input types and onboarding behavior for messaging channels. https://docs.thinkfleet.dev/start/onboarding
- Docs: refresh Android node discovery docs for the Gateway WS service type. https://docs.thinkfleet.dev/platforms/android
- Docs: surface Amazon Bedrock in provider lists and clarify Bedrock auth env vars. (#1289) https://docs.thinkfleet.dev/bedrock
- Docs: clarify WhatsApp voice notes. https://docs.thinkfleet.dev/channels/whatsapp
- Docs: clarify Windows WSL portproxy LAN access notes. https://docs.thinkfleet.dev/platforms/windows
- Docs: refresh bird skill install metadata and usage notes. (#1302) https://docs.thinkfleet.dev/tools/browser-login
...
- Build: use tsgo for dev/watch builds by default (opt out with `THINKFLEETBOT_TS_COMPILER=tsc`).
...
### Breaking
- **BREAKING:** Reject invalid/unknown config entries and refuse to start the gateway for safety. Run `thinkfleetbot doctor --fix` to repair, then update plugins (`thinkfleetbot plugins update`) if you use any.

### Fixes
- Discovery: shorten Bonjour DNS-SD service type to `_thinkfleetbot-gw._tcp` and update discovery clients/docs.
...
- CLI: keep `thinkfleetbot logs` output resilient to broken pipes while preserving progress output.
...
- Memory: show total file counts + scan issues in `thinkfleetbot memory status`.
...
- Memory: probe sqlite-vec availability in `thinkfleetbot memory status`.
...
### Highlights
- Hooks: add hooks system with bundled hooks, CLI tooling, and docs. (#1028) — thanks @ThomsenDrake. https://docs.thinkfleet.dev/hooks
- Media: add inbound media understanding (image/audio/video) with provider + CLI fallbacks. https://docs.thinkfleet.dev/nodes/media-understanding
- Plugins: add Zalo Personal plugin (`@thinkfleetbot/zalouser`) and unify channel directory for plugins. (#1032) — thanks @suminhthanh. https://docs.thinkfleet.dev/plugins/zalouser
- Models: add Vercel AI Gateway auth choice + onboarding updates. (#1016) — thanks @timolins. https://docs.thinkfleet.dev/providers/vercel-ai-gateway
- Sessions: add `session.identityLinks` for cross-platform DM session li  nking. (#1033) — thanks @thewilloftheshadow. https://docs.thinkfleet.dev/concepts/session
- Web search: add `country`/`language` parameters (schema + Brave API) and docs. (#1046) — thanks @YuriNachos. https://docs.thinkfleet.dev/tools/web

### Breaking
- **BREAKING:** `thinkfleetbot message` and message tool now require `target` (dropping `to`/`channelId` for destinations). (#1034) — thanks @tobalsan.
- **BREAKING:** Channel auth now prefers config over env for Discord/Telegram/Matrix (env is fallback only). (#1040) — thanks @thewilloftheshadow.
- **BREAKING:** Drop legacy `chatType: "room"` support; use `chatType: "channel"`.
- **BREAKING:** remove legacy provider-specific target resolution fallbacks; target resolution is centralized with plugin hints + directory lookups.
- **BREAKING:** `thinkfleetbot hooks` is now `thinkfleetbot webhooks`; hooks live under `thinkfleetbot hooks`. https://docs.thinkfleet.dev/cli/webhooks
- **BREAKING:** `thinkfleetbot plugins install <path>` now copies into `~/.thinkfleetbot/extensions` (use `--link` to keep path-based loading).
...
- Tools: add `exec` PTY support for interactive sessions. https://docs.thinkfleet.dev/tools/exec
...
- Directory: unify `thinkfleetbot directory` across channels and plugin channels.
...
- Hooks: add hook pack installs (npm/path/zip/tar) with `thinkfleetbot.hooks` manifests and `thinkfleetbot hooks install/update`.
...
- Security: default-deny slash/control commands unless a channel computed `CommandAuthorized` (fixes accidental “open” behavior), and ensure WhatsApp + Zalo plugin channels gate inline `/…` tokens correctly. https://docs.thinkfleet.dev/gateway/security
- Security: redact sensitive text in gateway WS logs.
- Tools: cap pending `exec` process output to avoid unbounded buffers.
- CLI: speed up `thinkfleetbot sandbox-explain` by avoiding heavy plugin imports when normalizing channel ids.
...
### Highlights
- Plugins: add provider auth registry + `thinkfleetbot models auth login` for plugin-driven OAuth/API key flows.
...
- **BREAKING:** Microsoft Teams is now a plugin; install `@thinkfleetbot/msteams` via `thinkfleetbot plugins install @thinkfleetbot/msteams`.
- **BREAKING:** Channel auth now prefers config over env for Discord/Telegram/Matrix (env is fallback only). (#1040) — thanks @thewilloftheshadow.

### Changes
- UI/Apps: move channel/config settings to schema-driven forms and rename Connections → Channels. (#1040) — thanks @thewilloftheshadow.
- CLI: set process titles to `thinkfleetbot-<command>` for clearer process listings.
...
- Security: warn on weak model tiers (Haiku, below GPT-5, below Claude 4.5) in `thinkfleetbot security audit`.
- Apps: store node auth tokens encrypted (Keychain/SecurePrefs).
- Daemon: share profile/state-dir resolution across service helpers and honor `THINKFLEETBOT_STATE_DIR` for Windows task scripts.
...
- Auth/Status: keep auth profiles sticky per session (rotate on compaction/new), surface provider usage headers in `/status` and `thinkfleetbot models status`, and update docs.
- CLI: add `--json` output for `thinkfleetbot daemon` lifecycle/install commands.
...
- Fix: make `thinkfleetbot update` auto-update global installs when installed via a package manager.
- Fix: list model picker entries as provider/model pairs for explicit selection. (#970) — thanks @mcinteerj.
- Fix: align OpenAI image-gen defaults with DALL-E 3 standard quality and document output formats. (#880) — thanks @mkbehr.
- Fix: persist `gateway.mode=local` after selecting Local run mode in `thinkfleetbot configure`, even if no other sections are chosen.
...
- Security: expanded `thinkfleetbot security audit` (+ `--fix`), detect-secrets CI scan, and a `SECURITY.md` reporting policy.

### Changes
- Docs: clarify per-agent auth stores, sandboxed skill binaries, and elevated semantics.
- Docs: add FAQ entries for missing provider auth after adding agents and Gemini thinking signature errors.
- Agents: add optional auth-profile copy prompt on `agents add` and improve auth error messaging.
- Security: expand `thinkfleetbot security audit` checks (model hygiene, config includes, plugin allowlists, exposure matrix) and extend `--fix` to tighten more sensitive state paths.
...
- Browser: add Chrome extension relay takeover mode (toolbar button), plus `thinkfleetbot browser extension install/path` and remote browser control (standalone server + token auth).
...
- Gateway/Dev: ensure `pnpm gateway:dev` always uses the dev profile config + state (`~/.thinkfleetbot-dev`).
...
- Memory: new `thinkfleetbot memory` CLI plus `memory_search`/`memory_get` tools with snippets + line ranges; index stored under `~/.thinkfleetbot/memory/{agentId}.sqlite` with watch-on-by-default.
...
- Gateway/CLI: add Tailscale binary discovery, custom bind mode, and probe auth retry; add `thinkfleetbot dashboard` auto-open flow; default native slash commands to `"auto"` with per-provider overrides. (#740) — thanks @jeffersonwarrior.
...
### Installer
- Install: run `thinkfleetbot doctor --non-interactive` after git installs/updates and nudge daemon restarts when detected.
...
- Onboarding/Auth: honor `THINKFLEETBOT_AGENT_DIR` / `PI_CODING_AGENT_DIR` when writing auth profiles (MiniMax). (#829) — thanks @roshanasingh4.
...
- Gateway/CLI: honor `THINKFLEETBOT_LAUNCHD_LABEL` / `THINKFLEETBOT_SYSTEMD_UNIT` overrides; `agents.list` respects explicit config; reduce noisy loopback WS logs during tests; run `thinkfleetbot doctor --non-interactive` during updates. (#781) — thanks @ronyrus.
...
- Plugins: add `thinkfleetbot plugins install` (path/tgz/npm), plus `list|info|enable|disable|doctor` UX.
...
- macOS: prompt to install the global `thinkfleetbot` CLI when missing in local mode; install via `thinkfleet.dev/install-cli.sh` (no onboarding) and use external launchd/CLI instead of the embedded gateway runtime.
...
- Installer tests: add root+non-root docker smokes, CI workflow to fetch thinkfleet.dev scripts and run install sh/cli with onboarding skipped.
- Installer UX: support `THINKFLEETBOT_NO_ONBOARD=1` for non-interactive installs; fix npm prefix on Linux and auto-install git.
...
- Plugins: treat `plugins.load.paths` directory entries as package roots when they contain `package.json` + `thinkfleetbot.extensions`; load plugin packages from config dirs; extract archives without system tar.
- Config: expand `~` in `THINKFLEETBOT_CONFIG_PATH` and common path-like config fields (including `plugins.load.paths`); guard invalid `$include` paths. (#731) — thanks @pasogott.
...
### Highlights
- CLI: `thinkfleetbot status` now table-based + shows OS/update/gateway/daemon/agents/sessions; `status --all` adds a full read-only debug report (tables, log tails, Tailscale summary, and scan progress via OSC-9 + spinner).
...
- CLI: add `thinkfleetbot update` (safe-ish git checkout update) + `--update` shorthand. (#673) — thanks @fm1randa.
...
- CLI: add `thinkfleetbot reset` and `thinkfleetbot uninstall` flows (interactive + non-interactive) plus docker cleanup smoke test.
...
- CLI/Gateway: clarify that `thinkfleetbot gateway status` reports RPC health (connect + RPC) and shows RPC failures separately from connect failures.
...
- Docs: make `thinkfleetbot status` the first diagnostic step, clarify `status --deep` behavior, and document `/whoami` + `/id`.
...
- Sandbox: add `thinkfleetbot sandbox explain` (effective policy inspector + fix-it keys); improve “sandbox jail” tool-policy/elevated errors with actionable config key paths; link to docs.
...
- CLI: respect `THINKFLEETBOT_STATE_DIR` for node pairing + voice wake settings storage. (#664) — thanks @azade-c.
...
- Gateway/Agent: accept image attachments on `agent` (multimodal message) and add live gateway image probe (`THINKFLEETBOT_LIVE_GATEWAY_IMAGE_PROBE=1`).
- CLI: `thinkfleetbot sessions` now includes `elev:*` + `usage:*` flags in the table output.
- CLI/Pairing: accept positional provider for `pairing list|approve` (npm-run compatible); update docs/bot hints.
- Branding: normalize legacy casing/branding to “ThinkFleetBot” (CLI, status, docs).
- Auto-reply: fix native `/model` not updating the actual chat session (Telegram/Slack/Discord). (#646)
- Doctor: offer to run `thinkfleetbot update` first on git installs (keeps doctor output aligned with latest).
- Doctor: avoid false legacy workspace warning when install dir is `~/thinkfleetbot`. (#660)
...
### Breaking
- CLI: `thinkfleetbot message` now subcommands (`message send|poll|...`) and requires `--provider` unless only one provider configured.
...
- Models/Auth: setup-token + token auth profiles; `thinkfleetbot models auth order {get,set,clear}`; per-agent auth candidates in `/model status`; OAuth expiry checks in doctor/status.
...
- Config/Onboarding: inline env vars; OpenAI API key flow to shared `~/.thinkfleetbot/.env`; Opus 4.5 default prompt for Anthropic auth; QuickStart auto-install gateway (Node-only) + provider picker tweaks + skip-systemd flags; TUI bootstrap prompt (`tui --message`); remove Bun runtime choice.
...
- Gateway/Daemon/Doctor: atomic config writes; repair gateway service entrypoint + install switches; non-interactive legacy migrations; systemd unit alignment + KillMode=process; node bridge keepalive/pings; Launch at Login persistence; bundle ThinkFleetBotKit resources + Swift 6.2 compat dylib; relay version check + remove smoke test; regen Swift GatewayModels + keep agent provider string; cron jobId alias + channel alias migration + main session key normalization; heartbeat Telegram accountId resolution; avoid WhatsApp fallback for internal runs; gateway listener error wording; serveBaseUrl param; honor gateway --dev; fix wide-area discovery updates; align agents.defaults schema; provider account metadata in daemon status; refresh Carbon patch for gateway fixes; restore doctor prompter initialValue handling.
...
- **SECURITY (update ASAP):** inbound DMs are now **locked down by default** on Telegram/WhatsApp/Signal/iMessage/Discord/Slack.
  ...
  - Approve requests via `thinkfleetbot pairing list <provider>` + `thinkfleetbot pairing approve <provider> <code>`.
...
- **Gateway/CLI UX:** `thinkfleetbot logs`, cron list colors/aliases, docs search, agents list/add/delete flows, status usage snapshots, runtime/auth source display, and `/status`/commands auth unification.
...
- **Docs:** new FAQ/Skills Hub/config examples/showcase entries and clarified auth, sandbox, and systemd docs.
...
- Env: load global `$THINKFLEETBOT_STATE_DIR/.env` (`~/.thinkfleetbot/.env`) as a fallback after CWD `.env`.
...
- Commands: unify /status (inline) and command auth across providers; group bypass for authorized control commands; remove Discord /thinkfleet slash handler.
- CLI: run `thinkfleetbot agent` via the Gateway by default; use `--local` to force embedded mode.
