...
- CLI: add `thinkfleetbot system` for system events + heartbeat controls; remove standalone `wake`. (commit 71203829d) https://docs.molt.bot/cli/system
- CLI: add live auth probes to `thinkfleetbot models status` for per-profile verification. (commit 40181afde) https://docs.molt.bot/cli/models
...
- CLI: add `thinkfleetbot update wizard` for interactive channel selection and restart prompts. https://docs.molt.bot/cli/update
...
- ACP: add `thinkfleetbot acp` for IDE integrations. https://docs.molt.bot/cli/acp
- ACP: add `thinkfleetbot acp client` interactive harness for debugging. https://docs.molt.bot/cli/acp
...
- Exec approvals: migrate approvals to `~/.thinkfleetbot/exec-approvals.json` with per-agent allowlists + skill auto-allow toggle, and add approvals UI + node exec lifecycle events. https://docs.molt.bot/tools/exec-approvals
- Nodes: add headless node host (`thinkfleetbot node start`) for `system.run`/`system.which`. https://docs.molt.bot/cli/node
...
- Plugins: add Zalo Personal plugin (`@thinkfleetbot/zalouser`) and unify channel directory for plugins. (#1032) — thanks @suminhthanh. https://docs.molt.bot/plugins/zalouser
...
- **BREAKING:** `thinkfleetbot hooks` is now `thinkfleetbot webhooks`; hooks live under `thinkfleetbot hooks`. https://docs.molt.bot/cli/webhooks
...
- macOS: prompt to install the global `thinkfleetbot` CLI when missing in local mode; install via `molt.bot/install-cli.sh` (no onboarding) and use external launchd/CLI instead of the embedded gateway runtime.
