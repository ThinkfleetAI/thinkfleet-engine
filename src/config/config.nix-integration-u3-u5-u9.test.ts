...
describe("Nix integration (U3, U5, U9)", () => {
  describe("U3: isNixMode env var detection", () => {
    it("isNixMode is false when THINKFLEET_NIX_MODE is not set", async () => {
      await withEnvOverride({ THINKFLEET_NIX_MODE: undefined }, async () => {
        const { isNixMode } = await import("./config.js");
        expect(isNixMode).toBe(false);
      });
    });

    it("isNixMode is false when THINKFLEET_NIX_MODE is empty", async () => {
      await withEnvOverride({ THINKFLEET_NIX_MODE: "" }, async () => {
        const { isNixMode } = await import("./config.js");
        expect(isNixMode).toBe(false);
      });
    });

    it("isNixMode is false when THINKFLEET_NIX_MODE is not '1'", async () => {
      await withEnvOverride({ THINKFLEET_NIX_MODE: "true" }, async () => {
        const { isNixMode } = await import("./config.js");
        expect(isNixMode).toBe(false);
      });
    });

    it("isNixMode is true when THINKFLEET_NIX_MODE=1", async () => {
      await withEnvOverride({ THINKFLEET_NIX_MODE: "1" }, async () => {
        const { isNixMode } = await import("./config.js");
  ...
  describe("U5: CONFIG_PATH and STATE_DIR env var overrides", () => {
    ...
    it("CONFIG_PATH defaults to ~/.thinkfleet/thinkfleet.json when env not set", async () => {
      await withEnvOverride(
        ...
        async () => {
          ...
          expect(CONFIG_PATH).toMatch(/\.thinkfleet[\\/]thinkfleet\.json$/);
        },
      );
    });
    ...
    it("CONFIG_PATH respects THINKFLEET_CONFIG_PATH override", async () => {
      await withEnvOverride(
        ...
        async () => {
          const { CONFIG_PATH } = await import("./config.js");
          expect(CONFIG_PATH).toBe(path.resolve("/nix/store/abc/thinkfleet.json"));
        },
      );
    });
    ...
    it("CONFIG_PATH expands ~ in THINKFLEET_CONFIG_PATH override", async () => {
      await withTempHome(async (home) => {
        await withEnvOverride(
          ...
          async () => {
            const { CONFIG_PATH } = await import("./config.js");
            expect(CONFIG_PATH).toBe(path.join(home, ".thinkfleet", "custom.json"));
          },
        );
      });
    });
    ...
    it("CONFIG_PATH uses STATE_DIR when only state dir is overridden", async () => {
      await withEnvOverride(
        ...
        async () => {
          const { CONFIG_PATH } = await import("./config.js");
          expect(CONFIG_PATH).toBe(path.join(path.resolve("/custom/state"), "thinkfleet.json"));
        },
      );
    });
  });
  ...
  describe("U5b: tilde expansion for config paths", () => {
    it("expands ~ in common path-ish config fields", async () => {
      await withTempHome(async (home) => {
        const configDir = path.join(home, ".thinkfleet");
        ...
        await fs.writeFile(
          path.join(pluginDir, "thinkfleet.plugin.json"),
          JSON.stringify(
            {
              id: "demo-plugin",
        ...
        await fs.writeFile(
          path.join(configDir, "thinkfleet.json"),
          JSON.stringify(
            {
              plugins: {
                load: {
                ...
                list: [
                  {
                    id: "main",
                    workspace: "~/ws-agent",
                    agentDir: "~/.thinkfleet/agents/main",
              ...
              channels: {
                whatsapp: {
                  accounts: {
                    personal: {
                      authDir: "~/.thinkfleet/credentials/wa-personal",
                    },
                  },
                },
              },
        ...
        expect(cfg.agents?.list?.[0]?.agentDir).toBe(
          path.join(home, ".thinkfleet", "agents", "main"),
        );
        expect(cfg.agents?.list?.[0]?.sandbox?.workspaceRoot).toBe(path.join(home, "sandbox-root"));
        expect(cfg.channels?.whatsapp?.accounts?.personal?.authDir).toBe(
          path.join(home, ".thinkfleet", "credentials", "wa-personal"),
        );
      });
    });
  });
  ...
  describe("U6: gateway port resolution", () => {
    it("uses default when env and config are unset", async () => {
      await withEnvOverride({ THINKFLEET_GATEWAY_PORT: undefined }, async () => {
        const { DEFAULT_GATEWAY_PORT, resolveGatewayPort } = await import("./config.js");
        expect(resolveGatewayPort({})).toBe(DEFAULT_GATEWAY_PORT);
      });
    });

    it("prefers THINKFLEET_GATEWAY_PORT over config", async () => {
      await withEnvOverride({ THINKFLEET_GATEWAY_PORT: "19001" }, async () => {
    ...
    it("falls back to config when env is invalid", async () => {
      await withEnvOverride({ THINKFLEET_GATEWAY_PORT: "nope" }, async () => {
        const { resolveGatewayPort } = await import("./config.js");
