...
const ETC_THINKFLEET_DIR = path.join(ROOT_DIR, "etc", "thinkfleet");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "thinkfleet.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcThinkfleetPath(...parts: string[]) {
  return path.join(ETC_THINKFLEET_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcThinkfleetPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "thinkfleet.json"))).toEqual({
      shared: true,
    });
  });
});
