...
import type { ThinkfleetConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import { validateConfigObjectWithPlugins } from "./validation.js";
import { compareThinkfleetVersions } from "./version.js";
...
const SHELL_ENV_EXPECTED_KEYS = [
  ...
  "SLACK_BOT_TOKEN",
  "SLACK_APP_TOKEN",
  "THINKFLEET_GATEWAY_TOKEN",
  "THINKFLEET_GATEWAY_PASSWORD",
];
...
function coerceConfig(value: unknown): ThinkfleetConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as ThinkfleetConfig;
}
...
function stampConfigVersion(cfg: ThinkfleetConfig): ThinkfleetConfig {
...
function warnIfConfigFromFuture(cfg: ThinkfleetConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) return;
  const cmp = compareThinkfleetVersions(VERSION, touched);
  if (cmp === null) return;
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer Thinkfleet (${touched}); current version is ${VERSION}.`,
    );
  }
}

function applyConfigEnv(cfg: ThinkfleetConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): ThinkfleetConfig {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as ThinkfleetConfig, deps.env);
      }
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as ThinkfleetConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: ThinkfleetConfig) {
    clearConfigCache();
...
// module scope. `THINKFLEET_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.THINKFLEET_CONFIG_CACHE_MS?.trim();
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.THINKFLEET_DISABLE_CONFIG_CACHE?.trim()) return false;
...
export function loadConfig(): ThinkfleetConfig {
...
export async function writeConfigFile(cfg: ThinkfleetConfig): Promise<void> {
  clearConfigCache();
