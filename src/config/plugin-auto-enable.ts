import type { ThinkfleetConfig } from "./config.js";
import {
  getChatChannelMeta,
  listChatChannels,
...
function isTelegramConfigured(cfg: ThinkfleetConfig, env: NodeJS.ProcessEnv): boolean {
...
function isDiscordConfigured(cfg: ThinkfleetConfig, env: NodeJS.ProcessEnv): boolean {
...
function isSlackConfigured(cfg: ThinkfleetConfig, env: NodeJS.ProcessEnv): boolean {
  if (
...
function isSignalConfigured(cfg: ThinkfleetConfig): boolean {
...
function isIMessageConfigured(cfg: ThinkfleetConfig): boolean {
...
function isWhatsAppConfigured(cfg: ThinkfleetConfig): boolean {
...
function isGenericChannelConfigured(cfg: ThinkfleetConfig, channelId: string): boolean {
...
function collectModelRefs(cfg: ThinkfleetConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: ThinkfleetConfig, providerId: string): boolean {
...
function isPluginExplicitlyDisabled(cfg: ThinkfleetConfig, pluginId: string): boolean {
  const entry = cfg.plugins?.entries?.[pluginId];
  return entry?.enabled === false;
}

function isPluginDenied(cfg: ThinkfleetConfig, pluginId: string): boolean {
...
function ensureAllowlisted(cfg: ThinkfleetConfig, pluginId: string): ThinkfleetConfig {
...
function enablePluginEntry(cfg: ThinkfleetConfig, pluginId: string): ThinkfleetConfig {
  const entries = {
    ...cfg.plugins?.entries,
    [pluginId]: {
