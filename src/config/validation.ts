...
import type { ThinkfleetConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: ThinkfleetConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: ThinkfleetConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = ThinkfleetSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as ThinkfleetConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as ThinkfleetConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as ThinkfleetConfig)),
    ),
  };
}
