  ...
  it("expands tilde for path-ish keys only", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = normalizeConfigPaths({
        ...
        logging: { file: "~/.thinkfleet/logs/thinkfleet.log" },
        hooks: {
          path: "~/.thinkfleet/hooks.json5",
        ...
        channels: {
          telegram: {
            accounts: {
              personal: {
                tokenFile: "~/.thinkfleet/telegram.token",
              },
            },
          },
          imessage: {
      ...
      expect(cfg.logging?.file).toBe(path.join(home, ".thinkfleet", "logs", "thinkfleet.log"));
      expect(cfg.hooks?.path).toBe(path.join(home, ".thinkfleet", "hooks.json5"));
      ...
      expect(cfg.channels?.telegram?.accounts?.personal?.tokenFile).toBe(
        path.join(home, ".thinkfleet", "telegram.token"),
      );
