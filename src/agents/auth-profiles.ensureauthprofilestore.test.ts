...
describe("ensureAuthProfileStore", () => {
  it("migrates legacy auth.json and deletes it (PR #368)", () => {
    const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "thinkfleet-auth-profiles-"));
    try {
  ...
  it("merges main auth profiles into agent store and keeps agent overrides", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "thinkfleet-auth-merge-"));
    ...
    try {
      ...
      process.env.THINKFLEET_AGENT_DIR = mainDir;
