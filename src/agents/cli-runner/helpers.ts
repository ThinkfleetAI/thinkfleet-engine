...
/**
 * Cleanup suspended Thinkfleet CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: ThinkfleetConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-cli-images-"));
  const paths: string[] = [];
