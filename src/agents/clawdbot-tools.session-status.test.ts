...
describe("session_status tool", () => {
  it("returns a status card for the current session", async () => {
    ...
    const tool = createThinkfleetTools({ agentSessionKey: "main" }).find(
    ...
    expect(details.statusText).toContain("Thinkfleet");
  ...
  it("blocks cross-agent session_status without agent-to-agent access", async () => {
    ...
    const tool = createThinkfleetTools({ agentSessionKey: "agent:main:main" }).find(
      (candidate) => candidate.name === "session_status",
    );
    expect(tool).toBeDefined();
    if (!tool) throw new Error("missing session_status tool");
  ...
  it("scopes bare session keys to the requester agent", async () => {
    ...
    const tool = createThinkfleetTools({ agentSessionKey: "agent:support:main" }).find(
      (candidate) => candidate.name === "session_status",
    );
    expect(tool).toBeDefined();
    if (!tool) throw new Error("missing session_status tool");
