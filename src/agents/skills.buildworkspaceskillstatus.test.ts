...
describe("buildWorkspaceSkillStatus", () => {
  it("reports missing requirements and install options", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-"));
    ...
    await writeSkill({
      ...
      metadata:
        '{"thinkfleet":{"requires":{"bins":["fakebin"],"env":["ENV_KEY"],"config":["browser.enabled"]},"install":[{"id":"brew","kind":"brew","formula":"fakebin","bins":["fakebin"],"label":"Install fakebin"}]}}',
    });
  ...
  it("marks bundled skills blocked by allowlist", async () => {
    ...
    const originalBundled = process.env.THINKFLEET_BUNDLED_SKILLS_DIR;
    ...
    try {
      process.env.THINKFLEET_BUNDLED_SKILLS_DIR = bundledDir;
      const report = buildWorkspaceSkillStatus(workspaceDir, {
        managedSkillsDir: path.join(workspaceDir, ".managed"),
      ...
      if (originalBundled === undefined) {
        delete process.env.THINKFLEET_BUNDLED_SKILLS_DIR;
      } else {
        process.env.THINKFLEET_BUNDLED_SKILLS_DIR = originalBundled;
      }
    }
  });
  ...
  it("filters install options by OS", async () => {
    ...
    await writeSkill({
      ...
      metadata:
        '{"thinkfleet":{"requires":{"bins":["missing-bin"]},"install":[{"id":"mac","kind":"download","os":["darwin"],"url":"https://example.com/mac.tar.bz2"},{"id":"linux","kind":"download","os":["linux"],"url":"https://example.com/linux.tar.bz2"},{"id":"win","kind":"download","os":["win32"],"url":"https://example.com/win.tar.bz2"}]}}',
    });
