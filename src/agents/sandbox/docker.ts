...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "thinkfleet.sandbox=1");
  args.push("--label", `thinkfleet.sessionKey=${params.scopeKey}`);
  args.push("--label", `thinkfleet.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `thinkfleet.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  const result = await execDocker(
    ["inspect", "-f", '{{ index .Config.Labels "thinkfleet.configHash" }}', containerName],
    { allowFailure: true },
  );
  if (result.code !== 0) return null;
...
function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`thinkfleet sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`thinkfleet sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("thinkfleet sandbox recreate --all");
}
