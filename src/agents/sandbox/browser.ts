...
function buildSandboxBrowserResolvedConfig(params: {
  ...
  return {
    ...
    color: DEFAULT_MANAGED_BROWSER_COLOR,
    ...
    profiles: {
      managed: { cdpPort: params.cdpPort, color: DEFAULT_MANAGED_BROWSER_COLOR },
    },
  };
}
...
export async function ensureSandboxBrowser(params: {
  ...
  if (!state.exists) {
    ...
    const args = buildSandboxCreateArgs({
      ...
      labels: { "thinkfleet.sandboxBrowser": "1" },
    });
    ...
    args.push("-e", `THINKFLEET_BROWSER_HEADLESS=${params.cfg.browser.headless ? "1" : "0"}`);
    args.push(
      "-e",
      `THINKFLEET_BROWSER_ENABLE_NOVNC=${params.cfg.browser.enableNoVnc ? "1" : "0"}`,
    );
    args.push("-e", `THINKFLEET_BROWSER_CDP_PORT=${params.cfg.browser.cdpPort}`);
    args.push("-e", `THINKFLEET_BROWSER_VNC_PORT=${params.cfg.browser.vncPort}`);
    args.push("-e", `THINKFLEET_BROWSER_NOVNC_PORT=${params.cfg.browser.noVncPort}`);
  ...
  const existingProfile = existing
    ? resolveProfile(existing.bridge.state.resolved, "managed")
    : null;
  const shouldReuse =
