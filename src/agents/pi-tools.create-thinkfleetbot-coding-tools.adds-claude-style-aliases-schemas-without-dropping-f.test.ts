...
describe("createThinkfleetCodingTools", () => {
  it("uses workspaceDir for Read tool path resolution", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-ws-"));
    try {
      ...
      const tools = createThinkfleetCodingTools({ workspaceDir: tmpDir });
  ...
  it("accepts Claude Code parameter aliases for read/write/edit", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-alias-"));
    try {
      const tools = createThinkfleetCodingTools({ workspaceDir: tmpDir });
      const readTool = tools.find((tool) => tool.name === "read");
      const writeTool = tools.find((tool) => tool.name === "write");
