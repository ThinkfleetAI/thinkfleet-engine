...
function resolveCacheTraceConfig(params: CacheTraceInit): CacheTraceConfig {
  ...
  const envEnabled = parseBooleanValue(env.THINKFLEET_CACHE_TRACE);
  const enabled = envEnabled ?? config?.enabled ?? false;
  const fileOverride = config?.filePath?.trim() || env.THINKFLEET_CACHE_TRACE_FILE?.trim();
  const filePath = fileOverride
    ? resolveUserPath(fileOverride)
    : path.join(resolveStateDir(env), "logs", "cache-trace.jsonl");

  const includeMessages =
    parseBooleanValue(env.THINKFLEET_CACHE_TRACE_MESSAGES) ?? config?.includeMessages;
  const includePrompt =
    parseBooleanValue(env.THINKFLEET_CACHE_TRACE_PROMPT) ?? config?.includePrompt;
  const includeSystem =
    parseBooleanValue(env.THINKFLEET_CACHE_TRACE_SYSTEM) ?? config?.includeSystem;
