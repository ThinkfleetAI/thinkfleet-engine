...
import { createThinkfleetCodingTools } from "./pi-tools.js";
...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("thinkfleet-ws-", async (workspaceDir) => {
      await withTempDir("thinkfleet-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createThinkfleetCodingTools({ workspaceDir });
    ...
    await withTempDir("thinkfleet-ws-", async (workspaceDir) => {
      await withTempDir("thinkfleet-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello thinkfleet");
        } finally {
          process.chdir(prevCwd);
        }
      });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("thinkfleet-ws-", async (workspaceDir) => {
      await withTempDir("thinkfleet-override-", async (overrideDir) => {
        const tools = createThinkfleetCodingTools({ workspaceDir });
        const execTool = tools.find((tool) => tool.name === "exec");
        expect(execTool).toBeDefined();
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("thinkfleet-sandbox-", async (sandboxDir) => {
      await withTempDir("thinkfleet-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
          ...
          workspaceAccess: "rw",
          containerName: "thinkfleet-sbx-test",
          containerWorkdir: "/workspace",
          docker: {
            image: "thinkfleet-sandbox:bookworm-slim",
            containerPrefix: "thinkfleet-sbx-",
            workdir: "/workspace",
            readOnlyRoot: true,
            tmpfs: [],
            network: "none",
        ...
        const tools = createThinkfleetCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
