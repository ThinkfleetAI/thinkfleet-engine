...
describe("buildSandboxCreateArgs", () => {
  it("includes hardening and resource flags", () => {
    const cfg: SandboxDockerConfig = {
      ...
      apparmorProfile: "thinkfleet-sandbox",
      dns: ["1.1.1.1"],
      extraHosts: ["internal.service:10.0.0.5"],
    };

    const args = buildSandboxCreateArgs({
      name: "thinkfleet-sbx-test",
      cfg,
      scopeKey: "main",
    ...
    expect(args).toEqual(
      expect.arrayContaining([
        ...
        "thinkfleet-sbx-test",
        "--label",
        "thinkfleet.sandbox=1",
        "--label",
        "thinkfleet.sessionKey=main",
        "--label",
        "thinkfleet.createdAtMs=1700000000000",
        "--label",
        "thinkfleet.sandboxBrowser=1",
        "--read-only",
        "--tmpfs",
        "/tmp",
        "--network",
