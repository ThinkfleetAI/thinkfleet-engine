...
describe("buildWorkspaceSkillsPrompt", () => {
  it("syncs merged skills into a target workspace", async () => {
    const sourceWorkspace = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-"));
    const targetWorkspace = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-"));
  ...
  it("filters skills based on env/config gates", async () => {
    ...
    try {
      await writeSkill({
        ...
        metadata:
          '{"thinkfleet":{"requires":{"env":["GEMINI_API_KEY"]},"primaryEnv":"GEMINI_API_KEY"}}',
        body: "# Nano Banana\n",
      });
