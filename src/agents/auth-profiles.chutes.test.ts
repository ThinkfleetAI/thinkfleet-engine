...
describe("auth-profiles (chutes)", () => {
  ...
  afterEach(async () => {
    ...
    if (previousAgentDir === undefined) delete process.env.THINKFLEET_AGENT_DIR;
    else process.env.THINKFLEET_AGENT_DIR = previousAgentDir;
  ...
  it("refreshes expired Chutes OAuth credentials", async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-chutes-"));
    process.env.THINKFLEET_STATE_DIR = tempDir;
    process.env.THINKFLEET_AGENT_DIR = path.join(tempDir, "agents", "main", "agent");
