...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "thinkfleet-models-" });
}

const MODELS_CONFIG: ThinkfleetConfig = {
  models: {
    providers: {
      "custom-proxy": {
...
describe("models-config", () => {
  ...
  it("skips writing models.json when no env token or profile exists", async () => {
    await withTempHome(async (home) => {
      ...
      try {
        vi.resetModules();
        const { ensureThinkfleetModelsJson } = await import("./models-config.js");

        const agentDir = path.join(home, "agent-empty");
        const result = await ensureThinkfleetModelsJson(
          {
            models: { providers: {} },
          },
          agentDir,
  ...
  it("writes models.json for configured providers", async () => {
    await withTempHome(async () => {
      vi.resetModules();
      const { ensureThinkfleetModelsJson } = await import("./models-config.js");
      const { resolveThinkfleetAgentDir } = await import("./agent-paths.js");

      await ensureThinkfleetModelsJson(MODELS_CONFIG);

      const modelPath = path.join(resolveThinkfleetAgentDir(), "models.json");
  ...
  it("adds minimax provider when MINIMAX_API_KEY is set", async () => {
    await withTempHome(async () => {
      ...
      try {
        ...
        await ensureThinkfleetModelsJson({});
