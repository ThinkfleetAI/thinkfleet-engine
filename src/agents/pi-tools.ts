...
function resolveExecConfig(cfg: ThinkfleetConfig | undefined) {
...
export function createThinkfleetCodingTools(options?: {
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      return [createThinkfleetReadTool(freshReadTool)];
    }
