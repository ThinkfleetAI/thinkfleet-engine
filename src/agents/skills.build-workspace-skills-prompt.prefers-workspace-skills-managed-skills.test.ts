...
describe("buildWorkspaceSkillsPrompt", () => {
  ...
  it("gates by bins, config, and always", async () => {
    ...
    await writeSkill({
      ...
      metadata: '{"thinkfleet":{"requires":{"bins":["fakebin"]}}}',
    });
    await writeSkill({
      dir: path.join(skillsDir, "anybin-skill"),
      name: "anybin-skill",
      description: "Needs any bin",
      metadata: '{"thinkfleet":{"requires":{"anyBins":["missingbin","fakebin"]}}}',
    });
    await writeSkill({
      dir: path.join(skillsDir, "config-skill"),
      name: "config-skill",
      description: "Needs config",
      metadata: '{"thinkfleet":{"requires":{"config":["browser.enabled"]}}}',
    });
    await writeSkill({
      dir: path.join(skillsDir, "always-skill"),
      name: "always-skill",
      description: "Always on",
      metadata: '{"thinkfleet":{"always":true,"requires":{"env":["MISSING"]}}}',
    });
    await writeSkill({
      dir: path.join(skillsDir, "env-skill"),
      name: "env-skill",
      description: "Needs env",
      metadata: '{"thinkfleet":{"requires":{"env":["ENV_KEY"]},"primaryEnv":"ENV_KEY"}}',
    });
  ...
  it("uses skillKey for config lookups", async () => {
    ...
    await writeSkill({
      ...
      metadata: '{"thinkfleet":{"skillKey":"alias"}}',
    });
