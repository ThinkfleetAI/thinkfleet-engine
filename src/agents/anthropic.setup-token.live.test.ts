...
import { resolveThinkfleetAgentDir } from "./agent-paths.js";
import {
...
import { ensureThinkfleetModelsJson } from "./models-config.js";

const LIVE =
  isTruthyEnvValue(process.env.LIVE) || isTruthyEnvValue(process.env.THINKFLEET_LIVE_TEST);
const SETUP_TOKEN_RAW = process.env.THINKFLEET_LIVE_SETUP_TOKEN?.trim() ?? "";
const SETUP_TOKEN_VALUE = process.env.THINKFLEET_LIVE_SETUP_TOKEN_VALUE?.trim() ?? "";
const SETUP_TOKEN_PROFILE = process.env.THINKFLEET_LIVE_SETUP_TOKEN_PROFILE?.trim() ?? "";
const SETUP_TOKEN_MODEL = process.env.THINKFLEET_LIVE_SETUP_TOKEN_MODEL?.trim() ?? "";
...
async function resolveTokenSource(): Promise<TokenSource> {
  ...
  if (explicitToken) {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-setup-token-"));
  ...
  if (SETUP_TOKEN_RAW && SETUP_TOKEN_RAW !== "1" && SETUP_TOKEN_RAW !== "auto") {
    throw new Error(
      "THINKFLEET_LIVE_SETUP_TOKEN did not look like a setup-token. Use THINKFLEET_LIVE_SETUP_TOKEN_VALUE for raw tokens.",
    );
  }

  if (candidates.length === 0) {
    throw new Error(
      "No Anthropics setup-token profiles found. Set THINKFLEET_LIVE_SETUP_TOKEN_VALUE or THINKFLEET_LIVE_SETUP_TOKEN_PROFILE.",
    );
  }
  ...
  it(
    ...
    async () => {
      ...
      try {
        ...
        await ensureThinkfleetModelsJson(cfg, tokenSource.agentDir);
