...
const defaultTools = createThinkfleetCodingTools();

describe("createThinkfleetCodingTools", () => {
  it("keeps read tool image metadata intact", async () => {
    const readTool = defaultTools.find((tool) => tool.name === "read");
    expect(readTool).toBeDefined();

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-read-"));
    try {
  ...
  it("returns text content without image blocks for text files", async () => {
    const tools = createThinkfleetCodingTools();
    ...
    try {
      ...
      const contents = "Hello from thinkfleet read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandbox = {
      ...
      workspaceDir: path.join(os.tmpdir(), "thinkfleet-sandbox"),
      agentWorkspaceDir: path.join(os.tmpdir(), "thinkfleet-workspace"),
    ...
    const tools = createThinkfleetCodingTools({ sandbox });
