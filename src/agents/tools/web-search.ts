...
type WebSearchConfig = NonNullable<ThinkfleetConfig["tools"]>["web"] extends infer Web
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("thinkfleet configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.thinkfleet.ai/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://thinkfleet.ai",
      "X-Title": "Thinkfleet Web Search",
    },
    body: JSON.stringify({
      model: params.model,
      messages: [
