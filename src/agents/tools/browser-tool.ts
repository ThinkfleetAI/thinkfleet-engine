...
function resolveBrowserBaseUrl(params: {
  ...
  if (!resolved.enabled) {
    throw new Error(
      "Browser control is disabled. Set browser.enabled=true in ~/.thinkfleet/thinkfleet.json.",
    );
  }
  return undefined;
}
...
export function createBrowserTool(opts?: {
  ...
  return {
    ...
    name: "browser",
    description: [
      "Control the browser via Thinkfleet's browser control server (status/start/stop/profiles/tabs/open/snapshot/screenshot/actions).",
      'Profiles: use profile="chrome" for Chrome extension relay takeover (your existing Chrome tabs). Use profile="managed" for the isolated thinkfleet-managed browser.',
      'If the user mentions the Chrome extension / Browser Relay / toolbar button / “attach tab”, ALWAYS use profile="chrome" (do not ask which profile).',
      'When a node-hosted browser proxy is available, the tool may auto-route to it. Pin a node with node=<id|name> or target="node".',
      "Chrome extension relay needs an attached tab: user must click the Thinkfleet Browser Relay toolbar icon on the tab (badge ON). If no tab is connected, ask them to attach it.",
        ...
        case "act": {
          ...
          try {
            ...
            if (msg.includes("404:") && msg.includes("tab not found") && profile === "chrome") {
              ...
              if (!tabs.length) {
                throw new Error(
                  "No Chrome tabs are attached via the Thinkfleet Browser Relay extension. Click the toolbar icon on the tab you want to control (badge ON), then retry.",
                );
              }
              throw new Error(
