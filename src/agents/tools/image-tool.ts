...
function resolveDefaultModelRef(cfg?: ThinkfleetConfig): {
  provider: string;
  model: string;
} {
  if (cfg) {
...
function pickMaxBytes(cfg?: ThinkfleetConfig, maxBytesMb?: number): number | undefined {
...
async function runImagePrompt(params: {
  ...
  const effectiveCfg: ThinkfleetConfig | undefined = params.cfg
    ? {
        ...params.cfg,
        agents: {
          ...params.cfg.agents,
  ...
  await ensureThinkfleetModelsJson(effectiveCfg, params.agentDir);
