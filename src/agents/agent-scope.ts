...
type AgentEntry = NonNullable<NonNullable<ThinkfleetConfig["agents"]>["list"]>[number];
...
function listAgents(cfg: ThinkfleetConfig): AgentEntry[] {
  const list = cfg.agents?.list;
  if (!Array.isArray(list)) return [];
  return list.filter((entry): entry is AgentEntry => Boolean(entry && typeof entry === "object"));
}

export function listAgentIds(cfg: ThinkfleetConfig): string[] {
...
export function resolveDefaultAgentId(cfg: ThinkfleetConfig): string {
  const agents = listAgents(cfg);
  if (agents.length === 0) return DEFAULT_AGENT_ID;
...
function resolveAgentEntry(cfg: ThinkfleetConfig, agentId: string): AgentEntry | undefined {
...
export function resolveAgentWorkspaceDir(cfg: ThinkfleetConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
