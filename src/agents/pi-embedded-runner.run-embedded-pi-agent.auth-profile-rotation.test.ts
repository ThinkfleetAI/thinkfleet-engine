...
const makeConfig = (opts?: { fallbacks?: string[]; apiKey?: string }): ThinkfleetConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
describe("runEmbeddedPiAgent auth profile rotation", () => {
  it("rotates for auto-pinned profiles", async () => {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-agent-"));
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-workspace-"));
    try {
