...
const DIRECT_ENABLED = Boolean(process.env.THINKFLEET_LIVE_MODELS?.trim());
const REQUIRE_PROFILE_KEYS = isTruthyEnvValue(process.env.THINKFLEET_LIVE_REQUIRE_PROFILE_KEYS);
...
describeLive("live models (profile keys)", () => {
  it(
    ...
    async () => {
      ...
      if (!DIRECT_ENABLED) {
        logProgress(
          "[live-models] skipping (set THINKFLEET_LIVE_MODELS=modern|all|<list>; all=modern)",
        );
        return;
      }
      ...
      const rawModels = process.env.THINKFLEET_LIVE_MODELS?.trim();
      const useModern = rawModels === "modern" || rawModels === "all";
      const useExplicit = Boolean(rawModels) && !useModern;
      const filter = useExplicit ? parseModelFilter(rawModels) : null;
      const allowNotFoundSkip = useModern;
      const providers = parseProviderFilter(process.env.THINKFLEET_LIVE_PROVIDERS);
      const perModelTimeoutMs = toInt(process.env.THINKFLEET_LIVE_MODEL_TIMEOUT_MS, 30_000);
