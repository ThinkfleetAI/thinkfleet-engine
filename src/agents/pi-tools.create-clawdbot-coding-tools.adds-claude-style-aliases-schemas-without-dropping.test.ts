...
import { __testing, createThinkfleetCodingTools } from "./pi-tools.js";
...
describe("createThinkfleetCodingTools", () => {
  ...
  it("keeps raw core tool schemas union-free", () => {
    const tools = createThinkfleetTools();
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createThinkfleetCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "thinkfleet-outside.txt");
