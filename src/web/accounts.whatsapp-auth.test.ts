...
describe("hasAnyWhatsAppAuth", () => {
  ...
  beforeEach(() => {
    previousOauthDir = process.env.THINKFLEET_OAUTH_DIR;
    tempOauthDir = fs.mkdtempSync(path.join(os.tmpdir(), "thinkfleet-oauth-"));
    process.env.THINKFLEET_OAUTH_DIR = tempOauthDir;
  });
  ...
  afterEach(() => {
    if (previousOauthDir === undefined) {
      ...
      process.env.THINKFLEET_OAUTH_DIR = previousOauthDir;
    }
    if (tempOauthDir) {
  ...
  it("includes authDir overrides", () => {
    const customDir = fs.mkdtempSync(path.join(os.tmpdir(), "thinkfleet-wa-auth-"));
    try {
