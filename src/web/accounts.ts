...
function listConfiguredAccountIds(cfg: ThinkfleetConfig): string[] {
  const accounts = cfg.channels?.whatsapp?.accounts;
  if (!accounts || typeof accounts !== "object") return [];
  return Object.keys(accounts).filter(Boolean);
}

export function listWhatsAppAuthDirs(cfg: ThinkfleetConfig): string[] {
...
export function hasAnyWhatsAppAuth(cfg: ThinkfleetConfig): boolean {
  return listWhatsAppAuthDirs(cfg).some((authDir) => hasWebCredsSync(authDir));
}

export function listWhatsAppAccountIds(cfg: ThinkfleetConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultWhatsAppAccountId(cfg: ThinkfleetConfig): string {
...
export function resolveWhatsAppAuthDir(params: { cfg: ThinkfleetConfig; accountId: string }): {
  authDir: string;
  isLegacy: boolean;
} {
...
export function listEnabledWhatsAppAccounts(cfg: ThinkfleetConfig): ResolvedWhatsAppAccount[] {
