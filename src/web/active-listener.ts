...
export function requireActiveWebListener(accountId?: string | null): {
  ...
  if (!listener) {
    throw new Error(
      `No active WhatsApp Web listener (account: ${id}). Start the gateway, then link WhatsApp with: ${formatCliCommand(`thinkfleet channels login --channel whatsapp --account ${id}`)}.`,
    );
  }
