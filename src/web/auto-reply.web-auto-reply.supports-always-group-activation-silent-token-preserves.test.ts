...
beforeEach(async () => {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-web-home-"));
...
const makeSessionStore = async (
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-session-"));
...
describe("web auto-reply", () => {
  ...
  it("ignores JID mentions in self-chat mode (group chats)", async () => {
    ...
    // Text-based mentionPatterns still work (user can type "thinkfleet" explicitly).
    await capturedOnMessage?.({
      body: "thinkfleet ping",
      from: "123@g.us",
      conversationId: "123@g.us",
      chatId: "123@g.us",
      chatType: "group",
  ...
  it("emits heartbeat logs with connection metadata", async () => {
    vi.useFakeTimers();
    const logPath = `/tmp/thinkfleet-heartbeat-${crypto.randomUUID()}.log`;
  ...
  it("logs outbound replies to file", async () => {
    const logPath = `/tmp/thinkfleet-log-test-${crypto.randomUUID()}.log`;
    setLoggerOverride({ level: "trace", file: logPath });
