...
async function makeEnv() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-gateway-lock-"));
  const configPath = path.join(dir, "thinkfleet.json");
  await fs.writeFile(configPath, "{}", "utf8");
  await fs.mkdir(resolveGatewayLockDir(), { recursive: true });
  return {
    env: {
      ...process.env,
      THINKFLEET_STATE_DIR: dir,
      THINKFLEET_CONFIG_PATH: configPath,
    },
    cleanup: async () => {
