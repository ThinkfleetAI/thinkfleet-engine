...
export async function resolveGlobalPackageRoot(
  ...
  if (!root) return null;
  return path.join(root, "thinkfleet");
}
...
export async function detectGlobalInstallManagerByPresence(
  ...
  for (const manager of ["npm", "pnpm"] as const) {
    const root = await resolveGlobalRoot(manager, runCommand, timeoutMs);
    if (!root) continue;
    if (await pathExists(path.join(root, "thinkfleet"))) return manager;
  }

  const bunRoot = resolveBunGlobalRoot();
  if (await pathExists(path.join(bunRoot, "thinkfleet"))) return "bun";
  return null;
}
