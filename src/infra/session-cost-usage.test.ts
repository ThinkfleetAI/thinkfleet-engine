...
describe("session cost usage", () => {
  it("aggregates daily totals with log cost and pricing fallback", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-cost-"));
    ...
    const originalState = process.env.THINKFLEET_STATE_DIR;
    process.env.THINKFLEET_STATE_DIR = root;
    try {
      ...
      if (originalState === undefined) delete process.env.THINKFLEET_STATE_DIR;
      else process.env.THINKFLEET_STATE_DIR = originalState;
    }
  });

  it("summarizes a single session file", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-cost-session-"));
