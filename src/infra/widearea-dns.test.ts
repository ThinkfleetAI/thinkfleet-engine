...
describe("wide-area DNS-SD zone rendering", () => {
  it("renders a thinkfleet.internal zone with gateway PTR/SRV/TXT records", () => {
    const txt = renderWideAreaGatewayZoneText({
      serial: 2025121701,
      gatewayPort: 18789,
      displayName: "Mac Studio (Thinkfleet)",
    ...
    expect(txt).toContain(`_thinkfleet-gw._tcp IN PTR studio-london._thinkfleet-gw._tcp`);
    expect(txt).toContain(`studio-london._thinkfleet-gw._tcp IN SRV 0 0 18789 studio-london`);
    expect(txt).toContain(`displayName=Mac Studio (Thinkfleet)`);
    expect(txt).toContain(`gatewayPort=18789`);
    expect(txt).toContain(`sshPort=22`);
    expect(txt).toContain(`cliPath=/opt/homebrew/bin/thinkfleet`);
  });
