...
function candidateBinDirs(opts: EnsureThinkfleetPathOpts): string[] {
  ...
  // Bundled macOS app: `thinkfleet` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingThinkfleet = path.join(execDir, "thinkfleet");
    if (isExecutable(siblingThinkfleet)) candidates.push(execDir);
  } catch {
    // ignore
  }

  // Project-local installs (best effort): if a `node_modules/.bin/thinkfleet` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "thinkfleet"))) candidates.push(localBinDir);
...
/**
 * Best-effort PATH bootstrap so skills that require the `thinkfleet` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureThinkfleetCliOnPath(opts: EnsureThinkfleetPathOpts = {}) {
  if (isTruthyEnvValue(process.env.THINKFLEET_PATH_BOOTSTRAPPED)) return;
  process.env.THINKFLEET_PATH_BOOTSTRAPPED = "1";
