...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.THINKFLEET_DISABLE_BONJOUR)) return true;
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "Thinkfleet";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(Thinkfleet\)\s*$/i, "").trim() || normalized;
}
