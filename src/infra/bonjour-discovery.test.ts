...
describe("bonjour-discovery", () => {
    ...
    const run = vi.fn(async (argv: string[], options: { timeoutMs: number }) => {
      ...
      if (argv[0] === "dns-sd" && argv[1] === "-B") {
        if (domain === "local.") {
          return {
            stdout: [
              "Add 2 3 local. _thinkfleet-gw._tcp. Peter\\226\\128\\153s Mac Studio Gateway",
              "Add 2 3 local. _thinkfleet-gw._tcp. Laptop Gateway",
              "",
            ].join("\n"),
            stderr: "",
            code: 0,
        ...
        if (domain === WIDE_AREA_DISCOVERY_DOMAIN) {
          return {
            stdout: [
              `Add 2 3 ${WIDE_AREA_DISCOVERY_DOMAIN} _thinkfleet-gw._tcp. Tailnet Gateway`,
              "",
            ].join("\n"),
            stderr: "",
            code: 0,
      ...
      if (argv[0] === "dns-sd" && argv[1] === "-L") {
        ...
        return {
          stdout: [
            `${instance}._thinkfleet-gw._tcp. can be reached at ${host}:18789`,
            txtParts.join(" "),
            "",
          ].join("\n"),
          stderr: "",
  ...
  it("decodes dns-sd octal escapes in TXT displayName", async () => {
    const run = vi.fn(async (argv: string[], options: { timeoutMs: number }) => {
      ...
      if (argv[0] === "dns-sd" && argv[1] === "-B" && domain === "local.") {
        return {
          stdout: ["Add 2 3 local. _thinkfleet-gw._tcp. Studio Gateway", ""].join("\n"),
          stderr: "",
          code: 0,
          signal: null,
          killed: false,
      ...
      if (argv[0] === "dns-sd" && argv[1] === "-L") {
        return {
          stdout: [
            "Studio Gateway._thinkfleet-gw._tcp. can be reached at studio.local:18789",
  ...
  it("falls back to tailnet DNS probing for wide-area when split DNS is not configured", async () => {
    ...
    const serviceBase = `_thinkfleet-gw._tcp.${zone}`;
        ...
        if (server === "100.123.224.76" && qtype === "TXT" && qname === studioService) {
          return {
            stdout: [
              ...
              `"cliPath=/opt/homebrew/bin/thinkfleet"`,
              "",
            ].join(" "),
            stderr: "",
            code: 0,
  ...
  it("normalizes domains and respects domains override", async () => {
    ...
    await discoverGatewayBeacons({
      platform: "darwin",
      timeoutMs: 1,
      domains: ["local", "thinkfleet.internal"],
      run: run as unknown as typeof runCommandWithTimeout,
    });

    expect(calls.filter((c) => c[1] === "-B").map((c) => c[3])).toEqual(
      expect.arrayContaining(["local.", "thinkfleet.internal."]),
    );
