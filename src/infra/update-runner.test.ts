...
describe("runGatewayUpdate", () => {
  ...
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-update-"));
  });
  ...
  it("skips git update when worktree is dirty", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({ name: "thinkfleet", version: "1.0.0" }),
      "utf-8",
    );
  ...
  it("uses stable tag when beta tag is older than release", async () => {
    ...
    await fs.writeFile(
      path.join(tempDir, "package.json"),
      JSON.stringify({ name: "thinkfleet", version: "1.0.0", packageManager: "pnpm@8.0.0" }),
      "utf-8",
    );
    ...
    const { runner, calls } = createRunner({
      ...
      "pnpm thinkfleet doctor --non-interactive": { stdout: "" },
    });
  ...
  it("skips update when no git root", async () => {
    await fs.writeFile(
      path.join(tempDir, "package.json"),
      JSON.stringify({ name: "thinkfleet", packageManager: "pnpm@8.0.0" }),
      "utf-8",
    );
  ...
  it("updates global npm installs when detected", async () => {
    ...
    const pkgRoot = path.join(nodeModules, "thinkfleet");
    ...
    const runCommand = async (argv: string[]) => {
      ...
      if (key === "npm i -g thinkfleet@latest") {
        await fs.writeFile(
          path.join(pkgRoot, "package.json"),
          JSON.stringify({ name: "thinkfleet", version: "2.0.0" }),
          "utf-8",
        );
    ...
    expect(calls.some((call) => call === "npm i -g thinkfleet@latest")).toBe(true);
  });
  ...
  it("updates global npm installs with tag override", async () => {
    ...
    const runCommand = async (argv: string[]) => {
      ...
      if (key === "npm i -g thinkfleet@beta") {
        await fs.writeFile(
          path.join(pkgRoot, "package.json"),
          JSON.stringify({ name: "thinkfleet", version: "2.0.0" }),
          "utf-8",
    ...
    expect(result.before?.version).toBe("1.0.0");
    expect(result.after?.version).toBe("2.0.0");
    expect(calls.some((call) => call === "npm i -g thinkfleet@beta")).toBe(true);
  });
  ...
  it("updates global bun installs when detected", async () => {
    ...
    try {
      ...
      const pkgRoot = path.join(bunGlobalRoot, "thinkfleet");
      await fs.mkdir(pkgRoot, { recursive: true });
      await fs.writeFile(
        path.join(pkgRoot, "package.json"),
        JSON.stringify({ name: "thinkfleet", version: "1.0.0" }),
      ...
      const runCommand = async (argv: string[]) => {
        ...
        if (key === "bun add -g thinkfleet@latest") {
          await fs.writeFile(
            path.join(pkgRoot, "package.json"),
            JSON.stringify({ name: "thinkfleet", version: "2.0.0" }),
            "utf-8",
      ...
      expect(result.before?.version).toBe("1.0.0");
      expect(result.after?.version).toBe("2.0.0");
      expect(calls.some((call) => call === "bun add -g thinkfleet@latest")).toBe(true);
    } finally {
  ...
  it("rejects git roots that are not a thinkfleet checkout", async () => {
    await fs.mkdir(path.join(tempDir, ".git"));
    ...
    expect(result.status).toBe("error");
    expect(result.reason).toBe("not-thinkfleet-root");
