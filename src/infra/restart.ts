...
export function triggerThinkfleetRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.THINKFLEET_SYSTEMD_UNIT,
        process.env.THINKFLEET_PROFILE,
      );
  ...
  const label =
    process.env.THINKFLEET_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.THINKFLEET_PROFILE);
