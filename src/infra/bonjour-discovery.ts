...
function parseDnsSdBrowse(stdout: string): string[] {
  ...
  for (const raw of stdout.split("\n")) {
    const line = raw.trim();
    if (!line || !line.includes("_thinkfleet-gw._tcp")) continue;
    if (!line.includes("Add")) continue;
    const match = line.match(/_thinkfleet-gw\._tcp\.?\s+(.+)$/);
    if (match?.[1]) {
...
async function discoverViaDnsSd(
  ...
  const browse = await run(["dns-sd", "-B", "_thinkfleet-gw._tcp", domain], {
    timeoutMs,
  });
  const instances = parseDnsSdBrowse(browse.stdout);
  const results: GatewayBonjourBeacon[] = [];
  for (const instance of instances) {
    const resolved = await run(["dns-sd", "-L", instance, "_thinkfleet-gw._tcp", domain], {
      timeoutMs,
    });
...
async function discoverWideAreaViaTailnetDns(
  ...
  const probeName = `_thinkfleet-gw._tcp.${domain.replace(/\.$/, "")}`;
  ...
  for (const ptr of ptrs) {
    ...
    if (!ptrName) continue;
    const instanceName = ptrName.replace(/\.?_thinkfleet-gw\._tcp\..*$/, "");
...
function parseAvahiBrowse(stdout: string): GatewayBonjourBeacon[] {
  ...
  for (const raw of stdout.split("\n")) {
    ...
    if (!line) continue;
    if (line.startsWith("=") && line.includes("_thinkfleet-gw._tcp")) {
      if (current) results.push(current);
      const marker = " _thinkfleet-gw._tcp";
...
async function discoverViaAvahi(
  ...
  const args = ["avahi-browse", "-rt", "_thinkfleet-gw._tcp"];
