...
export async function autoMigrateLegacyStateDir(params: {
  ...
  if (env.THINKFLEET_STATE_DIR?.trim() || env.THINKFLEET_STATE_DIR?.trim()) {
    return { migrated: false, skipped: true, changes: [], warnings: [] };
  }

  const homedir = params.homedir ?? os.homedir;
  const legacyDir = resolveLegacyStateDir(env, homedir);
  const targetDir = resolveNewStateDir(env, homedir);
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        warnings.push(
          `Rollback failed; set THINKFLEET_STATE_DIR=${targetDir} to avoid split state: ${String(rollbackErr)}`,
        );
...
export async function autoMigrateLegacyState(params: {
  ...
  if (env.THINKFLEET_AGENT_DIR?.trim() || env.PI_CODING_AGENT_DIR?.trim()) {
    return {
