...
export const WIDE_AREA_DISCOVERY_DOMAIN = "thinkfleet.internal.";
export const WIDE_AREA_ZONE_FILENAME = "thinkfleet.internal.db";
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*thinkfleet-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "thinkfleet";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "thinkfleet");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "thinkfleet-gw");
  ...
  records.push(`_thinkfleet-gw._tcp IN PTR ${instanceLabel}._thinkfleet-gw._tcp`);
  records.push(`${instanceLabel}._thinkfleet-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._thinkfleet-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; thinkfleet-content-hash: ${contentHash}\n${contentBody}`;
}
