...
function resolveActiveHoursTimezone(cfg: ThinkfleetConfig, raw?: string): string {
...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: ThinkfleetConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: ThinkfleetConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: ThinkfleetConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: ThinkfleetConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: ThinkfleetConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: ThinkfleetConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: ThinkfleetConfig) => {
    if (state.stopped) return;
    const now = Date.now();
