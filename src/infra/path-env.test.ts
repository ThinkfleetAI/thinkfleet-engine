...
import { ensureThinkfleetCliOnPath } from "./path-env.js";

describe("ensureThinkfleetCliOnPath", () => {
  it("prepends the bundled app bin dir when a sibling thinkfleet exists", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-path-"));
    try {
      const appBinDir = path.join(tmp, "AppBin");
      await fs.mkdir(appBinDir, { recursive: true });
      const cliPath = path.join(appBinDir, "thinkfleet");
      await fs.writeFile(cliPath, "#!/bin/sh\necho ok\n", "utf-8");
      await fs.chmod(cliPath, 0o755);

      const originalPath = process.env.PATH;
      const originalFlag = process.env.THINKFLEET_PATH_BOOTSTRAPPED;
      process.env.PATH = "/usr/bin";
      delete process.env.THINKFLEET_PATH_BOOTSTRAPPED;
      try {
        ensureThinkfleetCliOnPath({
          execPath: cliPath,
          cwd: tmp,
          homeDir: tmp,
          platform: "darwin",
        ...
        if (originalFlag === undefined) delete process.env.THINKFLEET_PATH_BOOTSTRAPPED;
        else process.env.THINKFLEET_PATH_BOOTSTRAPPED = originalFlag;
      }
    } finally {
  ...
  it("prepends mise shims when available", async () => {
    ...
    try {
      const appBinDir = path.join(tmp, "AppBin");
      await fs.mkdir(appBinDir, { recursive: true });
      const appCli = path.join(appBinDir, "thinkfleet");
      await fs.writeFile(appCli, "#!/bin/sh\necho ok\n", "utf-8");
      await fs.chmod(appCli, 0o755);

      const localBinDir = path.join(tmp, "node_modules", ".bin");
      await fs.mkdir(localBinDir, { recursive: true });
      const localCli = path.join(localBinDir, "thinkfleet");
