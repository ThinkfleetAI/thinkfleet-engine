  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(
        path.join(os.tmpdir(), "thinkfleet-update-preflight-"),
      );
    ...
    const doctorStep = await runStep(
      step(
        "thinkfleet doctor",
        managerScriptArgs(manager, "thinkfleet", ["doctor", "--non-interactive"]),
        gitRoot,
        { THINKFLEET_UPDATE_IN_PROGRESS: "1" },
      ),
    );
