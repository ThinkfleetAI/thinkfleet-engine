...
export function isUnhandledRejectionHandled(reason: unknown): boolean {
  for (const handler of handlers) {
    try {
      ...
      console.error(
        "[thinkfleet] Unhandled rejection handler failed:",
...
export function installUnhandledRejectionHandler(): void {
  process.on("unhandledRejection", (reason, _promise) => {
    ...
    if (isAbortError(reason)) {
      console.warn("[thinkfleet] Suppressed AbortError:", formatUncaughtError(reason));
      return;
    }

    if (isFatalError(reason)) {
      console.error("[thinkfleet] FATAL unhandled rejection:", formatUncaughtError(reason));
      process.exit(1);
      return;
    }
    ...
    if (isConfigError(reason)) {
      console.error(
        "[thinkfleet] CONFIGURATION ERROR - requires fix:",
    ...
    if (isTransientNetworkError(reason)) {
      console.warn(
        "[thinkfleet] Non-fatal unhandled rejection (continuing):",
        formatUncaughtError(reason),
      );
      return;
    }

    console.error("[thinkfleet] Unhandled promise rejection:", formatUncaughtError(reason));
    process.exit(1);
  });
}
