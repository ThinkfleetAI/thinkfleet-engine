...
import { resolveThinkfleetPackageRoot } from "./thinkfleet-root.js";
...
export async function runGatewayUpdateCheck(params: {
  ...
  if (cmp != null && cmp < 0) {
    ...
    if (shouldNotify) {
      params.log.info(
        `update available (${tag}): v${resolved.version} (current v${VERSION}). Run: ${formatCliCommand("thinkfleet update")}`,
      );
