...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: ThinkfleetConfig | null = null;

  constructor(private readonly ttlMs: number) {}

  get(key: string, cfg: ThinkfleetConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: ThinkfleetConfig): void {
    this.resetIfConfigChanged(cfg);
  ...
  clear(cfg?: ThinkfleetConfig): void {
    this.cache.clear();
    if (cfg) this.lastConfigRef = cfg;
  }

  private resetIfConfigChanged(cfg: ThinkfleetConfig): void {
