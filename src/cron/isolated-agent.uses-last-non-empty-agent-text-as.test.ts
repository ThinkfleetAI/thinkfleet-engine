...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "thinkfleet-cron-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".thinkfleet", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<ThinkfleetConfig> = {},
): ThinkfleetConfig {
  const base: ThinkfleetConfig = {
    agents: {
      defaults: {
        model: "anthropic/claude-opus-4-5",
        workspace: path.join(home, "thinkfleet"),
      },
    },
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".thinkfleet", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
