...
declare const __THINKFLEET_VERSION__: string | undefined;
...
async function main() {
  ...
  const { ensureThinkfleetCliOnPath } = await import("../infra/path-env.js");
  ...
  process.on("uncaughtException", (error) => {
    console.error("[thinkfleet] Uncaught exception:", formatUncaughtError(error));
    process.exit(1);
  });
