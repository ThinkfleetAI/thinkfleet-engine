...
declare const __THINKFLEET_VERSION__: string;

const BUNDLED_VERSION =
  (typeof __THINKFLEET_VERSION__ === "string" && __THINKFLEET_VERSION__) ||
  process.env.THINKFLEET_BUNDLED_VERSION ||
  "0.0.0";
...
async function main() {
  if (hasFlag(args, "--version") || hasFlag(args, "-v")) {
    // Match `thinkfleet --version` behavior for Swift env/version checks.
  ...
  const portRaw =
    argValue(args, "--port") ??
    process.env.THINKFLEET_GATEWAY_PORT ??
  ...
  const bindRaw =
    argValue(args, "--bind") ??
    process.env.THINKFLEET_GATEWAY_BIND ??
    cfg.gateway?.bind ??
    "loopback";
  const bind =
    bindRaw === "loopback" ||
...
void main().catch((err) => {
  console.error(
    "[thinkfleet] Gateway daemon failed:",
