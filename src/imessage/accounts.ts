...
export function listIMessageAccountIds(cfg: ThinkfleetConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultIMessageAccountId(cfg: ThinkfleetConfig): string {
...
export function listEnabledIMessageAccounts(cfg: ThinkfleetConfig): ResolvedIMessageAccount[] {
