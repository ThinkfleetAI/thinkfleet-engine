...
describe("logger helpers", () => {
  ...
  it("uses daily rolling default log file and prunes old ones", () => {
    ...
    const todayPath = path.join(DEFAULT_LOG_DIR, `thinkfleet-${today}.log`);

    // create an old file to be pruned
    const oldPath = path.join(DEFAULT_LOG_DIR, "thinkfleet-2000-01-01.log");
...
function pathForTest() {
  const file = path.join(os.tmpdir(), `thinkfleet-log-${crypto.randomUUID()}.log`);
