...
describe("canvas host", () => {
  it("injects live reload script", () => {
    ...
    expect(out).toContain("thinkfleetCanvasA2UIAction");
    expect(out).toContain("thinkfleetSendUserAction");
  });

  it("creates a default index.html when missing", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-canvas-"));
    ...
    try {
      ...
      expect(html).toContain("thinkfleetSendUserAction");
