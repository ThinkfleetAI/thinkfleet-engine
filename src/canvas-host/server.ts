...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      (window.webkit.messageHandlers.thinkfleetCanvasA2UIAction ||
        window.webkit.messageHandlers.thinkfleetCanvasA2UIAction)
    );
  const hasAndroid = () =>
    !!(
      (window.thinkfleetCanvasA2UIAction &&
        typeof window.thinkfleetCanvasA2UIAction.postMessage === "function") ||
      (window.thinkfleetCanvasA2UIAction &&
        typeof window.thinkfleetCanvasA2UIAction.postMessage === "function")
    );
  const legacySend = typeof window.thinkfleetSendUserAction === "function" ? window.thinkfleetSendUserAction : undefined;
  if (!window.thinkfleetSendUserAction && legacySend) {
    window.thinkfleetSendUserAction = legacySend;
  }
  if (!window.thinkfleetSendUserAction && typeof window.thinkfleetSendUserAction === "function") {
    window.thinkfleetSendUserAction = window.thinkfleetSendUserAction;
  }
  const hasHelper = () =>
    typeof window.thinkfleetSendUserAction === "function" ||
    typeof window.thinkfleetSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("thinkfleet:a2ui-action-status", (ev) => {
  ...
  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android Thinkfleet node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.thinkfleetSendUserAction === "function"
        ? window.thinkfleetSendUserAction
        : window.thinkfleetSendUserAction;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.THINKFLEET_SKIP_CANVAS_HOST)) return true;
...
export async function createCanvasHostHandler(
  ...
  const rootDir = resolveUserPath(opts.rootDir ?? path.join(os.homedir(), "thinkfleet", "canvas"));
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>Thinkfleet Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
