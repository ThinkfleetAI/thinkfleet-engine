...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".managed-profile-decorated");
}
...
export function decorateManagedProfile(
  ...
  const desiredName = opts?.name ?? DEFAULT_MANAGED_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_MANAGED_BROWSER_COLOR).toUpperCase();
