...
describe("browser chrome profile decoration", () => {
  ...
  it("writes expected name + signed ARGB seed to Chrome prefs", async () => {
    const userDataDir = await fsp.mkdtemp(path.join(os.tmpdir(), "thinkfleet-chrome-test-"));
    try {
      decorateManagedProfile(userDataDir, { color: DEFAULT_MANAGED_BROWSER_COLOR });
      ...
      expect(def.name).toBe(DEFAULT_MANAGED_BROWSER_PROFILE_NAME);
      expect(def.shortcut_name).toBe(DEFAULT_MANAGED_BROWSER_PROFILE_NAME);
      ...
      const marker = await fsp.readFile(
        path.join(userDataDir, ".managed-profile-decorated"),
        "utf-8",
      );
  ...
  it("best-effort writes name when color is invalid", async () => {
    const userDataDir = await fsp.mkdtemp(path.join(os.tmpdir(), "thinkfleet-chrome-test-"));
    try {
      decorateManagedProfile(userDataDir, { color: "lobster-orange" });
      const localState = await readJson(path.join(userDataDir, "Local State"));
      const profile = localState.profile as Record<string, unknown>;
      const infoCache = profile.info_cache as Record<string, unknown>;
      const def = infoCache.Default as Record<string, unknown>;
  ...
  it("is idempotent when rerun on an existing profile", async () => {
    ...
    try {
      ...
      expect(profile.name).toBe(DEFAULT_MANAGED_BROWSER_PROFILE_NAME);
    } finally {
      await fsp.rm(userDataDir, { recursive: true, force: true });
    }
  });
...
describe("browser chrome helpers", () => {
  ...
  it("stopManagedChrome no-ops when process is already killed", async () => {
    const proc = { killed: true, exitCode: null, kill: vi.fn() };
    await stopManagedChrome(
      {
        proc,
        cdpPort: 12345,
      } as unknown as Parameters<typeof stopManagedChrome>[0],
      10,
    );
    expect(proc.kill).not.toHaveBeenCalled();
  });

  it("stopManagedChrome sends SIGTERM and returns once CDP is down", async () => {
