...
describe("browser config", () => {
  ...
  it("derives default ports from THINKFLEET_GATEWAY_PORT when unset", () => {
    const prev = process.env.THINKFLEET_GATEWAY_PORT;
    process.env.THINKFLEET_GATEWAY_PORT = "19001";
    try {
      const resolved = resolveBrowserConfig(undefined);
      ...
      const thinkfleet = resolveProfile(resolved, "thinkfleet");
      expect(thinkfleet?.cdpPort).toBe(19012);
      expect(thinkfleet?.cdpUrl).toBe("http://127.0.0.1:19012");
    } finally {
      if (prev === undefined) {
        delete process.env.THINKFLEET_GATEWAY_PORT;
      } else {
        process.env.THINKFLEET_GATEWAY_PORT = prev;
      }
    }
  });
  ...
  it("derives default ports from gateway.port when env is unset", () => {
    ...
    try {
      ...
      const thinkfleet = resolveProfile(resolved, "thinkfleet");
      expect(thinkfleet?.cdpPort).toBe(19022);
      expect(thinkfleet?.cdpUrl).toBe("http://127.0.0.1:19022");
    } finally {
      if (prev === undefined) {
        delete process.env.THINKFLEET_GATEWAY_PORT;
      } else {
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "thinkfleet");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    const resolved = resolveBrowserConfig({
      profiles: {
        thinkfleet: { cdpPort: 18792, color: "#FF4500" },
      },
    });
    expect(resolveProfile(resolved, "chrome")).toBe(null);
    expect(resolved.defaultProfile).toBe("thinkfleet");
  });
});
