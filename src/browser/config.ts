import type { BrowserConfig, BrowserProfileConfig, ThinkfleetConfig } from "../config/config.js";
import {
...
import {
  DEFAULT_MANAGED_BROWSER_COLOR,
  DEFAULT_MANAGED_BROWSER_ENABLED,
  DEFAULT_BROWSER_EVALUATE_ENABLED,
  DEFAULT_BROWSER_DEFAULT_PROFILE_NAME,
  DEFAULT_MANAGED_BROWSER_PROFILE_NAME,
} from "./constants.js";
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) return DEFAULT_MANAGED_BROWSER_COLOR;
  const normalized = value.startsWith("#") ? value : `#${value}`;
  if (!/^#[0-9a-fA-F]{6}$/.test(normalized)) return DEFAULT_MANAGED_BROWSER_COLOR;
...
/**
 * Ensure the default "managed" (legacy name) profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_MANAGED_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_MANAGED_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is a Thinkfleet browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for managed CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: ThinkfleetConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_MANAGED_BROWSER_ENABLED;
