...
vi.mock("./chrome.js", () => ({
  resolveManagedUserDataDir: vi.fn(() => "/tmp/thinkfleet-test/thinkfleet/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveManagedUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "thinkfleet-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveManagedUserDataDir).mockReturnValue(userDataDir);
