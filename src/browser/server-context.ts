...
function createProfileContext(
  ...
  const ensureBrowserAvailable = async (): Promise<void> => {
    ...
    if (isExtension) {
      ...
      throw new Error(
        `Chrome extension relay is running, but no tab is connected. Click the Thinkfleet Chrome extension icon on a tab to attach it (profile "${profile.name}").`,
      );
    }
    ...
    if (!httpReachable) {
      ...
      const launched = await launchManagedChrome(current.resolved, profile);
    ...
    if (!profileState.running) {
      throw new Error(
        `Port ${profile.cdpPort} is in use for profile "${profile.name}" but not by thinkfleet. ` +
    ...
    await stopManagedChrome(profileState.running);
    setProfileRunning(null);

    const relaunched = await launchManagedChrome(current.resolved, profile);
      ...
      if (profile.driver === "extension") {
        throw new Error(
          `tab not found (no attached Chrome tabs for profile "${profile.name}"). ` +
            "Click the Thinkfleet Browser Relay toolbar icon on the tab you want to control (badge ON).",
        );
      }
  ...
  const resetProfile = async () => {
    ...
    const userDataDir = resolveManagedUserDataDir(profile.name);
    const profileState = getProfileState();
