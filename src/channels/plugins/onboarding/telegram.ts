...
function setTelegramDmPolicy(cfg: ThinkfleetConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
async function noteTelegramUserIdHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      `1) DM your bot, then read from.id in \`${formatCliCommand("thinkfleet logs --follow")}\` (safest)`,
