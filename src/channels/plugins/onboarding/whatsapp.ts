...
function setWhatsAppDmPolicy(cfg: ThinkfleetConfig, dmPolicy: DmPolicy): ThinkfleetConfig {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: ThinkfleetConfig, allowFrom?: string[]): ThinkfleetConfig {
  return mergeWhatsAppConfig(cfg, { allowFrom }, { unsetOnUndefined: ["allowFrom"] });
}

function setWhatsAppSelfChatMode(cfg: ThinkfleetConfig, selfChatMode: boolean): ThinkfleetConfig {
...
async function detectWhatsAppLinked(cfg: ThinkfleetConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppAllowFrom(
  ...
  if (options?.forceAllowlist) {
    await prompter.note(
      "We need the sender/owner number so Thinkfleet can allowlist you.",
      "WhatsApp number",
    );
  ...
  const phoneMode = (await prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for Thinkfleet" },
    ],
...
export const whatsappOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("thinkfleet channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
