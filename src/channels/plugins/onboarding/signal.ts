...
function setSignalDmPolicy(cfg: ThinkfleetConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
export const signalOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({
    ...
    await prompter.note(
      [
        'Link device with: signal-cli link -n "Thinkfleet"',
        "Scan QR in Signal → Linked Devices",
        `Then run: ${formatCliCommand("thinkfleet gateway call channels.status --params '{\"probe\":true}'")}`,
        `Docs: ${formatDocsLink("/signal", "signal")}`,
      ].join("\n"),
      "Signal next steps",
    );
