...
import { discoverThinkfleetPlugins } from "../../plugins/discovery.js";
import type { PluginOrigin } from "../../plugins/types.js";
import type { ThinkfleetPackageManifest } from "../../plugins/manifest.js";
...
type ExternalCatalogEntry = {
  ...
  version?: string;
  description?: string;
  thinkfleet?: ThinkfleetPackageManifest;
  [LEGACY_MANIFEST_KEY]?: ThinkfleetPackageManifest;
};
...
const ENV_CATALOG_PATHS = ["THINKFLEET_PLUGIN_CATALOG_PATHS", "THINKFLEET_MPM_CATALOG_PATHS"];
...
function toChannelMeta(params: {
  channel: NonNullable<ThinkfleetPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
...
function resolveInstallInfo(params: {
  manifest: ThinkfleetPackageManifest;
  packageName?: string;
  packageDir?: string;
  workspaceDir?: string;
...
function buildCatalogEntry(candidate: {
  ...
  packageDir?: string;
  workspaceDir?: string;
  packageThinkfleet?: ThinkfleetPackageManifest;
}): ChannelPluginCatalogEntry | null {
  const manifest = candidate.packageThinkfleet;
...
function buildExternalCatalogEntry(entry: ExternalCatalogEntry): ChannelPluginCatalogEntry | null {
  const manifest = entry.thinkfleet ?? entry[LEGACY_MANIFEST_KEY];
  return buildCatalogEntry({
    packageName: entry.name,
    packageThinkfleet: manifest,
  });
}
...
export function listChannelPluginCatalogEntries(
  ...
  const discovery = discoverThinkfleetPlugins({ workspaceDir: options.workspaceDir });
