  ...
  for (const entry of accounts) {
    ...
    if (!linked) {
      issues.push({
        ...
        fix: `Run: ${formatCliCommand("thinkfleet channels login")} (scan QR on the gateway host).`,
      });
      continue;
    }
    ...
    if (running && !connected) {
      issues.push({
        ...
        fix: `Run: ${formatCliCommand("thinkfleet doctor")} (or restart the gateway). If it persists, relink via channels login and check logs.`,
      });
    }
  }
  return issues;
