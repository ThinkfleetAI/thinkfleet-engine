    ...
    if (addressInUse) {
      log.warn(
        "gog serve failed to bind (address already in use); stopping restarts. " +
          "Another watcher is likely running. Set THINKFLEET_SKIP_GMAIL_WATCHER=1 or stop the other process.",
      );
      watcherProcess = null;
      return;
    }
 ...
 */
export async function startGmailWatcher(cfg: ThinkfleetConfig): Promise<GmailWatcherStartResult> {
