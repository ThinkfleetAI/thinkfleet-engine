...
describe("loader", () => {
  ...
  beforeEach(async () => {
    ...
    tmpDir = path.join(os.tmpdir(), `thinkfleet-test-${Date.now()}`);
    await fs.mkdir(tmpDir, { recursive: true });

    // Disable bundled hooks during tests by setting env var to non-existent directory
    originalBundledDir = process.env.THINKFLEET_BUNDLED_HOOKS_DIR;
    process.env.THINKFLEET_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
  ...
  afterEach(async () => {
    ...
    if (originalBundledDir === undefined) {
      ...
      process.env.THINKFLEET_BUNDLED_HOOKS_DIR = originalBundledDir;
    }
