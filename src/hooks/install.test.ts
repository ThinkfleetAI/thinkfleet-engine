...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `thinkfleet-hook-install-${randomUUID()}`);
...
describe("installHooksFromArchive", () => {
  it("installs hook packs from zip archives", async () => {
    ...
    zip.file(
      ...
      JSON.stringify({
        name: "@thinkfleet/zip-hooks",
        version: "0.0.1",
        thinkfleet: { hooks: ["./hooks/zip-hook"] },
      }),
    );
    zip.file(
  ...
  it("installs hook packs from tar archives", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@thinkfleet/tar-hooks",
        version: "0.0.1",
        thinkfleet: { hooks: ["./hooks/tar-hook"] },
      }),
      "utf-8",
    );
    fs.writeFileSync(
