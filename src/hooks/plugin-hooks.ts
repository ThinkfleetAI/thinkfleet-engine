...
import type { ThinkfleetPluginApi } from "../plugins/types.js";
...
function resolveHookDir(api: ThinkfleetPluginApi, dir: string): string {
  if (path.isAbsolute(dir)) return dir;
  return path.resolve(path.dirname(api.source), dir);
}

function normalizePluginHookEntry(api: ThinkfleetPluginApi, entry: HookEntry): HookEntry {
  return {
    ...entry,
    hook: {
      ...entry.hook,
