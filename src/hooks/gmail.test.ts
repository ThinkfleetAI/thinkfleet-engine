...
import { type ThinkfleetConfig, DEFAULT_GATEWAY_PORT } from "../config/config.js";
import {
  buildDefaultHookUrl,
  buildTopicPath,
  parseTopicPath,
...
describe("gmail hook config", () => {
  ...
  it("resolves runtime config with defaults", () => {
    ...
    if (result.ok) {
      expect(result.value.account).toBe("thinkfleet@gmail.com");
