...
homepage: https://docs.thinkfleet.dev/hooks#command-logger
metadata:
  {
    "thinkfleetbot":
      {
        "emoji": "📝",
        "events": ["command"],
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with ThinkFleetBot" }],
      },
  }
---
...
2. **Appends to log file** - Writes a JSON line to `~/.thinkfleetbot/logs/commands.log`
...
`~/.thinkfleetbot/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.thinkfleetbot/logs/commands.log ~/.thinkfleetbot/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/thinkfleetbot`:
   ```
   /home/username/.thinkfleetbot/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
