...
This directory contains hooks that ship with ThinkFleetBot. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
metadata:
  { "thinkfleetbot": { "emoji": "🔗", "events": ["command:new"], "requires": { "bins": ["node"] } } }
---
# Hook Title
...
Hooks can be configured in `~/.thinkfleetbot/thinkfleetbot.json`:
...
2. Restart gateway: `pkill -9 -f 'thinkfleetbot.*gateway' && pnpm thinkfleetbot gateway`
3. Enable the hook: `thinkfleetbot hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.thinkfleet.dev/hooks
