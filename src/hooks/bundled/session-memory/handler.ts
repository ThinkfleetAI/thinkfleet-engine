...
const saveSessionToMemory: HookHandler = async (event) => {
  ...
  try {
    ...
    const cfg = context.cfg as ThinkfleetConfig | undefined;
    const agentId = resolveAgentIdFromSessionKey(event.sessionKey);
    const workspaceDir = cfg
      ? resolveAgentWorkspaceDir(cfg, agentId)
      : path.join(os.homedir(), "thinkfleet");
    ...
    if (sessionFile) {
      ...
      if (sessionContent && cfg) {
        ...
        const thinkfleetRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "../..");
        const slugGenPath = path.join(thinkfleetRoot, "llm-slug-generator.js");
