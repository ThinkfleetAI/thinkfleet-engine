...
async function makeTempDir() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-hooks-e2e-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("hooks install (e2e)", () => {
  ...
  beforeEach(async () => {
    ...
    prevStateDir = process.env.THINKFLEET_STATE_DIR;
    prevBundledDir = process.env.THINKFLEET_BUNDLED_HOOKS_DIR;
    process.env.THINKFLEET_STATE_DIR = path.join(baseDir, "state");
    process.env.THINKFLEET_BUNDLED_HOOKS_DIR = path.join(baseDir, "bundled-none");
    vi.resetModules();
  });
  ...
  afterEach(async () => {
    ...
    if (prevBundledDir === undefined) {
      delete process.env.THINKFLEET_BUNDLED_HOOKS_DIR;
    } else {
      process.env.THINKFLEET_BUNDLED_HOOKS_DIR = prevBundledDir;
    }
  ...
  it("installs a hook pack and triggers the handler", async () => {
      ...
      JSON.stringify(
        {
          ...
          version: "0.0.0",
          thinkfleet: { hooks: ["./hooks/hello-hook"] },
        },
        null,
        2,
      ),
    ...
    await fs.writeFile(
      ...
      [
        ...
        'metadata: {"thinkfleet":{"events":["command:new"]}}',
        "---",
        "",
        "# Hello Hook",
        "",
