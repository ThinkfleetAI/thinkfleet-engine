...
`;
    ...
    expect(parsed.thinkfleet.emoji).toBe("💾");
    expect(parsed.thinkfleet.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata as string);
    expect(parsed.thinkfleet.emoji).toBe("📝");
    expect(parsed.thinkfleet.events).toEqual(["command"]);
    expect(parsed.thinkfleet.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.thinkfleet.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"thinkfleet": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"thinkfleet": {"events": ["test"]}}');
  });
...
describe("resolveThinkfleetMetadata", () => {
  it("extracts thinkfleet metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveThinkfleetMetadata(frontmatter);
  ...
  it("returns undefined when thinkfleet key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        thinkfleet: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Thinkfleet" },
            { id: "npm", kind: "npm", package: "@thinkfleet/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@thinkfleet/hook");
  });
...
homepage: https://docs.thinkfleet.ai/hooks#session-memory
metadata:
  {
    "thinkfleet":
      {
        ...
        "events": ["command:new"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with Thinkfleet" }],
      },
  }
---
...
`;
    ...
    const thinkfleet = resolveThinkfleetMetadata(frontmatter);
    expect(thinkfleet).toBeDefined();
    expect(thinkfleet?.emoji).toBe("💾");
    expect(thinkfleet?.events).toEqual(["command:new"]);
    expect(thinkfleet?.requires?.config).toEqual(["workspace.dir"]);
    expect(thinkfleet?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const thinkfleet = resolveThinkfleetMetadata(frontmatter);
    expect(thinkfleet?.emoji).toBe("disk");
    expect(thinkfleet?.events).toEqual(["command:new"]);
  });
});
