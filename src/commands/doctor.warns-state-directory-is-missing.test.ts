...
beforeEach(() => {
  ...
  resolveThinkfleetPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.THINKFLEET_STATE_DIR;
  originalUpdateInProgress = process.env.THINKFLEET_UPDATE_IN_PROGRESS;
  process.env.THINKFLEET_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "thinkfleet-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.THINKFLEET_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.THINKFLEET_UPDATE_IN_PROGRESS;
  } else {
    process.env.THINKFLEET_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
const resolveThinkfleetPackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadThinkfleetPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../config/config.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    CONFIG_PATH: "/tmp/thinkfleet.json",
    createConfigIO,
...
vi.mock("../infra/thinkfleet-root.js", () => ({
  resolveThinkfleetPackageRoot,
}));
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "thinkfleet-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.THINKFLEET_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when THINKFLEET_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/thinkfleet.json",
      exists: true,
      raw: "{}",
    ...
    process.env.THINKFLEET_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
