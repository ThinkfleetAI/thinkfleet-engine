...
describe("onboard (non-interactive): gateway and remote auth", () => {
  const prev = {
    ...
    password: process.env.THINKFLEET_GATEWAY_PASSWORD,
  };
  ...
  const initStateDir = async (prefix: string) => {
    ...
    process.env.THINKFLEET_STATE_DIR = stateDir;
  ...
  beforeAll(async () => {
    ...
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-onboard-"));
  ...
  afterAll(async () => {
    ...
    process.env.THINKFLEET_GATEWAY_PASSWORD = prev.password;
  });
  ...
  it("writes gateway token auth into config and gateway enforces it", async () => {
    ...
    const workspace = path.join(stateDir, "thinkfleet");
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    process.env.THINKFLEET_STATE_DIR = stateDir;
    process.env.THINKFLEET_CONFIG_PATH = path.join(stateDir, "thinkfleet.json");
