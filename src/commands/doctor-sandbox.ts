...
function resolveSandboxDockerImage(cfg: ThinkfleetConfig): string {
  const image = cfg.agents?.defaults?.sandbox?.docker?.image?.trim();
  return image ? image : DEFAULT_SANDBOX_IMAGE;
}

function resolveSandboxBrowserImage(cfg: ThinkfleetConfig): string {
  const image = cfg.agents?.defaults?.sandbox?.browser?.image?.trim();
  return image ? image : DEFAULT_SANDBOX_BROWSER_IMAGE;
}

function updateSandboxDockerImage(cfg: ThinkfleetConfig, image: string): ThinkfleetConfig {
  return {
    ...cfg,
    agents: {
      ...cfg.agents,
...
function updateSandboxBrowserImage(cfg: ThinkfleetConfig, image: string): ThinkfleetConfig {
  return {
    ...cfg,
    agents: {
      ...cfg.agents,
...
export function noteSandboxScopeWarnings(cfg: ThinkfleetConfig) {
