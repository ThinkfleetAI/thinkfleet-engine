...
const SEARCH_TOOL = "https://docs.thinkfleet.ai/mcp.SearchThinkfleet";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.thinkfleet.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('thinkfleet docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.thinkfleet.ai/");
      runtime.log(`Search: ${formatCliCommand('thinkfleet docs "your query"')}`);
    }
    return;
  }
