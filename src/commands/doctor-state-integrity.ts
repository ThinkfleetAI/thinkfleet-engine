...
function findOtherStateDirs(stateDir: string): string[] {
  ...
  for (const root of roots) {
    ...
    for (const entry of entries) {
      ...
      const candidate = path.resolve(root, entry.name, ".thinkfleet");
...
export async function noteStateIntegrity(
  ...
  const defaultStateDir = path.join(homedir(), ".thinkfleet");
...
export function noteWorkspaceBackupTip(workspaceDir: string) {
  ...
  note(
    [
      ...
      "- Keep ~/.thinkfleet out of git; it contains credentials and session history.",
