...
describe("resolveGatewayDevMode", () => {
  it("detects dev mode for src ts entrypoints", () => {
    expect(resolveGatewayDevMode(["node", "/Users/me/thinkfleet/src/cli/index.ts"])).toBe(true);
    expect(resolveGatewayDevMode(["node", "C:\\Users\\me\\thinkfleet\\src\\cli\\index.ts"])).toBe(
      true,
    );
    expect(resolveGatewayDevMode(["node", "/Users/me/thinkfleet/dist/cli/index.js"])).toBe(false);
  });
});
...
describe("buildGatewayInstallPlan", () => {
  it("uses provided nodePath and returns plan", async () => {
    ...
    mocks.buildServiceEnvironment.mockReturnValue({ THINKFLEET_PORT: "3000" });
    ...
    expect(plan.environment).toEqual({ THINKFLEET_PORT: "3000" });
  ...
  it("merges config env vars into the environment", async () => {
    ...
    expect(plan.environment.THINKFLEET_PORT).toBe("3000");
...
describe("gatewayInstallErrorHint", () => {
  it("returns platform-specific hints", () => {
    ...
    expect(gatewayInstallErrorHint("linux")).toMatch(
      /(?:thinkfleet-engine|thinkfleet)( --profile isolated)? gateway install/,
    );
  });
});
