...
const debugHealth = (...args: unknown[]) => {
  if (isTruthyEnvValue(process.env.THINKFLEET_DEBUG_HEALTH)) {
...
export async function healthCommand(
  opts: { json?: boolean; timeoutMs?: number; verbose?: boolean; config?: ThinkfleetConfig },
  runtime: RuntimeEnv,
) {
  ...
  if (opts.json) {
    ...
    const debugEnabled = isTruthyEnvValue(process.env.THINKFLEET_DEBUG_HEALTH);
    if (opts.verbose) {
