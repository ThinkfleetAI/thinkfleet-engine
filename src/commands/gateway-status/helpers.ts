...
import type { ThinkfleetConfig, ConfigFileSnapshot } from "../../config/types.js";
...
export function resolveTargets(cfg: ThinkfleetConfig, explicitUrl?: string): GatewayStatusTarget[] {
...
export function resolveAuthForTarget(
  ...
  const envToken = process.env.THINKFLEET_GATEWAY_TOKEN?.trim() || "";
  const envPassword = process.env.THINKFLEET_GATEWAY_PASSWORD?.trim() || "";
  const cfgToken =
...
export function buildNetworkHints(cfg: ThinkfleetConfig) {
