...
async function detectThinkfleetGitCheckout(root: string): Promise<"git" | "not-git" | "unknown"> {
...
export async function maybeOfferUpdateBeforeDoctor(params: {
  ...
  const updateInProgress = isTruthyEnvValue(process.env.THINKFLEET_UPDATE_IN_PROGRESS);
  ...
  const git = await detectThinkfleetGitCheckout(params.root);
  if (git === "git") {
    const shouldUpdate = await params.confirm({
      message: "Update Thinkfleet from git before running doctor?",
      initialValue: true,
    });
  ...
  if (git === "not-git") {
    note(
      [
        ...
        `Run \`${formatCliCommand("thinkfleet update")}\` to update via your package manager (npm/pnpm), then rerun doctor.`,
      ].join("\n"),
      "Update",
    );
  }
