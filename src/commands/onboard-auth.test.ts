...
const requireAgentDir = () => {
  const agentDir = process.env.THINKFLEET_AGENT_DIR;
  if (!agentDir) throw new Error("THINKFLEET_AGENT_DIR not set");
  return agentDir;
};

describe("writeOAuthCredentials", () => {
  const previousStateDir = process.env.THINKFLEET_STATE_DIR;
  const previousAgentDir = process.env.THINKFLEET_AGENT_DIR;
  ...
  afterEach(async () => {
    ...
    if (previousStateDir === undefined) {
      ...
      process.env.THINKFLEET_STATE_DIR = previousStateDir;
    }
    if (previousAgentDir === undefined) {
      delete process.env.THINKFLEET_AGENT_DIR;
    } else {
      process.env.THINKFLEET_AGENT_DIR = previousAgentDir;
    }
    ...
    delete process.env.THINKFLEET_OAUTH_DIR;
  });

  it("writes auth-profiles.json under THINKFLEET_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-oauth-"));
    process.env.THINKFLEET_STATE_DIR = tempStateDir;
    process.env.THINKFLEET_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.THINKFLEET_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to THINKFLEET_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-minimax-"));
    process.env.THINKFLEET_STATE_DIR = tempStateDir;
    process.env.THINKFLEET_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.THINKFLEET_AGENT_DIR;
