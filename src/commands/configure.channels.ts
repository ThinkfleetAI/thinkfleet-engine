...
export async function removeChannelConfigWizard(
  ...
  while (true) {
    ...
    if (configured.length === 0) {
      note(
        [
          "No channel config found in thinkfleet.json.",
          `Tip: \`${formatCliCommand("thinkfleet channels status")}\` shows what is configured and enabled.`,
        ].join("\n"),
        "Remove channel",
      );
      return next;
    ...
    next = {
      ...
      channels: Object.keys(nextChannels).length
        ? (nextChannels as ThinkfleetConfig["channels"])
        : undefined,
    };
