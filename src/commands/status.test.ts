...
beforeAll(() => {
  previousProfile = process.env.THINKFLEET_PROFILE;
  process.env.THINKFLEET_PROFILE = "isolated";
});
...
afterAll(() => {
  if (previousProfile === undefined) {
    ...
    process.env.THINKFLEET_PROFILE = previousProfile;
  }
});
...
vi.mock("../infra/thinkfleet-root.js", () => ({
  resolveThinkfleetPackageRoot: vi.fn().mockResolvedValue("/tmp/thinkfleet"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/thinkfleet/pnpm-lock.yaml",
      markerPath: "/tmp/thinkfleet/node_modules/.modules.yaml",
    },
...
vi.mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.thinkfleet.gateway.plist",
    }),
  }),
}));
vi.mock("../daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.thinkfleet.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("Thinkfleet status"))).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.THINKFLEET_GATEWAY_TOKEN = "abcd1234";
    try {
