...
export async function maybeMigrateLegacyGatewayService(
  ...
  const migrate = await prompter.confirmSkipInNonInteractive({
    message: "Migrate legacy gateway services to Thinkfleet now?",
    initialValue: true,
  });
  if (!migrate) return;
  ...
  if (loaded) {
    note(`Thinkfleet ${service.label} already ${service.loadedText}.`, "Gateway");
    return;
  }

  const install = await prompter.confirmSkipInNonInteractive({
    message: "Install Thinkfleet gateway service now?",
    initialValue: true,
  });
  if (!install) return;
  ...
  const { programArguments, workingDirectory, environment } = await buildGatewayInstallPlan({
    env: process.env,
    port,
    token: cfg.gateway?.auth?.token ?? process.env.THINKFLEET_GATEWAY_TOKEN,
