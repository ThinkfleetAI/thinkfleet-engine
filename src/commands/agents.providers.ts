...
function resolveDefaultAccountId(cfg: ThinkfleetConfig, provider: ChannelId): string {
  const plugin = getChannelPlugin(provider);
  if (!plugin) return DEFAULT_ACCOUNT_ID;
  return resolveChannelDefaultAccountId({ plugin, cfg });
}

function shouldShowProviderEntry(entry: ProviderAccountStatus, cfg: ThinkfleetConfig): boolean {
...
export function summarizeBindings(cfg: ThinkfleetConfig, bindings: AgentBinding[]): string[] {
