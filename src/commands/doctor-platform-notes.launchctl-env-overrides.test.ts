...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "THINKFLEET_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("THINKFLEET_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv THINKFLEET_GATEWAY_TOKEN");
    expect(message).not.toContain("THINKFLEET_GATEWAY_PASSWORD");
  });
