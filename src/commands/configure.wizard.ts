    ...
    await select({
      ...
      options: [
        ...
        },
        {
          ...
          hint: "Delete channel tokens/settings from thinkfleet.json",
        },
      ],
...
export async function runConfigureWizard(
  ...
  try {
    ...
    intro(opts.command === "update" ? "Thinkfleet update wizard" : "Thinkfleet configure");
    const prompter = createClackPrompter();

    const snapshot = await readConfigFileSnapshot();
    const baseConfig: ThinkfleetConfig = snapshot.valid ? snapshot.config : {};
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid && snapshot.issues.length > 0) {
        note(
          [
            ...
            "",
            "Docs: https://docs.thinkfleet.ai/gateway/configuration",
          ].join("\n"),
          "Config issues",
        );
      }
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("thinkfleet doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
    ...
    const localProbe = await probeGatewayReachable({
      url: localUrl,
      token: baseConfig.gateway?.auth?.token ?? process.env.THINKFLEET_GATEWAY_TOKEN,
      password: baseConfig.gateway?.auth?.password ?? process.env.THINKFLEET_GATEWAY_PASSWORD,
    });
    ...
    if (opts.sections) {
      ...
      if (selected.includes("health")) {
        ...
        const token = nextConfig.gateway?.auth?.token ?? process.env.THINKFLEET_GATEWAY_TOKEN;
        const password =
          nextConfig.gateway?.auth?.password ?? process.env.THINKFLEET_GATEWAY_PASSWORD;
    ...
    const newPassword =
      nextConfig.gateway?.auth?.password ?? process.env.THINKFLEET_GATEWAY_PASSWORD;
    const oldPassword =
      baseConfig.gateway?.auth?.password ?? process.env.THINKFLEET_GATEWAY_PASSWORD;
    const token = nextConfig.gateway?.auth?.token ?? process.env.THINKFLEET_GATEWAY_TOKEN;
