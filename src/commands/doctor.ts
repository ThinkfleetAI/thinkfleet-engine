...
function resolveMode(cfg: ThinkfleetConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("Thinkfleet doctor");

  const root = await resolveThinkfleetPackageRoot({
  ...
  let cfg: ThinkfleetConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("thinkfleet configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("thinkfleet config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("thinkfleet setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("thinkfleet doctor --fix")}" to apply changes.`);
  }
