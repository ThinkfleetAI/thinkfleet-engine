...
export function listAgentEntries(cfg: ThinkfleetConfig): AgentEntry[] {
...
function resolveAgentName(cfg: ThinkfleetConfig, agentId: string) {
...
function resolveAgentModel(cfg: ThinkfleetConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: ThinkfleetConfig): AgentSummary[] {
