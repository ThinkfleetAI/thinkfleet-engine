...
import type { ThinkfleetConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: ThinkfleetConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("thinkfleet security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("thinkfleet configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("thinkfleet config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("thinkfleet doctor --fix")} to generate a token`,
              `  Or set token directly: ${formatCliCommand(
                "thinkfleet config set gateway.auth.mode token",
              )}`,
            ];
      warnings.push(
        `- CRITICAL: Gateway bound to ${bindDescriptor} without authentication.`,
        `  Anyone on your network (or internet if port-forwarded) can fully control your agent.`,
        `  Fix: ${formatCliCommand("thinkfleet config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
