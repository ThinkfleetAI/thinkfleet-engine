...
function resolveConfiguredModelRaw(cfg: ThinkfleetConfig): string {
  const raw = cfg.agents?.defaults?.model as { primary?: string } | string | undefined;
  if (typeof raw === "string") return raw.trim();
  return raw?.primary?.trim() ?? "";
}

function resolveConfiguredModelKeys(cfg: ThinkfleetConfig): string[] {
...
export function applyPrimaryModel(cfg: ThinkfleetConfig, model: string): ThinkfleetConfig {
...
export function applyModelAllowlist(cfg: ThinkfleetConfig, models: string[]): ThinkfleetConfig {
  const defaults = cfg.agents?.defaults;
