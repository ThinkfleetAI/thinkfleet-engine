...
export async function maybeRepairGatewayDaemon(params: {
  ...
  if (process.platform === "darwin" && params.cfg.gateway?.mode !== "remote") {
    ...
    await maybeRepairLaunchAgentBootstrap({
      env: { ...process.env, THINKFLEET_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel() },
      title: "Node",
      runtime: params.runtime,
      prompter: params.prompter,
    });
    ...
    if (params.cfg.gateway?.mode !== "remote") {
      ...
      if (install) {
        ...
        const { programArguments, workingDirectory, environment } = await buildGatewayInstallPlan({
          env: process.env,
          port,
          token: params.cfg.gateway?.auth?.token ?? process.env.THINKFLEET_GATEWAY_TOKEN,
  ...
  if (process.platform === "darwin") {
    const label = resolveGatewayLaunchAgentLabel(process.env.THINKFLEET_PROFILE);
    note(
      `LaunchAgent loaded; stopping requires "${formatCliCommand("thinkfleet gateway stop")}" or launchctl bootout gui/$UID/${label}.`,
      "Gateway",
    );
  }
