...
export async function modelsStatusCommand(
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("thinkfleet models auth setup-token")}\` or \`${formatCliCommand("thinkfleet configure")}\`.`
          : `Run \`${formatCliCommand("thinkfleet configure")}\` or set an API key env var.`;
