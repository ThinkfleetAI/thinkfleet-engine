...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveThinkfleetAgentDir: vi.fn().mockReturnValue("/tmp/thinkfleet-agent"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveThinkfleetAgentDir: mocks.resolveThinkfleetAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(payload.auth.storePath).toBe("/tmp/thinkfleet-agent/auth-profiles.json");
