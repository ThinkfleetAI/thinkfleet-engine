...
import { readConfigFileSnapshot, type ThinkfleetConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: ThinkfleetConfig, model: string): ThinkfleetConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("thinkfleet plugins install")}\`.`,
    );
  }
