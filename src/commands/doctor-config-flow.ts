...
function stripUnknownConfigKeys(config: ThinkfleetConfig): {
  config: ThinkfleetConfig;
  removed: string[];
} {
  const parsed = ThinkfleetSchema.safeParse(config);
  if (parsed.success) {
    return { config, removed: [] };
  }

  const next = structuredClone(config) as ThinkfleetConfig;
...
function noteOpencodeProviderOverrides(cfg: ThinkfleetConfig) {
...
function hasExplicitConfigPath(env: NodeJS.ProcessEnv): boolean {
  return Boolean(env.THINKFLEET_CONFIG_PATH?.trim() || env.THINKFLEET_CONFIG_PATH?.trim());
}
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  let cfg: ThinkfleetConfig = baseCfg;
  let candidate = structuredClone(baseCfg) as ThinkfleetConfig;
  ...
  if (snapshot.legacyIssues.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(
        `Run "${formatCliCommand("thinkfleet doctor --fix")}" to apply legacy migrations.`,
      );
    }
  }
  ...
  if (normalized.changes.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("thinkfleet doctor --fix")}" to apply these changes.`);
    }
  }
  ...
  if (unknown.removed.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push('Run "thinkfleet doctor --fix" to remove these keys.');
    }
  }
