...
export async function noteMacLaunchAgentOverrides() {
  ...
  const markerPath = path.join(resolveHomeDir(), ".thinkfleet", "disable-launchagent");
...
function hasConfigGatewayCreds(cfg: ThinkfleetConfig): boolean {
  const localToken =
...
export async function noteMacLaunchctlGatewayEnvOverrides(
  ...
  const envToken = await getenv("THINKFLEET_GATEWAY_TOKEN");
  const envPassword = await getenv("THINKFLEET_GATEWAY_PASSWORD");
  if (!envToken && !envPassword) return;

  const lines = [
    "- launchctl environment overrides detected (can cause confusing unauthorized errors).",
    envToken ? "- `THINKFLEET_GATEWAY_TOKEN` is set; it overrides config tokens." : undefined,
    envPassword
      ? "- `THINKFLEET_GATEWAY_PASSWORD` is set; it overrides config passwords."
      : undefined,
    "- Clear overrides and restart the app/gateway:",
    envToken ? "  launchctl unsetenv THINKFLEET_GATEWAY_TOKEN" : undefined,
    envPassword ? "  launchctl unsetenv THINKFLEET_GATEWAY_PASSWORD" : undefined,
  ].filter((line): line is string => Boolean(line));
