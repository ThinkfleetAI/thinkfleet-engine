...
export async function statusCommand(
  ...
  runtime.log(theme.heading("Thinkfleet status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("thinkfleet security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("thinkfleet security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.thinkfleet.ai/faq");
  runtime.log("Troubleshooting: https://docs.thinkfleet.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("thinkfleet status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("thinkfleet logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("thinkfleet status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("thinkfleet gateway probe")}`);
  }
}
