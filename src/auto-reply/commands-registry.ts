import type { ThinkfleetConfig } from "../config/types.js";
...
export function isCommandEnabled(cfg: ThinkfleetConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: ThinkfleetConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as ThinkfleetConfig),
...
export function getCommandDetection(_cfg?: ThinkfleetConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: ThinkfleetConfig) {
