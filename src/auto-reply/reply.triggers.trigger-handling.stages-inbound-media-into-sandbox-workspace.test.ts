...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(async (home) => await fn(home), { prefix: "thinkfleet-triggers-" });
}
...
describe("stageSandboxMedia", () => {
  it("stages inbound media into the sandbox workspace", async () => {
    await withTempHome(async (home) => {
      const inboundDir = join(home, ".thinkfleet", "media", "inbound");
      ...
      await stageSandboxMedia({
        ...
        cfg: {
          agents: {
            defaults: {
              ...
              workspace: join(home, "thinkfleet"),
              sandbox: {
                mode: "non-main",
        ...
        workspaceDir: join(home, "thinkfleet"),
      });
