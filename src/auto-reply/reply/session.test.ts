...
describe("initSessionState thread forking", () => {
  it("forks a new session from the parent session file", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-thread-session-"));
  ...
  it("records topic-specific session files when MessageThreadId is present", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-topic-session-"));
    const storePath = path.join(root, "sessions.json");
...
describe("initSessionState RawBody", () => {
  it("triggerBodyNormalized correctly extracts commands when Body contains context but RawBody is clean", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-rawbody-"));
    const storePath = path.join(root, "sessions.json");
    const cfg = { session: { store: storePath } } as ThinkfleetConfig;
  ...
  it("Reset triggers (/new, /reset) work with RawBody", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-rawbody-reset-"));
    const storePath = path.join(root, "sessions.json");
    const cfg = { session: { store: storePath } } as ThinkfleetConfig;
  ...
  it("preserves argument casing while still matching reset triggers case-insensitively", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-rawbody-reset-case-"));
    const storePath = path.join(root, "sessions.json");
  ...
  it("falls back to Body when RawBody is undefined", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-rawbody-fallback-"));
    const storePath = path.join(root, "sessions.json");
    const cfg = { session: { store: storePath } } as ThinkfleetConfig;
...
describe("initSessionState reset policy", () => {
  it("defaults to daily reset at 4am local time", async () => {
    ...
    try {
      const root = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-reset-daily-"));
      const storePath = path.join(root, "sessions.json");
  ...
  it("treats sessions as stale before the daily reset when updated before yesterday's boundary", async () => {
    ...
    try {
      const root = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-reset-daily-edge-"));
      const storePath = path.join(root, "sessions.json");
  ...
  it("expires sessions when idle timeout wins over daily reset", async () => {
    ...
    try {
      const root = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-reset-idle-"));
      const storePath = path.join(root, "sessions.json");
  ...
  it("uses per-type overrides for thread sessions", async () => {
    ...
    vi.setSystemTime(new Date(2026, 0, 18, 5, 0, 0));
    try {
      const root = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-reset-thread-"));
      const storePath = path.join(root, "sessions.json");
  ...
  it("detects thread sessions without thread key suffix", async () => {
    ...
    vi.setSystemTime(new Date(2026, 0, 18, 5, 0, 0));
    try {
      const root = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-reset-thread-nosuffix-"));
      const storePath = path.join(root, "sessions.json");
  ...
  it("defaults to daily resets when only resetByType is configured", async () => {
    ...
    vi.setSystemTime(new Date(2026, 0, 18, 5, 0, 0));
    try {
      const root = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-reset-type-default-"));
      const storePath = path.join(root, "sessions.json");
  ...
  it("keeps legacy idleMinutes behavior without reset config", async () => {
    ...
    vi.setSystemTime(new Date(2026, 0, 18, 5, 0, 0));
    try {
      const root = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-reset-legacy-"));
      const storePath = path.join(root, "sessions.json");
...
describe("initSessionState channel reset overrides", () => {
  it("uses channel-specific reset policy when configured", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-channel-idle-"));
    const storePath = path.join(root, "sessions.json");
