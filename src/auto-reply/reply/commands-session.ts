...
import { scheduleGatewaySigusr1Restart, triggerThinkfleetRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting thinkfleet in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerThinkfleetRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting thinkfleet via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
