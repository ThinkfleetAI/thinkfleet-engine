...
describe("runReplyAgent typing (heartbeat)", () => {
  ...
  it("retries after compaction failure by resetting the session", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(tmpdir(), "thinkfleet-session-compaction-reset-"));
  ...
  it("retries after context overflow payload by resetting the session", async () => {
    const prevStateDir = process.env.THINKFLEET_STATE_DIR;
    const stateDir = await fs.mkdtemp(path.join(tmpdir(), "thinkfleet-session-overflow-reset-"));
    process.env.THINKFLEET_STATE_DIR = stateDir;
    try {
      const sessionId = "session";
      const storePath = path.join(stateDir, "sessions", "sessions.json");
  ...
  it("resets the session after role ordering payloads", async () => {
    const prevStateDir = process.env.THINKFLEET_STATE_DIR;
    const stateDir = await fs.mkdtemp(path.join(tmpdir(), "thinkfleet-session-role-ordering-"));
    process.env.THINKFLEET_STATE_DIR = stateDir;
    try {
      const sessionId = "session";
      const storePath = path.join(stateDir, "sessions", "sessions.json");
