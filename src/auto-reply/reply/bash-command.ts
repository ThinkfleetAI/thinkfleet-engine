...
function resolveForegroundMs(cfg: ThinkfleetConfig): number {
...
function formatElevatedUnavailableMessage(params: {
  ...
  if (params.sessionKey) {
    lines.push(
      `See: ${formatCliCommand(`thinkfleet sandbox explain --session ${params.sessionKey}`)}`,
    );
  }
  return lines.join("\n");
}
...
export async function handleBashChatCommand(params: {
  ...
  if (params.cfg.commands?.bash !== true) {
    return {
      text: "⚠️ bash is disabled. Set commands.bash=true to enable. Docs: https://docs.thinkfleet.ai/tools/slash-commands#config",
    };
  }
