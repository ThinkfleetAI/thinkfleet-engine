...
type AgentConfig = Partial<NonNullable<NonNullable<ThinkfleetConfig["agents"]>["defaults"]>>;
...
const readUsageFromSessionLog = (
  ...
  | undefined => {
  // Transcripts are stored at the session file path (fallback: ~/.thinkfleet/sessions/<SessionId>.jsonl)
...
export function buildStatusMessage(args: StatusArgs): string {
  ...
  const versionLine = `🦞 Thinkfleet ${VERSION}${commit ? ` (${commit})` : ""}`;
...
export function buildHelpMessage(cfg?: ThinkfleetConfig): string {
  const lines = ["ℹ️ Help", ""];
