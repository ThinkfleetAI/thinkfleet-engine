...
describe("media store", () => {
  ...
  const snapshotEnv = () => {
    for (const key of ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "THINKFLEET_STATE_DIR"]) {
  ...
  beforeAll(async () => {
    snapshotEnv();
    home = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-test-home-"));
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.THINKFLEET_STATE_DIR = path.join(home, ".thinkfleet");
    ...
    await fs.mkdir(path.join(home, ".thinkfleet"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.thinkfleet${path.sep}media`);
