...
function prefersSips(): boolean {
  return (
    process.env.THINKFLEET_IMAGE_BACKEND === "sips" ||
    (process.env.THINKFLEET_IMAGE_BACKEND !== "sharp" && isBun() && process.platform === "darwin")
  );
}
...
async function withTempDir<T>(fn: (dir: string) => Promise<T>): Promise<T> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-img-"));
  try {
    return await fn(dir);
  } finally {
