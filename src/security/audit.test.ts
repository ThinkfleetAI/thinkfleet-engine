...
describe("security audit", () => {
  ...
  it("treats Windows ACL-only perms as secure", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-security-audit-win-"));
    const stateDir = path.join(tmp, "state");
    await fs.mkdir(stateDir, { recursive: true });
    const configPath = path.join(stateDir, "thinkfleet.json");
  ...
  it("flags Windows ACLs when Users can read the state dir", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-security-audit-win-open-"));
    const stateDir = path.join(tmp, "state");
    await fs.mkdir(stateDir, { recursive: true });
    const configPath = path.join(stateDir, "thinkfleet.json");
    await fs.writeFile(configPath, "{}\n", "utf-8");
  ...
  it("warns when multiple DM senders share the main session", async () => {
    const cfg: ThinkfleetConfig = { session: { dmScope: "main" } };
  ...
  it("flags Discord native commands without a guild user allowlist", async () => {
    const prevStateDir = process.env.THINKFLEET_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-security-audit-discord-"));
    process.env.THINKFLEET_STATE_DIR = tmp;
    ...
    try {
      ...
      if (prevStateDir == null) delete process.env.THINKFLEET_STATE_DIR;
      else process.env.THINKFLEET_STATE_DIR = prevStateDir;
    }
  });
  ...
  it("does not flag Discord slash commands when dm.allowFrom includes a Discord snowflake id", async () => {
    ...
    const tmp = await fs.mkdtemp(
      path.join(os.tmpdir(), "thinkfleet-security-audit-discord-allowfrom-snowflake-"),
    );
    process.env.THINKFLEET_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
  ...
  it("flags Discord slash commands when access-group enforcement is disabled and no users allowlist exists", async () => {
    const prevStateDir = process.env.THINKFLEET_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-security-audit-discord-open-"));
    process.env.THINKFLEET_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: ThinkfleetConfig = {
  ...
  it("flags Slack slash commands without a channel users allowlist", async () => {
    const prevStateDir = process.env.THINKFLEET_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-security-audit-slack-"));
    process.env.THINKFLEET_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: ThinkfleetConfig = {
  ...
  it("flags Slack slash commands when access-group enforcement is disabled", async () => {
    const prevStateDir = process.env.THINKFLEET_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-security-audit-slack-open-"));
    process.env.THINKFLEET_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: ThinkfleetConfig = {
  ...
  it("flags Telegram group commands without a sender allowlist", async () => {
    const prevStateDir = process.env.THINKFLEET_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-security-audit-telegram-"));
    process.env.THINKFLEET_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: ThinkfleetConfig = {
  ...
  it("adds a warning when deep probe fails", async () => {
    const cfg: ThinkfleetConfig = { gateway: { mode: "local" } };
  ...
  it("warns when hooks token reuses the gateway env token", async () => {
    const prevToken = process.env.THINKFLEET_GATEWAY_TOKEN;
    process.env.THINKFLEET_GATEWAY_TOKEN = "shared-gateway-token-1234567890";
    const cfg: ThinkfleetConfig = {
    ...
    try {
      ...
      if (prevToken === undefined) delete process.env.THINKFLEET_GATEWAY_TOKEN;
      else process.env.THINKFLEET_GATEWAY_TOKEN = prevToken;
    }
  });
  ...
  it("warns when state/config look like a synced folder", async () => {
    ...
    const res = await runSecurityAudit({
      ...
      includeFilesystem: false,
      includeChannelSecurity: false,
      stateDir: "/Users/test/Dropbox/.thinkfleet",
      configPath: "/Users/test/Dropbox/.thinkfleet/thinkfleet.json",
    });
  ...
  it("flags group/world-readable config include files", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-security-audit-"));
    const stateDir = path.join(tmp, "state");
    ...
    try {
      const cfg: ThinkfleetConfig = { logging: { redactSensitive: "off" } };
      const user = "DESKTOP-TEST\\Tester";
  ...
  it("flags extensions without plugins.allow", async () => {
    ...
    try {
      ...
      const res = await runSecurityAudit({
        ...
        includeChannelSecurity: false,
        stateDir,
        configPath: path.join(stateDir, "thinkfleet.json"),
      });
  ...
  describe("maybeProbeGateway auth selection", () => {
    const originalEnvToken = process.env.THINKFLEET_GATEWAY_TOKEN;
    const originalEnvPassword = process.env.THINKFLEET_GATEWAY_PASSWORD;

    beforeEach(() => {
      delete process.env.THINKFLEET_GATEWAY_TOKEN;
      delete process.env.THINKFLEET_GATEWAY_PASSWORD;
    });
    ...
    afterEach(() => {
      if (originalEnvToken == null) {
        ...
        process.env.THINKFLEET_GATEWAY_TOKEN = originalEnvToken;
      }
      if (originalEnvPassword == null) {
        delete process.env.THINKFLEET_GATEWAY_PASSWORD;
      } else {
        process.env.THINKFLEET_GATEWAY_PASSWORD = originalEnvPassword;
      }
    });
    ...
    it("prefers env token over local config token", async () => {
      process.env.THINKFLEET_GATEWAY_TOKEN = "env-token";
      let capturedAuth: { token?: string; password?: string } | undefined;
      const cfg: ThinkfleetConfig = {
        gateway: {
          mode: "local",
    ...
    it("prefers env password over remote password", async () => {
      process.env.THINKFLEET_GATEWAY_PASSWORD = "env-pass";
      let capturedAuth: { token?: string; password?: string } | undefined;
      const cfg: ThinkfleetConfig = {
        gateway: {
          mode: "remote",
