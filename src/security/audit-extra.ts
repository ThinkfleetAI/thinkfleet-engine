...
import type { ThinkfleetConfig, ConfigFileSnapshot } from "../config/config.js";
...
function summarizeGroupPolicy(cfg: ThinkfleetConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
export function collectAttackSurfaceSummaryFindings(cfg: ThinkfleetConfig): SecurityAuditFinding[] {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep THINKFLEET_STATE_DIR on a local-only volume and re-run "${formatCliCommand("thinkfleet security audit --fix")}".`,
    });
  }
  return findings;
}
...
export function collectSecretsInConfigFindings(cfg: ThinkfleetConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer THINKFLEET_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectHooksHardeningFindings(cfg: ThinkfleetConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectModels(cfg: ThinkfleetConfig): ModelRef[] {
  const out: ModelRef[] = [];
...
export function collectModelHygieneFindings(cfg: ThinkfleetConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function hasWebSearchKey(cfg: ThinkfleetConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: ThinkfleetConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: ThinkfleetConfig): boolean {
  const enabled = cfg.tools?.web?.fetch?.enabled;
  if (enabled === false) return false;
  return true;
}

function isBrowserEnabled(cfg: ThinkfleetConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: ThinkfleetConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
...
export function collectExposureMatrixFindings(cfg: ThinkfleetConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
