  ...
  if (stateDirPerms.ok) {
    ...
    if (stateDirPerms.worldWritable) {
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; other users can write into your Thinkfleet state.`,
      ...
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; group users can write into your Thinkfleet state.`,
        remediation: formatPermissionRemediation({
          targetPath: params.stateDir,
          perms: stateDirPerms,
          isDir: true,
...
function collectBrowserControlFindings(cfg: ThinkfleetConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  ...
  try {
    ...
    findings.push({
      ...
      detail: String(err),
      remediation: `Fix browser.cdpUrl in ${resolveConfigPath()} and re-run "${formatCliCommand("thinkfleet security audit --deep")}".`,
    });
    return findings;
  }
...
function collectLoggingFindings(cfg: ThinkfleetConfig): SecurityAuditFinding[] {
...
function collectElevatedFindings(cfg: ThinkfleetConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
    ...
    const token =
      mode === "remote"
        ? typeof remote?.token === "string" && remote.token.trim()
          ? remote.token.trim()
          : undefined
        : process.env.THINKFLEET_GATEWAY_TOKEN?.trim() ||
          (typeof authToken === "string" && authToken.trim() ? authToken.trim() : undefined);
    const password =
      process.env.THINKFLEET_GATEWAY_PASSWORD?.trim() ||
      (mode === "remote"
...
export async function runSecurityAudit(opts: SecurityAuditOptions): Promise<SecurityAuditReport> {
  ...
  if (deep?.gateway?.attempted && deep.gateway.ok === false) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("thinkfleet status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("thinkfleet security audit --deep")}".`,
    });
  }
