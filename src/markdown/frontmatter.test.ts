...
`;
    ...
    const parsed = JSON5.parse(result.metadata ?? "") as { thinkfleet?: { emoji?: string } };
    expect(parsed.thinkfleet?.emoji).toBe("disk");
  });
...
`;
    ...
    const parsed = JSON5.parse(result.metadata ?? "") as { thinkfleet?: { events?: string[] } };
