...
const execHostEnforced = process.env.THINKFLEET_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed =
  process.env.THINKFLEET_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
function ensureNodePathEnv(): string {
  ensureThinkfleetCliOnPath({ pathEnv: process.env.PATH ?? "" });
