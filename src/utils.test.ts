...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    const tmp = await fs.promises.mkdtemp(path.join(os.tmpdir(), "thinkfleet-test-"));
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    const authDir = fs.mkdtempSync(path.join(os.tmpdir(), "thinkfleet-auth-"));
  ...
  it("falls back through lidMappingDirs in order", () => {
    const first = fs.mkdtempSync(path.join(os.tmpdir(), "thinkfleet-lid-a-"));
    const second = fs.mkdtempSync(path.join(os.tmpdir(), "thinkfleet-lid-b-"));
...
describe("resolveConfigDir", () => {
  it("prefers ~/.thinkfleet when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "thinkfleet-config-dir-"));
    try {
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/thinkfleet")).toBe(path.resolve(os.homedir(), "thinkfleet"));
  });
