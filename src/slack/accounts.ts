...
export function listSlackAccountIds(cfg: ThinkfleetConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultSlackAccountId(cfg: ThinkfleetConfig): string {
...
function mergeSlackAccountConfig(cfg: ThinkfleetConfig, accountId: string): SlackAccountConfig {
...
export function listEnabledSlackAccounts(cfg: ThinkfleetConfig): ResolvedSlackAccount[] {
