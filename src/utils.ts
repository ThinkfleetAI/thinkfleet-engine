...
export function resolveConfigDir(
  ...
  const override = env.THINKFLEET_STATE_DIR?.trim() || env.THINKFLEET_STATE_DIR?.trim();
  if (override) return resolveUserPath(override);
  const legacyDir = path.join(homedir(), ".thinkfleet");
  const newDir = path.join(homedir(), ".thinkfleet");
  try {
...
// Configuration root; can be overridden via THINKFLEET_STATE_DIR.
