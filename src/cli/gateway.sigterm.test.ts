...
describe("gateway SIGTERM", () => {
  ...
  it("exits 0 on SIGTERM", { timeout: 180_000 }, async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "thinkfleet-gateway-test-"));
    const out: string[] = [];
    const err: string[] = [];
    ...
    const env = {
      ...process.env,
      THINKFLEET_NO_RESPAWN: "1",
      THINKFLEET_STATE_DIR: stateDir,
      THINKFLEET_SKIP_CHANNELS: "1",
      THINKFLEET_SKIP_GMAIL_WATCHER: "1",
      THINKFLEET_SKIP_CRON: "1",
      THINKFLEET_SKIP_BROWSER_CONTROL_SERVER: "1",
      THINKFLEET_SKIP_CANVAS_HOST: "1",
    };
    const bootstrapPath = path.join(stateDir, "thinkfleet-entry-bootstrap.mjs");
