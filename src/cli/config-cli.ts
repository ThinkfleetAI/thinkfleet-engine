...
async function loadValidConfig() {
  ...
  defaultRuntime.error(`Run \`${formatCliCommand("thinkfleet doctor")}\` to repair, then retry.`);
...
export function registerConfigCli(program: Command) {
  const cmd = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/config", "docs.thinkfleet.ai/cli/config")}\n`,
    )
    .option(
      "--section <section>",
