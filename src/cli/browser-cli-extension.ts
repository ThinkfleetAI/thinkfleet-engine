...
export async function installChromeExtension(opts?: {
  ...
  if (!hasManifest(src)) {
    throw new Error("Bundled Chrome extension is missing. Reinstall Thinkfleet and try again.");
  }
      ...
      defaultRuntime.error(
        info(
          [
            ...
            `- Pin “Thinkfleet Browser Relay”, then click it on the tab (badge shows ON)`,
            "",
            `${theme.muted("Docs:")} ${formatDocsLink("/tools/chrome-extension", "docs.thinkfleet.ai/tools/chrome-extension")}`,
          ].join("\n"),
        ),
      );
    });
    ...
    .action(async (_opts, cmd) => {
      ...
      if (!hasManifest(dir)) {
        defaultRuntime.error(
          danger(
            [
              `Chrome extension is not installed. Run: "${formatCliCommand("thinkfleet browser extension install")}"`,
              `Docs: ${formatDocsLink("/tools/chrome-extension", "docs.thinkfleet.ai/tools/chrome-extension")}`,
            ].join("\n"),
          ),
        );
        defaultRuntime.exit(1);
