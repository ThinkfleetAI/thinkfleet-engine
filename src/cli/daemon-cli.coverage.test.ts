...
describe("daemon-cli coverage", () => {
  const originalEnv = {
    THINKFLEET_STATE_DIR: process.env.THINKFLEET_STATE_DIR,
    THINKFLEET_CONFIG_PATH: process.env.THINKFLEET_CONFIG_PATH,
    THINKFLEET_GATEWAY_PORT: process.env.THINKFLEET_GATEWAY_PORT,
    THINKFLEET_PROFILE: process.env.THINKFLEET_PROFILE,
  };

  beforeEach(() => {
    process.env.THINKFLEET_STATE_DIR = "/tmp/thinkfleet-cli-state";
    process.env.THINKFLEET_CONFIG_PATH = "/tmp/thinkfleet-cli-state/thinkfleet.json";
    delete process.env.THINKFLEET_GATEWAY_PORT;
    delete process.env.THINKFLEET_PROFILE;
    serviceReadCommand.mockResolvedValue(null);
  });

  afterEach(() => {
    if (originalEnv.THINKFLEET_STATE_DIR !== undefined)
      process.env.THINKFLEET_STATE_DIR = originalEnv.THINKFLEET_STATE_DIR;
    else delete process.env.THINKFLEET_STATE_DIR;

    if (originalEnv.THINKFLEET_CONFIG_PATH !== undefined)
      process.env.THINKFLEET_CONFIG_PATH = originalEnv.THINKFLEET_CONFIG_PATH;
    else delete process.env.THINKFLEET_CONFIG_PATH;

    if (originalEnv.THINKFLEET_GATEWAY_PORT !== undefined)
      process.env.THINKFLEET_GATEWAY_PORT = originalEnv.THINKFLEET_GATEWAY_PORT;
    else delete process.env.THINKFLEET_GATEWAY_PORT;

    if (originalEnv.THINKFLEET_PROFILE !== undefined)
      process.env.THINKFLEET_PROFILE = originalEnv.THINKFLEET_PROFILE;
    else delete process.env.THINKFLEET_PROFILE;
  });
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        THINKFLEET_PROFILE: "dev",
        THINKFLEET_STATE_DIR: "/tmp/thinkfleet-daemon-state",
