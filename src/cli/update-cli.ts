...
import { resolveThinkfleetPackageRoot } from "../infra/thinkfleet-root.js";
import {
  checkUpdateStatus,
...
const STEP_LABELS: Record<string, string> = {
  ...
  "thinkfleet doctor": "Running doctor checks",
...
const UPDATE_QUIPS = [
  ...
  "New firmware, same mission. Let's roll.",
  ...
  "Upgrade complete. Please don't look at the old version.",
  "Version bump! Same chaos energy, fewer crashes (probably).",
];

const MAX_LOG_CHARS = 8000;
const DEFAULT_PACKAGE_NAME = "thinkfleet";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME, "thinkfleet"]);
const CLI_NAME = resolveCliName();
const THINKFLEET_REPO_URL = "https://github.com/thinkfleet/thinkfleet.git";
const DEFAULT_GIT_DIR = path.join(os.homedir(), "thinkfleet");

function normalizeTag(value?: string | null): string | null {
  if (!value) return null;
  const trimmed = value.trim();
  if (!trimmed) return null;
  if (trimmed.startsWith("thinkfleet@")) return trimmed.slice("thinkfleet@".length);
...
function resolveGitInstallDir(): string {
  const override = process.env.THINKFLEET_GIT_DIR?.trim();
...
async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", THINKFLEET_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `THINKFLEET_GIT_DIR points at a non-git directory: ${params.dir}. Set THINKFLEET_GIT_DIR to an empty folder or a thinkfleet checkout.`,
      );
    }
    return await runUpdateStep({
      name: "git clone",
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`THINKFLEET_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function updateStatusCommand(opts: UpdateStatusOptions): Promise<void> {
  ...
  const root =
    (await resolveThinkfleetPackageRoot({
  ...
  defaultRuntime.log(theme.heading("Thinkfleet update status"));
  defaultRuntime.log("");
  defaultRuntime.log(
    renderTable({
      width: tableWidth,
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating Thinkfleet..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this Thinkfleet install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("thinkfleet doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("thinkfleet gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g thinkfleet@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g thinkfleet@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
  ...
  if (shouldRestart) {
    ...
    try {
      ...
      if (!opts.json && restarted) {
        ...
        defaultRuntime.log("");
        process.env.THINKFLEET_UPDATE_IN_PROGRESS = "1";
        try {
          ...
          delete process.env.THINKFLEET_UPDATE_IN_PROGRESS;
        }
      }
    } catch (err) {
      if (!opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("thinkfleet gateway restart"), CLI_NAME)}`,
          ),
        );
      }
    }
    ...
    if (result.mode === "npm" || result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("thinkfleet doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("thinkfleet gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("thinkfleet gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
export async function updateWizardCommand(opts: UpdateWizardOptions = {}): Promise<void> {
  if (!process.stdin.isTTY) {
    defaultRuntime.error(
      "Update wizard requires a TTY. Use `thinkfleet update --channel <stable|beta|dev>` instead.",
    );
    defaultRuntime.exit(1);
    return;
  }
  ...
  if (requestedChannel === "dev" && updateStatus.installKind !== "git") {
    ...
    if (!hasGit) {
      ...
      if (dirExists) {
        ...
        if (!empty) {
          defaultRuntime.error(
            `THINKFLEET_GIT_DIR points at a non-git directory: ${gitDir}. Set THINKFLEET_GIT_DIR to an empty folder or a thinkfleet checkout.`,
          );
          defaultRuntime.exit(1);
          return;
        }
      ...
      const ok = await confirm({
        message: stylePromptMessage(
          `Create a git checkout at ${gitDir}? (override via THINKFLEET_GIT_DIR)`,
        ),
        initialValue: true,
      });
      if (isCancel(ok) || ok === false) {
...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update Thinkfleet to the latest version")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["thinkfleet update", "Update a source checkout (git)"],
        ["thinkfleet update --channel beta", "Switch to beta channel (git + npm)"],
        ["thinkfleet update --channel dev", "Switch to dev channel (git + npm)"],
        ["thinkfleet update --tag beta", "One-off update to a dist-tag or version"],
        ["thinkfleet update --no-restart", "Update without restarting the service"],
        ["thinkfleet update --json", "Output result as JSON"],
        ["thinkfleet update --yes", "Non-interactive (accept downgrade prompts)"],
        ["thinkfleet update wizard", "Interactive update wizard"],
        ["thinkfleet --update", "Shorthand for thinkfleet update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run thinkfleet update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.thinkfleet.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["thinkfleet update status", "Show channel + version status."],
          ["thinkfleet update status --json", "JSON output."],
          ["thinkfleet update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.thinkfleet.ai/cli/update")}`,
    )
    .action(async (opts) => {
      try {
