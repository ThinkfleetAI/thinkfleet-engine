...
const shouldRegisterPrimaryOnly = (argv: string[]) => {
  if (isTruthyEnvValue(process.env.THINKFLEET_DISABLE_LAZY_SUBCOMMANDS)) return false;
  if (hasHelpOrVersion(argv)) return false;
  return true;
};

const shouldEagerRegisterSubcommands = (_argv: string[]) => {
  return isTruthyEnvValue(process.env.THINKFLEET_DISABLE_LAZY_SUBCOMMANDS);
};

const loadConfig = async (): Promise<ThinkfleetConfig> => {
