...
async function runGatewayCommand(opts: GatewayRunOpts) {
  const isDevProfile = process.env.THINKFLEET_PROFILE?.trim().toLowerCase() === "dev";
  ...
  if (opts.claudeCliLogs) {
    ...
    process.env.THINKFLEET_CLAUDE_CLI_LOG_OUTPUT = "1";
  }
  ...
  if (opts.rawStream) {
    process.env.THINKFLEET_RAW_STREAM = "1";
  }
  const rawStreamPath = toOptionString(opts.rawStreamPath);
  if (rawStreamPath) {
    process.env.THINKFLEET_RAW_STREAM_PATH = rawStreamPath;
  }
  ...
  if (opts.token) {
    ...
    if (token) process.env.THINKFLEET_GATEWAY_TOKEN = token;
  }
  ...
  if (!opts.allowUnconfigured && mode !== "local") {
    if (!configExists) {
      defaultRuntime.error(
        `Missing config. Run \`${formatCliCommand("thinkfleet setup")}\` or set gateway.mode=local (or pass --allow-unconfigured).`,
      );
    } else {
      defaultRuntime.error(
  ...
  if (resolvedAuthMode === "token" && !hasToken && !resolvedAuth.allowTailscale) {
    defaultRuntime.error(
      [
        ...
        "Set gateway.auth.token (or THINKFLEET_GATEWAY_TOKEN), or pass --token.",
        ...authHints,
      ]
        .filter(Boolean)
        .join("\n"),
  ...
  if (resolvedAuthMode === "password" && !hasPassword) {
    defaultRuntime.error(
      [
        ...
        "Set gateway.auth.password (or THINKFLEET_GATEWAY_PASSWORD), or pass --password.",
        ...authHints,
      ]
        .filter(Boolean)
        .join("\n"),
  ...
  if (bind !== "loopback" && !hasSharedSecret) {
    defaultRuntime.error(
      [
        ...
        "Set gateway.auth.token/password (or THINKFLEET_GATEWAY_TOKEN/THINKFLEET_GATEWAY_PASSWORD) or pass --token/--password.",
        ...authHints,
      ]
        .filter(Boolean)
        .join("\n"),
  ...
  try {
    ...
    if (
      ...
      defaultRuntime.error(
        `Gateway failed to start: ${errMessage}\nIf the gateway is supervised, stop it with: ${formatCliCommand("thinkfleet gateway stop")}`,
      );
      try {
...
export function addGatewayRunCommand(cmd: Command): Command {
  return cmd
    ...
    .option(
      "--token <token>",
      "Shared token required in connect.params.auth.token (default: THINKFLEET_GATEWAY_TOKEN env if set)",
    )
