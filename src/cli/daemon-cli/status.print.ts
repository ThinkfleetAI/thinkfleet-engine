...
export function printDaemonStatus(status: DaemonStatus, opts: { json: boolean }) {
  ...
  if (service.configAudit?.issues.length) {
    ...
    defaultRuntime.error(
      warnText(
        `Recommendation: run "${formatCliCommand("thinkfleet doctor")}" (or "${formatCliCommand("thinkfleet doctor --repair")}").`,
      ),
    );
  }
  ...
  if (status.config) {
    ...
    if (status.config.mismatch) {
      ...
      defaultRuntime.error(
        errorText(
          `Fix: rerun \`${formatCliCommand("thinkfleet gateway install --force")}\` from the same --profile / THINKFLEET_STATE_DIR you expect.`,
        ),
      );
    }
    spacer();
  ...
  if (service.runtime?.cachedLabel) {
    ...
    const labelValue = resolveGatewayLaunchAgentLabel(env.THINKFLEET_PROFILE);
    defaultRuntime.error(
      errorText(
    ...
    defaultRuntime.error(
      errorText(`Then reinstall: ${formatCliCommand("thinkfleet gateway install")}`),
    );
    spacer();
  }
  ...
  if (
    ...
    if (process.platform === "linux") {
      const env = (service.command?.environment ?? process.env) as NodeJS.ProcessEnv;
      const unit = resolveGatewaySystemdServiceName(env.THINKFLEET_PROFILE);
      defaultRuntime.error(
  ...
  if (legacyServices.length > 0) {
    ...
    defaultRuntime.error(errorText(`Cleanup: ${formatCliCommand("thinkfleet doctor")}`));
    spacer();
  }
  ...
  defaultRuntime.log(`${label("Troubles:")} run ${formatCliCommand("thinkfleet status")}`);
  defaultRuntime.log(`${label("Troubleshooting:")} https://docs.thinkfleet.ai/troubleshooting`);
}
