...
function createLogWriters() {
  const writer = createSafeStreamWriter({
    ...
    onBrokenPipe: (err, stream) => {
      ...
      const message = `thinkfleet logs: output ${target} closed (${code}). Stopping tail.`;
      try {
...
function emitGatewayError(
  ...
  const hint = `Hint: run \`${formatCliCommand("thinkfleet doctor")}\`.`;
...
export function registerLogsCli(program: Command) {
  const logs = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/logs", "docs.thinkfleet.ai/cli/logs")}\n`,
    );
