...
function resolveProfileStateDir(profile: string, homedir: () => string): string {
  ...
  return path.join(homedir(), `.thinkfleet${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.THINKFLEET_PROFILE = profile;

  const stateDir = env.THINKFLEET_STATE_DIR?.trim() || resolveProfileStateDir(profile, homedir);
  if (!env.THINKFLEET_STATE_DIR?.trim()) env.THINKFLEET_STATE_DIR = stateDir;

  if (!env.THINKFLEET_CONFIG_PATH?.trim()) {
    env.THINKFLEET_CONFIG_PATH = path.join(stateDir, "thinkfleet.json");
  }

  if (profile === "dev" && !env.THINKFLEET_GATEWAY_PORT?.trim()) {
    env.THINKFLEET_GATEWAY_PORT = "19001";
  }
}
