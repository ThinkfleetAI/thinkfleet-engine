...
export function registerDnsCli(program: Command) {
  const dns = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/dns", "docs.thinkfleet.ai/cli/dns")}\n`,
    );

  dns
    .command("setup")
    .description(
      "Set up CoreDNS to serve thinkfleet.internal for unicast DNS-SD (Wide-Area Bonjour)",
    )
    .option(
      "--apply",
    ...
    .action(async (opts) => {
      ...
      defaultRuntime.log("");
      defaultRuntime.log(theme.heading("Recommended ~/.thinkfleet/thinkfleet.json:"));
      defaultRuntime.log(
        JSON.stringify(
          {
            gateway: { bind: "auto" },
      ...
      defaultRuntime.log(theme.muted("- Restrict to domain (Split DNS): thinkfleet.internal"));
      ...
      const serverPath = path.join(confDir, "thinkfleet.internal.server");
      ...
      if (zoneFileNeedsBootstrap(zonePath)) {
        ...
        const zoneLines = [
          `; created by thinkfleet dns setup (will be overwritten by the gateway when wide-area discovery is enabled)`,
      ...
      if (cfg.discovery?.wideArea?.enabled !== true) {
        ...
        defaultRuntime.log(
          theme.muted(
            "Note: enable discovery.wideArea.enabled in ~/.thinkfleet/thinkfleet.json on the gateway and restart the gateway so it writes the DNS-SD zone.",
          ),
        );
      }
    });
