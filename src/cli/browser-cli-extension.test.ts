...
describe("browser extension install", () => {
  it("installs into the state dir (never node_modules)", async () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "thinkfleet-ext-"));
  ...
  it("copies extension path to clipboard", async () => {
    ...
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "thinkfleet-ext-path-"));
    ...
    try {
      ...
      if (prev === undefined) delete process.env.THINKFLEET_STATE_DIR;
      else process.env.THINKFLEET_STATE_DIR = prev;
    }
  });
});
