...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.thinkfleet.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + Thinkfleet hooks")
    ...
    .option("--hook-url <url>", "Thinkfleet hook URL")
    .option("--hook-token <token>", "Thinkfleet hook token")
