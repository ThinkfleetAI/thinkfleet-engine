...
function appendSkillsHubHint(output: string, json?: boolean): string {
  ...
  return `${output}\n\nTip: use \`thinkfleet skills search\` to find and install skills.`;
}
...
export function formatSkillsList(report: SkillStatusReport, opts: SkillsListOptions): string {
  ...
  if (skills.length === 0) {
    const message = opts.eligible
      ? `No eligible skills found. Run \`${formatCliCommand("thinkfleet skills list")}\` to see all skills.`
      : "No skills found.";
...
export function formatSkillInfo(
  ...
  if (!skill) {
    ...
    return appendSkillsHubHint(
      `Skill "${skillName}" not found. Run \`${formatCliCommand("thinkfleet skills list")}\` to see available skills.`,
      opts.json,
    );
  }
...
export function registerSkillsCli(program: Command) {
  const skills = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/skills", "docs.thinkfleet.ai/cli/skills")}\n`,
    );
