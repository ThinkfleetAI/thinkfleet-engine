...
function buildHooksReport(config: ThinkfleetConfig): HookStatusReport {
...
export function formatHooksList(report: HookStatusReport, opts: HooksListOptions): string {
  ...
  if (hooks.length === 0) {
    const message = opts.eligible
      ? `No eligible hooks found. Run \`${formatCliCommand("thinkfleet hooks list")}\` to see all hooks.`
      : "No hooks found.";
    return message;
  }
...
export function formatHookInfo(
  ...
  if (!hook) {
    ...
    return `Hook "${hookName}" not found. Run \`${formatCliCommand("thinkfleet hooks list")}\` to see available hooks.`;
  }
...
export function registerHooksCli(program: Command): void {
  const hooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/hooks", "docs.thinkfleet.ai/cli/hooks")}\n`,
    );
