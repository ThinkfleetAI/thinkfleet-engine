...
vi.mock("../infra/thinkfleet-root.js", () => ({
  resolveThinkfleetPackageRoot: vi.fn(),
}));
...
describe("update-cli", () => {
  ...
  beforeEach(async () => {
    vi.clearAllMocks();
    const { resolveThinkfleetPackageRoot } = await import("../infra/thinkfleet-root.js");
    const { readConfigFileSnapshot } = await import("../config/config.js");
    const { checkUpdateStatus, fetchNpmTagVersion, resolveNpmChannelTag } =
      await import("../infra/update-check.js");
    const { runCommandWithTimeout } = await import("../process/exec.js");
    vi.mocked(resolveThinkfleetPackageRoot).mockResolvedValue(process.cwd());
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("Thinkfleet update status");
  });
  ...
  it("defaults to stable channel for package installs when unset", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-update-"));
    try {
      await fs.writeFile(
      ...
      vi.mocked(resolveThinkfleetPackageRoot).mockResolvedValue(tempDir);
      vi.mocked(checkUpdateStatus).mockResolvedValue({
        root: tempDir,
        installKind: "package",
        packageManager: "npm",
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-update-wizard-"));
    const previousGitDir = process.env.THINKFLEET_GIT_DIR;
    try {
      setTty(true);
      process.env.THINKFLEET_GIT_DIR = tempDir;
      ...
      process.env.THINKFLEET_GIT_DIR = previousGitDir;
      await fs.rm(tempDir, { recursive: true, force: true });
    }
  });
});
