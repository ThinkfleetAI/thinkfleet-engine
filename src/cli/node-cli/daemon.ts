...
function renderNodeServiceStartHints(): string[] {
  const base = [
    formatCliCommand("thinkfleet node install"),
    formatCliCommand("thinkfleet node start"),
  ];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("thinkfleet node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    THINKFLEET_LOG_PREFIX: baseEnv.THINKFLEET_LOG_PREFIX ?? "node",
