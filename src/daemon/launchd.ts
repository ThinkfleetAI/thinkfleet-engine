...
function resolveLaunchAgentLabel(args?: { env?: Record<string, string | undefined> }): string {
  const envLabel = args?.env?.THINKFLEET_LAUNCHD_LABEL?.trim();
  if (envLabel) return envLabel;
  return resolveGatewayLaunchAgentLabel(args?.env?.THINKFLEET_PROFILE);
}
...
export function resolveGatewayLogPaths(env: Record<string, string | undefined>): {
  ...
  const prefix = env.THINKFLEET_LOG_PREFIX?.trim() || "gateway";
  return {
    logDir,
...
export async function findLegacyLaunchAgents(
  ...
  for (const label of resolveLegacyGatewayLaunchAgentLabels(env.THINKFLEET_PROFILE)) {
...
export async function installLaunchAgent({
  ...
  const domain = resolveGuiDomain();
  const label = resolveLaunchAgentLabel({ env });
  for (const legacyLabel of resolveLegacyGatewayLaunchAgentLabels(env.THINKFLEET_PROFILE)) {
  ...
  const serviceDescription =
    description ??
    formatGatewayServiceDescription({
      profile: env.THINKFLEET_PROFILE,
      version: environment?.THINKFLEET_SERVICE_VERSION ?? env.THINKFLEET_SERVICE_VERSION,
    });
