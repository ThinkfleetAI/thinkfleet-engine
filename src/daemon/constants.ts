...
export function resolveLegacyGatewayLaunchAgentLabels(profile?: string): string[] {
  ...
  return [...LEGACY_GATEWAY_LAUNCH_AGENT_LABELS, `com.thinkfleet.${normalized}`];
}

export function resolveGatewaySystemdServiceName(profile?: string): string {
  const suffix = resolveGatewayProfileSuffix(profile);
  if (!suffix) return GATEWAY_SYSTEMD_SERVICE_NAME;
  return `thinkfleet-gateway${suffix}`;
}

export function resolveGatewayWindowsTaskName(profile?: string): string {
  const normalized = normalizeGatewayProfile(profile);
  if (!normalized) return GATEWAY_WINDOWS_TASK_NAME;
  return `Thinkfleet Gateway (${normalized})`;
}
...
export function formatGatewayServiceDescription(params?: {
  ...
  if (parts.length === 0) return "Thinkfleet Gateway";
  return `Thinkfleet Gateway (${parts.join(", ")})`;
}
...
export function formatNodeServiceDescription(params?: { version?: string }): string {
  const version = params?.version?.trim();
  if (!version) return "Thinkfleet Node Host";
  return `Thinkfleet Node Host (v${version})`;
}
