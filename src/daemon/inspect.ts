...
const EXTRA_MARKERS = ["thinkfleet"];
const execFileAsync = promisify(execFile);

export function renderGatewayServiceCleanupHints(
  env: Record<string, string | undefined> = process.env as Record<string, string | undefined>,
): string[] {
  const profile = env.THINKFLEET_PROFILE;
...
function hasGatewayServiceMarker(content: string): boolean {
  const lower = content.toLowerCase();
  return (
    lower.includes("thinkfleet_service_marker") &&
    lower.includes(GATEWAY_SERVICE_MARKER.toLowerCase()) &&
    lower.includes("thinkfleet_service_kind") &&
    lower.includes(GATEWAY_SERVICE_KIND.toLowerCase())
  );
}

function isThinkfleetGatewayLaunchdService(label: string, contents: string): boolean {
...
function isThinkfleetGatewaySystemdService(name: string, contents: string): boolean {
  if (hasGatewayServiceMarker(contents)) return true;
  if (!name.startsWith("thinkfleet-gateway")) return false;
  return contents.toLowerCase().includes("gateway");
}

function isThinkfleetGatewayTaskName(name: string): boolean {
  const normalized = name.trim().toLowerCase();
  if (!normalized) return false;
  const defaultName = resolveGatewayWindowsTaskName().toLowerCase();
  return normalized === defaultName || normalized.startsWith("thinkfleet gateway");
}
...
function isIgnoredLaunchdLabel(label: string): boolean {
  return (
    ...
    resolveLegacyGatewayLaunchAgentLabels(process.env.THINKFLEET_PROFILE).includes(label)
  );
}
...
async function scanLaunchdDir(params: {
  ...
  for (const entry of entries) {
    ...
    if (isThinkfleetGatewayLaunchdService(label, contents)) continue;
    results.push({
      platform: "darwin",
      label,
...
async function scanSystemdDir(params: {
  ...
  for (const entry of entries) {
    ...
    if (!containsMarker(contents)) continue;
    if (isThinkfleetGatewaySystemdService(name, contents)) continue;
    results.push({
      platform: "linux",
      label: entry,
...
export async function findExtraGatewayServices(
  ...
  if (process.platform === "win32") {
    ...
    for (const task of tasks) {
      ...
      if (!name) continue;
      if (isThinkfleetGatewayTaskName(name)) continue;
