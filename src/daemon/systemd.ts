...
function resolveSystemdServiceName(env: Record<string, string | undefined>): string {
  const override = env.THINKFLEET_SYSTEMD_UNIT?.trim();
  if (override) {
    return override.endsWith(".service") ? override.slice(0, -".service".length) : override;
  }
  return resolveGatewaySystemdServiceName(env.THINKFLEET_PROFILE);
}
...
export async function installSystemdService({
  ...
  const serviceName = resolveGatewaySystemdServiceName(env.THINKFLEET_PROFILE);
