...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `thinkfleet-plugin-install-${randomUUID()}`);
...
describe("installPluginFromArchive", () => {
  it("installs into ~/.thinkfleet/extensions and uses unscoped id", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        version: "0.0.1",
        thinkfleet: { extensions: ["./dist/index.js"] },
      }),
      "utf-8",
    );
  ...
  it("rejects packages without thinkfleet.extensions", async () => {
    const stateDir = makeTempDir();
    const workDir = makeTempDir();
    const pkgDir = path.join(workDir, "package");
    ...
    fs.writeFileSync(
      path.join(pkgDir, "package.json"),
      JSON.stringify({ name: "@thinkfleet/nope", version: "0.0.1" }),
      "utf-8",
    );
    ...
    if (result.ok) return;
    expect(result.error).toContain("thinkfleet.extensions");
  });
});
