...
import { discoverThinkfleetPlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: ThinkfleetPluginDefinition;
  register?: ThinkfleetPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as ThinkfleetPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as ThinkfleetPluginDefinition;
...
export function loadThinkfleetPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverThinkfleetPlugins({
    workspaceDir: options.workspaceDir,
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: ThinkfleetPluginModule | null = null;
    try {
      mod = jiti(candidate.source) as ThinkfleetPluginModule;
    } catch (err) {
