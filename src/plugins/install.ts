...
async function ensureThinkfleetExtensions(manifest: PackageManifest) {
  const extensions = manifest.thinkfleet?.extensions ?? manifest[LEGACY_MANIFEST_KEY]?.extensions;
  if (!Array.isArray(extensions)) {
    throw new Error("package.json missing thinkfleet.extensions");
  }
  const list = extensions.map((e) => (typeof e === "string" ? e.trim() : "")).filter(Boolean);
  if (list.length === 0) {
    throw new Error("package.json thinkfleet.extensions is empty");
  }
  return list;
}
...
async function installPluginFromPackageDir(params: {
  ...
  try {
    extensions = await ensureThinkfleetExtensions(manifest);
  } catch (err) {
...
export async function installPluginFromArchive(params: {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-plugin-"));
...
export async function installPluginFromNpmSpec(params: {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-npm-pack-"));
