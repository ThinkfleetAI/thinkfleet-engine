...
import { loadThinkfleetPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const tempDirs: string[] = [];
const prevBundledDir = process.env.THINKFLEET_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `thinkfleet-plugin-${randomUUID()}`);
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "thinkfleet.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.THINKFLEET_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadThinkfleetPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.THINKFLEET_BUNDLED_PLUGINS_DIR = bundledDir;

    const registry = loadThinkfleetPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadThinkfleetPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it("preserves package.json metadata for bundled memory plugins", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@thinkfleet/memory-core",
        version: "1.2.3",
        description: "Memory plugin package",
        thinkfleet: { extensions: ["./index.ts"] },
      }),
      "utf-8",
    );
    writePlugin({
