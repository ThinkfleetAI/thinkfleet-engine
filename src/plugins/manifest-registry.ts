...
import { discoverThinkfleetPlugins, type PluginCandidate } from "./discovery.js";
...
function resolveManifestCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.THINKFLEET_PLUGIN_MANIFEST_CACHE_MS?.trim();
...
function shouldUseManifestCache(env: NodeJS.ProcessEnv): boolean {
  const disabled = env.THINKFLEET_DISABLE_PLUGIN_MANIFEST_CACHE?.trim();
