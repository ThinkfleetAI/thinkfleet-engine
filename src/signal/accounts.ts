...
export function listSignalAccountIds(cfg: ThinkfleetConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultSignalAccountId(cfg: ThinkfleetConfig): string {
...
function mergeSignalAccountConfig(cfg: ThinkfleetConfig, accountId: string): SignalAccountConfig {
...
export function listEnabledSignalAccounts(cfg: ThinkfleetConfig): ResolvedSignalAccount[] {
