...
const { sessionStorePath } = vi.hoisted(() => ({
  sessionStorePath: `/tmp/thinkfleet-telegram-bot-${Math.random().toString(16).slice(2)}.json`,
}));
...
describe("createTelegramBot", () => {
    ...
    await handler({
      message: {
        ...
        reply_to_message: {
          message_id: 42,
          text: "original reply",
          from: { id: 999, first_name: "Thinkfleet" },
        },
      },
      me: { id: 999, username: "thinkfleet_bot" },
      getFile: async () => ({ download: async () => new Uint8Array() }),
    });
  ...
  it("honors routed group activation from session store", async () => {
    ...
    const replySpy = replyModule.__replySpy as unknown as ReturnType<typeof vi.fn>;
    replySpy.mockReset();
    const storeDir = fs.mkdtempSync(path.join(os.tmpdir(), "thinkfleet-telegram-"));
