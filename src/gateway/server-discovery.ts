...
export function formatBonjourInstanceName(displayName: string) {
  ...
  if (!trimmed) return "Thinkfleet";
  if (/thinkfleet/i.test(trimmed)) return trimmed;
  return `${trimmed} (Thinkfleet)`;
}

export function resolveBonjourCliPath(opts: ResolveBonjourCliPathOptions = {}): string | undefined {
  const env = opts.env ?? process.env;
  const envPath = env.THINKFLEET_CLI_PATH?.trim();
  ...
  const siblingCli = path.join(execDir, "thinkfleet");
  ...
  const binCli = path.join(cwd, "bin", "thinkfleet.js");
...
export async function resolveTailnetDnsHint(opts?: {
  ...
  const env = opts?.env ?? process.env;
  const envRaw = env.THINKFLEET_TAILNET_DNS?.trim();
