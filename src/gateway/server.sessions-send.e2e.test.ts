...
import { createThinkfleetTools } from "../agents/thinkfleet-tools.js";
...
beforeAll(async () => {
  ...
  prevGatewayToken = process.env.THINKFLEET_GATEWAY_TOKEN;
  gatewayPort = await getFreePort();
  process.env.THINKFLEET_GATEWAY_PORT = String(gatewayPort);
  process.env.THINKFLEET_GATEWAY_TOKEN = "test-token";
...
afterAll(async () => {
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.THINKFLEET_GATEWAY_TOKEN = prevGatewayToken;
  }
});
...
describe("sessions_send gateway loopback", () => {
  it("returns reply when lifecycle ends before agent.wait", async () => {
    ...
    const tool = createThinkfleetTools().find((candidate) => candidate.name === "sessions_send");
