...
async function setupGatewayTestHome() {
  ...
  previousStateDir = process.env.THINKFLEET_STATE_DIR;
  previousConfigPath = process.env.THINKFLEET_CONFIG_PATH;
  previousSkipBrowserControl = process.env.THINKFLEET_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.THINKFLEET_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.THINKFLEET_SKIP_CANVAS_HOST;
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.THINKFLEET_STATE_DIR = path.join(tempHome, ".thinkfleet");
  delete process.env.THINKFLEET_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.THINKFLEET_SKIP_BROWSER_CONTROL_SERVER = "1";
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  tempConfigRoot = options.uniqueConfigRoot
    ? await fs.mkdtemp(path.join(tempHome, "thinkfleet-test-"))
    : path.join(tempHome, ".thinkfleet-test");
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousStateDir === undefined) delete process.env.THINKFLEET_STATE_DIR;
    else process.env.THINKFLEET_STATE_DIR = previousStateDir;
    if (previousConfigPath === undefined) delete process.env.THINKFLEET_CONFIG_PATH;
    else process.env.THINKFLEET_CONFIG_PATH = previousConfigPath;
    if (previousSkipBrowserControl === undefined)
      delete process.env.THINKFLEET_SKIP_BROWSER_CONTROL_SERVER;
    else process.env.THINKFLEET_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    if (previousSkipGmailWatcher === undefined) delete process.env.THINKFLEET_SKIP_GMAIL_WATCHER;
    else process.env.THINKFLEET_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    if (previousSkipCanvasHost === undefined) delete process.env.THINKFLEET_SKIP_CANVAS_HOST;
    else process.env.THINKFLEET_SKIP_CANVAS_HOST = previousSkipCanvasHost;
  }
...
export async function startServerWithClient(token?: string, opts?: GatewayServerOptions) {
  ...
  const prev = process.env.THINKFLEET_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.THINKFLEET_GATEWAY_TOKEN = fallbackToken;
  }
  ...
  const defaultPassword =
    opts?.skipDefaultAuth === true
      ...
      : typeof (testState.gatewayAuth as { password?: unknown } | undefined)?.password === "string"
        ...
        : process.env.THINKFLEET_GATEWAY_PASSWORD;
