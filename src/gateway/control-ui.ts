...
function injectControlUiConfig(html: string, opts: ControlUiInjectionOpts): string {
  ...
  const script =
    `<script>` +
    `window.__THINKFLEET_CONTROL_UI_BASE_PATH__=${JSON.stringify(basePath)};` +
    `window.__THINKFLEET_ASSISTANT_NAME__=${JSON.stringify(
      assistantName ?? DEFAULT_ASSISTANT_IDENTITY.name,
    )};` +
    `window.__THINKFLEET_ASSISTANT_AVATAR__=${JSON.stringify(
      assistantAvatar ?? DEFAULT_ASSISTANT_IDENTITY.avatar,
    )};` +
    `</script>`;
  // Check if already injected
  if (html.includes("__THINKFLEET_ASSISTANT_NAME__")) return html;
