...
export function resolveAgentIdFromHeader(req: IncomingMessage): string | undefined {
  const raw =
    getHeader(req, "x-thinkfleet-agent-id")?.trim() ||
    getHeader(req, "x-thinkfleet-agent")?.trim() ||
    "";
...
export function resolveAgentIdFromModel(model: string | undefined): string | undefined {
  ...
  const m =
    raw.match(/^thinkfleet[:/](?<agentId>[a-z0-9][a-z0-9_-]{0,63})$/i) ??
...
export function resolveSessionKey(params: {
  ...
  const explicit = getHeader(params.req, "x-thinkfleet-session-key")?.trim();
