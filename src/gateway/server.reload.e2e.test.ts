...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.THINKFLEET_SKIP_CHANNELS;
    prevSkipGmail = process.env.THINKFLEET_SKIP_GMAIL_WATCHER;
    process.env.THINKFLEET_SKIP_CHANNELS = "0";
    delete process.env.THINKFLEET_SKIP_GMAIL_WATCHER;
  });

  afterEach(() => {
    if (prevSkipChannels === undefined) {
      delete process.env.THINKFLEET_SKIP_CHANNELS;
    } else {
      process.env.THINKFLEET_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.THINKFLEET_SKIP_GMAIL_WATCHER;
    } else {
      process.env.THINKFLEET_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
  });
