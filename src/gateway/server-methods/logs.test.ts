...
describe("logs.tail", () => {
  ...
  it("falls back to latest rolling log file when today is missing", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-logs-"));
    const older = path.join(tempDir, "thinkfleet-2026-01-20.log");
    const newer = path.join(tempDir, "thinkfleet-2026-01-21.log");
    ...
    setLoggerOverride({ file: path.join(tempDir, "thinkfleet-2026-01-22.log") });
