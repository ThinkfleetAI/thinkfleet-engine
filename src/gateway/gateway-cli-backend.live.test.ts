...
const LIVE =
  isTruthyEnvValue(process.env.LIVE) || isTruthyEnvValue(process.env.THINKFLEET_LIVE_TEST);
const CLI_LIVE = isTruthyEnvValue(process.env.THINKFLEET_LIVE_CLI_BACKEND);
const CLI_IMAGE = isTruthyEnvValue(process.env.THINKFLEET_LIVE_CLI_BACKEND_IMAGE_PROBE);
const CLI_RESUME = isTruthyEnvValue(process.env.THINKFLEET_LIVE_CLI_BACKEND_RESUME_PROBE);
...
function parseImageMode(raw?: string): "list" | "repeat" | undefined {
  ...
  throw new Error("THINKFLEET_LIVE_CLI_BACKEND_IMAGE_MODE must be 'list' or 'repeat'.");
}
...
describeLive("gateway live (cli backend)", () => {
  it("runs the agent pipeline against the local CLI backend", async () => {
    const previous = {
      configPath: process.env.THINKFLEET_CONFIG_PATH,
      token: process.env.THINKFLEET_GATEWAY_TOKEN,
      skipChannels: process.env.THINKFLEET_SKIP_CHANNELS,
      skipGmail: process.env.THINKFLEET_SKIP_GMAIL_WATCHER,
      skipCron: process.env.THINKFLEET_SKIP_CRON,
      skipCanvas: process.env.THINKFLEET_SKIP_CANVAS_HOST,
      anthropicApiKey: process.env.ANTHROPIC_API_KEY,
      anthropicApiKeyOld: process.env.ANTHROPIC_API_KEY_OLD,
    };

    process.env.THINKFLEET_SKIP_CHANNELS = "1";
    process.env.THINKFLEET_SKIP_GMAIL_WATCHER = "1";
    process.env.THINKFLEET_SKIP_CRON = "1";
    process.env.THINKFLEET_SKIP_CANVAS_HOST = "1";
    delete process.env.ANTHROPIC_API_KEY;
    delete process.env.ANTHROPIC_API_KEY_OLD;

    const token = `test-${randomUUID()}`;
    process.env.THINKFLEET_GATEWAY_TOKEN = token;

    const rawModel = process.env.THINKFLEET_LIVE_CLI_BACKEND_MODEL ?? DEFAULT_MODEL;
    const parsed = parseModelRef(rawModel, "claude-cli");
    if (!parsed) {
      throw new Error(
        `THINKFLEET_LIVE_CLI_BACKEND_MODEL must resolve to a CLI backend model. Got: ${rawModel}`,
      );
    }
    ...
    const cliCommand = process.env.THINKFLEET_LIVE_CLI_BACKEND_COMMAND ?? providerDefaults?.command;
    if (!cliCommand) {
      throw new Error(
        `THINKFLEET_LIVE_CLI_BACKEND_COMMAND is required for provider "${providerId}".`,
      );
    }
    const baseCliArgs =
      parseJsonStringArray(
        "THINKFLEET_LIVE_CLI_BACKEND_ARGS",
        process.env.THINKFLEET_LIVE_CLI_BACKEND_ARGS,
      ) ?? providerDefaults?.args;
    if (!baseCliArgs || baseCliArgs.length === 0) {
      throw new Error(`THINKFLEET_LIVE_CLI_BACKEND_ARGS is required for provider "${providerId}".`);
    }
    const cliClearEnv =
      parseJsonStringArray(
        "THINKFLEET_LIVE_CLI_BACKEND_CLEAR_ENV",
        process.env.THINKFLEET_LIVE_CLI_BACKEND_CLEAR_ENV,
      ) ?? (providerId === "claude-cli" ? DEFAULT_CLEAR_ENV : []);
    const cliImageArg = process.env.THINKFLEET_LIVE_CLI_BACKEND_IMAGE_ARG?.trim() || undefined;
    const cliImageMode = parseImageMode(process.env.THINKFLEET_LIVE_CLI_BACKEND_IMAGE_MODE);

    if (cliImageMode && !cliImageArg) {
      throw new Error(
        "THINKFLEET_LIVE_CLI_BACKEND_IMAGE_MODE requires THINKFLEET_LIVE_CLI_BACKEND_IMAGE_ARG.",
      );
    }

    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-live-cli-"));
    const disableMcpConfig = process.env.THINKFLEET_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG !== "0";
    ...
    const tempConfigPath = path.join(tempDir, "thinkfleet.json");
    await fs.writeFile(tempConfigPath, `${JSON.stringify(nextCfg, null, 2)}\n`);
    process.env.THINKFLEET_CONFIG_PATH = tempConfigPath;
    ...
    try {
      ...
      if (previous.configPath === undefined) delete process.env.THINKFLEET_CONFIG_PATH;
      else process.env.THINKFLEET_CONFIG_PATH = previous.configPath;
      if (previous.token === undefined) delete process.env.THINKFLEET_GATEWAY_TOKEN;
      else process.env.THINKFLEET_GATEWAY_TOKEN = previous.token;
      if (previous.skipChannels === undefined) delete process.env.THINKFLEET_SKIP_CHANNELS;
      else process.env.THINKFLEET_SKIP_CHANNELS = previous.skipChannels;
      if (previous.skipGmail === undefined) delete process.env.THINKFLEET_SKIP_GMAIL_WATCHER;
      else process.env.THINKFLEET_SKIP_GMAIL_WATCHER = previous.skipGmail;
      if (previous.skipCron === undefined) delete process.env.THINKFLEET_SKIP_CRON;
      else process.env.THINKFLEET_SKIP_CRON = previous.skipCron;
      if (previous.skipCanvas === undefined) delete process.env.THINKFLEET_SKIP_CANVAS_HOST;
      else process.env.THINKFLEET_SKIP_CANVAS_HOST = previous.skipCanvas;
