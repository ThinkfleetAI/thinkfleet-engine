export const MAX_PAYLOAD_BYTES = 5 * 1024 * 1024; // cap incoming frame size (5 MB — matches SaaS client-side expectations)
...
export const getHandshakeTimeoutMs = () => {
  if (process.env.VITEST && process.env.THINKFLEET_TEST_HANDSHAKE_TIMEOUT_MS) {
    const parsed = Number(process.env.THINKFLEET_TEST_HANDSHAKE_TIMEOUT_MS);
