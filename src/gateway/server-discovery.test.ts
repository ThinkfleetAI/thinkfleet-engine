...
describe("resolveTailnetDnsHint", () => {
  ...
  beforeEach(() => {
    prevTailnetDns.value = process.env.THINKFLEET_TAILNET_DNS;
    delete process.env.THINKFLEET_TAILNET_DNS;
  ...
  afterEach(() => {
    if (prevTailnetDns.value === undefined) {
      ...
      process.env.THINKFLEET_TAILNET_DNS = prevTailnetDns.value;
    }
  });

  test("returns env hint when disabled", async () => {
    process.env.THINKFLEET_TAILNET_DNS = "studio.tailnet.ts.net.";
