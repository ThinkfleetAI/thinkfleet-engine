  ...
  const applyHotReload = async (
    ...
    if (plan.restartGmailWatcher) {
      ...
      if (!isTruthyEnvValue(process.env.THINKFLEET_SKIP_GMAIL_WATCHER)) {
        ...
        params.logHooks.info("skipping gmail watcher restart (THINKFLEET_SKIP_GMAIL_WATCHER=1)");
      }
    }
    ...
    if (plan.restartChannels.size > 0) {
      if (
        ...
        params.logChannels.info(
          "skipping channel reload (THINKFLEET_SKIP_CHANNELS=1 or THINKFLEET_SKIP_PROVIDERS=1)",
        );
      } else {
