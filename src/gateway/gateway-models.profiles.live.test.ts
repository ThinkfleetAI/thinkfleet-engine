...
import { resolveThinkfleetAgentDir } from "../agents/agent-paths.js";
...
import { ensureThinkfleetModelsJson } from "../agents/models-config.js";
import { loadConfig } from "../config/config.js";
import type { ThinkfleetConfig, ModelProviderConfig } from "../config/types.js";
...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.THINKFLEET_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.THINKFLEET_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.THINKFLEET_LIVE_GATEWAY_PROVIDERS);
...
function sanitizeAuthConfig(params: {
  cfg: ThinkfleetConfig;
  agentDir: string;
}): ThinkfleetConfig["auth"] | undefined {
  ...
  let profiles: NonNullable<ThinkfleetConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    ...
    agentDir: process.env.THINKFLEET_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.THINKFLEET_STATE_DIR,
  };
  ...
  const hostAgentDir = resolveThinkfleetAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-live-state-"));
  process.env.THINKFLEET_STATE_DIR = tempStateDir;
  ...
  process.env.THINKFLEET_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.thinkfleet-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveThinkfleetAgentDir();
  const sanitizedCfg: ThinkfleetConfig = {
    ...params.cfg,
  ...
  try {
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "Thinkfleet live tool probe (local, safe): " +
    ...
    process.env.THINKFLEET_CONFIG_PATH = previous.configPath;
    process.env.THINKFLEET_GATEWAY_TOKEN = previous.token;
    process.env.THINKFLEET_SKIP_CHANNELS = previous.skipChannels;
    process.env.THINKFLEET_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.THINKFLEET_SKIP_CRON = previous.skipCron;
    process.env.THINKFLEET_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.THINKFLEET_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.THINKFLEET_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureThinkfleetModelsJson(cfg);
      ...
      const rawModels = process.env.THINKFLEET_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.thinkfleet-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
