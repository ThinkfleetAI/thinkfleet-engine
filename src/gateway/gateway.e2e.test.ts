...
describe("gateway e2e", () => {
  it(
    ...
    async () => {
      const prev = {
        ...
        skipBrowser: process.env.THINKFLEET_SKIP_BROWSER_CONTROL_SERVER,
      };

      const { baseUrl: openaiBaseUrl, restore } = installOpenAiResponsesMock();

      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-gw-mock-home-"));
      ...
      const workspaceDir = path.join(tempHome, "thinkfleet");
      await fs.mkdir(workspaceDir, { recursive: true });

      const nonceA = randomUUID();
      const nonceB = randomUUID();
      const toolProbePath = path.join(workspaceDir, `.thinkfleet-tool-probe.${nonceA}.txt`);
      await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

      const configDir = path.join(tempHome, ".thinkfleet");
      await fs.mkdir(configDir, { recursive: true });
      const configPath = path.join(configDir, "thinkfleet.json");
      ...
      process.env.THINKFLEET_CONFIG_PATH = configPath;
      ...
      try {
        ...
        process.env.THINKFLEET_CONFIG_PATH = prev.configPath;
        process.env.THINKFLEET_GATEWAY_TOKEN = prev.token;
        process.env.THINKFLEET_SKIP_CHANNELS = prev.skipChannels;
        process.env.THINKFLEET_SKIP_GMAIL_WATCHER = prev.skipGmail;
        process.env.THINKFLEET_SKIP_CRON = prev.skipCron;
        process.env.THINKFLEET_SKIP_CANVAS_HOST = prev.skipCanvas;
        process.env.THINKFLEET_SKIP_BROWSER_CONTROL_SERVER = prev.skipBrowser;
      }
    },
  );
  ...
  it("runs wizard over ws and writes auth token config", { timeout: 90_000 }, async () => {
    ...
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-wizard-home-"));
    process.env.HOME = tempHome;
    ...
    try {
      ...
      await fs.rm(tempHome, { recursive: true, force: true });
      process.env.HOME = prev.home;
      process.env.THINKFLEET_STATE_DIR = prev.stateDir;
      process.env.THINKFLEET_CONFIG_PATH = prev.configPath;
      process.env.THINKFLEET_GATEWAY_TOKEN = prev.token;
      process.env.THINKFLEET_SKIP_CHANNELS = prev.skipChannels;
      process.env.THINKFLEET_SKIP_GMAIL_WATCHER = prev.skipGmail;
