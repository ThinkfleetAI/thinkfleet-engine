...
describe("gateway server chat", () => {
  test("handles chat send and history flows", async () => {
    ...
    try {
      ...
      const sendPolicyDir = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-gw-"));
      ...
      const agentBlockedDir = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-gw-"));
      ...
      const historyDir = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-gw-"));
  ...
  test("routes chat.send slash commands without agent runs", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-gw-"));
    try {
