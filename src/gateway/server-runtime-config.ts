...
export async function resolveGatewayRuntimeConfig(params: {
  ...
  const canvasHostEnabled =
    process.env.THINKFLEET_SKIP_CANVAS_HOST !== "1" && params.cfg.canvasHost?.enabled !== false;

  assertGatewayAuthConfigured(resolvedAuth);
  if (tailscaleMode === "funnel" && authMode !== "password") {
    throw new Error(
      "tailscale funnel requires gateway auth mode=password (set gateway.auth.password or THINKFLEET_GATEWAY_PASSWORD)",
    );
  }
  ...
  if (!isLoopbackHost(bindHost) && !hasSharedSecret) {
    throw new Error(
      `refusing to bind gateway to ${bindHost}:${params.port} without auth (set gateway.auth.token/password, or set THINKFLEET_GATEWAY_TOKEN/THINKFLEET_GATEWAY_PASSWORD)`,
    );
  }
