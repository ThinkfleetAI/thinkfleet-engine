...
beforeAll(async () => {
  previousToken = process.env.THINKFLEET_GATEWAY_TOKEN;
...
afterAll(async () => {
  await server.close();
  if (previousToken === undefined) delete process.env.THINKFLEET_GATEWAY_TOKEN;
  else process.env.THINKFLEET_GATEWAY_TOKEN = previousToken;
});
...
describe("gateway server sessions", () => {
  ...
  test("sessions.preview returns transcript previews", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleet-sessions-preview-"));
    const storePath = path.join(dir, "sessions.json");
    testState.sessionStorePath = storePath;
