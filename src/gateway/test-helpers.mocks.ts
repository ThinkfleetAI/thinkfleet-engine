...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `thinkfleet-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.THINKFLEET_CONFIG_PATH = path.join(root, "thinkfleet.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "thinkfleet.json");
  ...
  return {
    ...
    loadConfig: () => {
      ...
      const defaults = {
        ...
        workspace: path.join(os.tmpdir(), "thinkfleet-gateway-test"),
        ...fileDefaults,
