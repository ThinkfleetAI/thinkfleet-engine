...
export function resolveGatewayAuth(params: {
  ...
  const token = authConfig.token ?? env.THINKFLEET_GATEWAY_TOKEN ?? undefined;
  const password = authConfig.password ?? env.THINKFLEET_GATEWAY_PASSWORD ?? undefined;
...
export function assertGatewayAuthConfigured(auth: ResolvedGatewayAuth): void {
  if (auth.mode === "token" && !auth.token) {
    ...
    throw new Error(
      "gateway auth mode is token, but no token was configured (set gateway.auth.token or THINKFLEET_GATEWAY_TOKEN)",
    );
  }
