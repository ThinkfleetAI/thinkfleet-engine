...
export function resolveHooksConfig(cfg: ThinkfleetConfig): HooksConfigResolved | null {
...
export function extractHookToken(req: IncomingMessage, url: URL): HookTokenResult {
  ...
  const headerToken =
    typeof req.headers["x-thinkfleet-token"] === "string"
      ? req.headers["x-thinkfleet-token"].trim()
      : "";
