    ...
    try {
...

      {
        ...
        const res = await postChatCompletions(
          port,
          { model: "thinkfleet", messages: [{ role: "user", content: "hi" }] },
          { "x-thinkfleet-agent-id": "beta" },
        );
...

      {
        ...
        const res = await postChatCompletions(
          ...
          {
            "x-thinkfleet-agent-id": "beta",
            "x-thinkfleet-session-key": "agent:beta:openai:custom",
          },
        );
        expect(res.status).toBe(200);
