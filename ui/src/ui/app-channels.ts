...
export async function handleWhatsAppStart(host: ThinkFleetBotApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: ThinkFleetBotApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: ThinkFleetBotApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: ThinkFleetBotApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: ThinkFleetBotApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: ThinkFleetBotApp): string {
...
export function handleNostrProfileCancel(host: ThinkFleetBotApp) {
...
export function handleNostrProfileToggleAdvanced(host: ThinkFleetBotApp) {
  const state = host.nostrProfileFormState;
  if (!state) return;
  host.nostrProfileFormState = {
    ...state,
...
export async function handleNostrProfileSave(host: ThinkFleetBotApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: ThinkFleetBotApp) {
  const state = host.nostrProfileFormState;
