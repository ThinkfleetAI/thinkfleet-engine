...
export async function handleAbortChat(host: ChatHost) {
  ...
  host.chatMessage = "";
  await abortChatRun(host as unknown as ThinkFleetBotApp);
}
...
async function sendChatMessageNow(
  ...
  const ok = await sendChatMessage(host as unknown as ThinkFleetBotApp, message, opts?.attachments);
...
export async function refreshChat(host: ChatHost) {
  await Promise.all([
    loadChatHistory(host as unknown as ThinkFleetBotApp),
