...
import { ThinkFleetBotApp } from "./app";
import "../styles.css";

const originalConnect = ThinkFleetBotApp.prototype.connect;

function mountApp(pathname: string) {
  window.history.replaceState({}, "", pathname);
  const app = document.createElement("thinkfleetbot-app") as ThinkFleetBotApp;
...
beforeEach(() => {
  ThinkFleetBotApp.prototype.connect = () => {
    // no-op: avoid real gateway WS connections in browser tests
  };
  window.__THINKFLEETBOT_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});

afterEach(() => {
  ThinkFleetBotApp.prototype.connect = originalConnect;
  window.__THINKFLEETBOT_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});
...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/thinkfleetbot/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/thinkfleetbot");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/thinkfleetbot/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__THINKFLEETBOT_CONTROL_UI_BASE_PATH__ = "/thinkfleetbot";
    const app = mountApp("/thinkfleetbot/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/thinkfleetbot");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/thinkfleetbot/sessions");
  });
  ...
  it("hydrates token from URL params even when settings already set", async () => {
    localStorage.setItem(
      "thinkfleetbot.control.settings.v1",
