...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in ThinkFleetBot
---

# Node + tsx "__name is not a function" crash

## Summary
Running ThinkFleetBot via Node with `tsx` fails at startup with:

```
[thinkfleetbot] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `thinkfleetbot status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch thinkfleetbot.mjs status
  ```
- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node thinkfleetbot.mjs status` works on Node 25.
