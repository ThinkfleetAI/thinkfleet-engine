---
summary: "OpenProse: .prose workflows, slash commands, and state in ThinkFleetBot"
read_when:
...
OpenProse is a portable, markdown-first workflow format for orchestrating AI sessions. In ThinkFleetBot it ships as a plugin that installs an OpenProse skill pack plus a `/prose` slash command. Programs live in `.prose` files and can spawn multiple sub-agents with explicit control flow.
...
```bash
thinkfleetbot plugins enable open-prose
```

Restart the Gateway after enabling the plugin.

Dev/local checkout: `thinkfleetbot plugins install ./extensions/open-prose`

Related docs: [Plugins](/plugin), [Plugin manifest](/plugins/manifest), [Skills](/tools/skills).

## Slash command

OpenProse registers `/prose` as a user-invocable skill command. It routes to the OpenProse VM instructions and uses ThinkFleetBot tools under the hood.
...
## ThinkFleetBot runtime mapping

OpenProse programs map to ThinkFleetBot primitives:

| OpenProse concept | ThinkFleetBot tool |
| --- | --- |
...
Treat `.prose` files like code. Review before running. Use ThinkFleetBot tool allowlists and approval gates to control side effects.
