...
Remote Gateway:
```bash
thinkfleetbot tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `thinkfleetbot tui --deliver`
...
- Check the Gateway logs: `thinkfleetbot logs --follow`.
- Confirm the agent can run: `thinkfleetbot status` and `thinkfleetbot models status`.
- If you expect messages in a chat channel, enable delivery (`/deliver on` or `--deliver`).
- `--history-limit <n>`: History entries to load (default 200)

## Troubleshooting
- `disconnected`: ensure the Gateway is running and your `--url/--token/--password` are correct.
- No agents in picker: check `thinkfleetbot agents list` and your routing config.
