---
summary: "First-run onboarding flow for ThinkFleetBot (macOS app)"
read_when:
...
- Writes credentials to `~/.thinkfleetbot/credentials/oauth.json`
...
The app can install the global `thinkfleetbot` CLI via npm/pnpm so terminal
...
On the first agent run, ThinkFleetBot bootstraps a workspace (default `~/thinkfleet`):
...
```bash
thinkfleetbot webhooks gmail setup --account you@gmail.com
```
...
- `~/.thinkfleetbot/credentials/oauth.json`
