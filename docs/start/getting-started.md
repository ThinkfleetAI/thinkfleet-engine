...
Fastest chat: open the Control UI (no channel setup needed). Run `thinkfleetbot dashboard`
and chat in the browser, or open `http://127.0.0.1:18789/` on the gateway host.
Docs: [Dashboard](/web/dashboard) and [Control UI](/web/control-ui).

Recommended path: use the **CLI onboarding wizard** (`thinkfleetbot onboard`). It sets up:
...
- **Recommended:** Brave Search API key for web search. Easiest path:
  `thinkfleetbot configure --section web` (stores `tools.web.search.apiKey`).
...
- OAuth credentials (legacy import): `~/.thinkfleetbot/credentials/oauth.json`
...
```bash
node thinkfleetbot.mjs gateway --port 18789 --verbose
```
...
If `thinkfleetbot health` shows “no auth configured”, go back to the wizard and set OAuth/key auth — the agent won’t be able to respond without it.

Tip: `thinkfleetbot status --all` is the best pasteable, read-only debug report.
Health probes: `thinkfleetbot health` (or `thinkfleetbot status --deep`) asks the running gateway for a health snapshot.
