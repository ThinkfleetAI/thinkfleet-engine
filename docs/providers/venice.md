---
summary: "Use Venice AI privacy-focused models in ThinkFleetBot"
read_when:
  - You want privacy-focused inference in ThinkFleetBot
...
```bash
thinkfleetbot onboard --auth-choice venice-api-key
```
...
```bash
thinkfleetbot chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, ThinkFleetBot shows all available Venice models. Pick based on your needs:
...
```bash
thinkfleetbot models set venice/claude-opus-45
thinkfleetbot models set venice/llama-3.3-70b
```

List all available models:

```bash
thinkfleetbot models list | grep venice
```

## Configure via `thinkfleetbot configure`

1. Run `thinkfleetbot configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
ThinkFleetBot automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
thinkfleetbot chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
thinkfleetbot chat --model venice/claude-opus-45

# Use uncensored model
thinkfleetbot chat --model venice/venice-uncensored

# Use vision model with image
thinkfleetbot chat --model venice/qwen3-vl-235b-a22b

# Use coding model
thinkfleetbot chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `thinkfleetbot models list` to see currently available models. Some models may be temporarily offline.
