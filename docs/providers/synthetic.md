---
summary: "Use Synthetic's Anthropic-compatible API in ThinkFleetBot"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. ThinkFleetBot registers it as the
...
```bash
thinkfleetbot onboard --auth-choice synthetic-api-key
```
...
Note: ThinkFleetBot's Anthropic client appends `/v1` to the base URL, so use
