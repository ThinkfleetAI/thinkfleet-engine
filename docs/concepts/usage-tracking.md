...
- `/usage cost` in chats: local cost summary aggregated from ThinkFleetBot session logs.
- CLI: `thinkfleetbot status --usage` prints a full per-provider breakdown.
- CLI: `thinkfleetbot channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
