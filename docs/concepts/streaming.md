...
ThinkFleetBot has two separate “streaming” layers:
...
When block streaming is enabled, ThinkFleetBot can **merge consecutive block chunks**
...
When draft streaming is active, ThinkFleetBot disables block streaming for that reply to avoid double-streaming.
