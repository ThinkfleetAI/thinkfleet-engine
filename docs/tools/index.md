---
summary: "Agent tool surface for ThinkFleetBot (browser, canvas, nodes, message, cron) replacing legacy `thinkfleetbot-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `thinkfleetbot-*` skills
---

# Tools (ThinkFleetBot)

ThinkFleetBot exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `thinkfleetbot-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `thinkfleetbot.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, ThinkFleetBot logs a warning and ignores the allowlist so core tools stay available.
...
- `host=node` can target a macOS companion app or a headless node host (`thinkfleetbot node run`).
...
Notes:
- Requires a Brave API key (recommended: `thinkfleetbot configure --section web`, or set `BRAVE_API_KEY`).
...
- Quick smoke: `thinkfleetbot nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
Core actions:
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `thinkfleetbot gateway` restart in-place)
