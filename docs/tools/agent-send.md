---
summary: "Direct `thinkfleetbot agent` CLI runs (with optional delivery)"
read_when:
  - Adding or modifying the agent CLI entrypoint
---
# `thinkfleetbot agent` (direct agent runs)

`thinkfleetbot agent` runs a single agent turn without needing an inbound chat message.
...
- Optional delivery back to a channel with `--deliver` + `--channel` (target formats match `thinkfleetbot message --target`).
...
```bash
thinkfleetbot agent --to +15555550123 --message "status update"
...
thinkfleetbot agent --to +15555550123 --message "Trace logs" --verbose on --json
thinkfleetbot agent --to +15555550123 --message "Summon reply" --deliver
