---
summary: "Fix Chrome/Brave/Edge/Chromium CDP startup issues for ThinkFleetBot browser control on Linux"
...
On Ubuntu (and many Linux distros), the default Chromium installation is a **snap package**. Snap's AppArmor confinement interferes with how ThinkFleetBot spawns and monitors the browser process.
...
Then update your ThinkFleetBot config (`~/.thinkfleetbot/thinkfleetbot.json`):
...
If you must use snap Chromium, configure ThinkFleetBot to attach to a manually-started browser:
...
You’re using the `chrome` profile (extension relay). It expects the ThinkFleetBot
