...
read_when:
  ...
  - Debugging why the managed browser is interfering with your own Chrome
...
- The `managed` profile does **not** touch your personal browser profile.
- The agent can **open tabs, read pages, click, and type** in a safe lane.
- The default `chrome` profile uses the **system default Chromium browser** via the
  extension relay; switch to `managed` for the isolated managed browser.

## What you get

- A separate browser profile named **managed** (orange accent by default).
- Deterministic tab control (list/open/focus/close).
- Agent actions (click/type/drag/select), snapshots, screenshots, PDFs.
- Optional multi-profile support (`managed`, `work`, `remote`, ...).
...
```bash
thinkfleetbot browser --browser-profile managed status
...
## Profiles: `managed` vs `chrome`

- `managed`: managed, isolated browser (no extension required).
- `chrome`: extension relay to your **system browser** (requires the ThinkFleetBot
  extension to be attached to a tab).

Set `browser.defaultProfile: "managed"` if you want managed mode by default.
...
- Default profile is `chrome` (extension relay). Use `defaultProfile: "managed"` for the managed browser.
- Auto-detect order: system default browser if Chromium-based; otherwise Chrome → Brave → Edge → Chromium → Chrome Canary.
- Local `managed` profiles auto-assign `cdpPort`/`cdpUrl` — set those only for remote CDP.
...
- **managed**: a dedicated Chromium-based browser instance with its own user data directory + CDP port
- **remote**: an explicit CDP URL (Chromium-based browser running elsewhere)
- **extension relay**: your existing Chrome tab(s) via the local relay + Chrome extension

Defaults:
- The `managed` profile is auto-created if missing.
...
ThinkFleetBot can also drive **your existing Chrome tabs** (no separate managed Chrome instance) via a local CDP relay + a Chrome extension.
...
error. ARIA snapshots and basic screenshots still work for managed Chrome.
...
- `snapshot`:
  ...
  - Config default (tool/CLI only): set `browser.snapshotDefaults.mode: "efficient"` to use efficient snapshots when the caller does not pass a mode (see [Gateway configuration](/gateway/configuration#browser-managed-browser)).
...
- The managed browser profile may contain logged-in sessions; treat it as sensitive.
...
- `browser` accepts:
  - `profile` to choose a named browser profile (managed, chrome, or remote CDP).
