...
- `commands.config` (default `false`) enables `/config` (reads/writes `thinkfleetbot.json`).
...
- For full provider usage breakdown, use `thinkfleetbot status --usage`.
- `/allowlist add|remove` requires `commands.config=true` and honors channel `configWrites`.
- `/usage` controls the per-response usage footer; `/usage cost` prints a local cost summary from ThinkFleetBot session logs.
...
Notes:
- Overrides apply immediately to new config reads, but do **not** write to `thinkfleetbot.json`.
- Use `/debug reset` to clear all overrides and return to the on-disk config.

## Config updates

`/config` writes to your on-disk config (`thinkfleetbot.json`). Owner-only. Disabled by default; enable with `commands.config: true`.
...
/config set messages.responsePrefix="[thinkfleetbot]"
...
- **Slack:** `channels.slack.slashCommand` is still supported for a single `/thinkfleet`-style command. If you enable `commands.native`, you must create one Slack slash command per built-in command (same names as `/help`). Command argument menus for Slack are delivered as ephemeral Block Kit buttons.
