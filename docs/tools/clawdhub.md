...
If you want to add new capabilities to your ThinkFleetBot agent, ClawdHub is the easiest way to find and install skills. You do not need to know how the backend works. You can:
...
By default, the CLI installs skills into `./skills` under your current working directory. If a ThinkFleetBot workspace is configured, `clawdhub` falls back to that workspace unless you override `--workdir` (or `CLAWDHUB_WORKDIR`). ThinkFleetBot loads workspace skills from `<workspace>/skills` and will pick them up in the **next** session. If you already use `~/.thinkfleetbot/skills` or bundled skills, workspace skills take precedence.
...
`clawdhub sync` scans your current workdir first. If no skills are found, it falls back to known legacy locations (for example `~/thinkfleetbot/skills` and `~/.thinkfleetbot/skills`). This is designed to find older skill installs without extra flags.
