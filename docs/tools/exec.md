...
- `gateway`/`node` approvals are controlled by `~/.thinkfleetbot/exec-approvals.json`.
...
- `host=sandbox`: runs `sh -lc` (login shell) inside the container, so `/etc/profile` may reset `PATH`.
  ThinkFleetBot prepends `env.PATH` after profile sourcing via an internal env var (no shell interpolation);
