...
- **gateway host** → `thinkfleetbot` process on the gateway machine
...
`~/.thinkfleetbot/exec-approvals.json` directly.

CLI: `thinkfleetbot approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
