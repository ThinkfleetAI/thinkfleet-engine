---
summary: "Chrome extension: let ThinkFleetBot drive your existing Chrome tab"
read_when:
...
The ThinkFleetBot Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate clawd-managed Chrome profile.
...
ThinkFleetBot then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the ThinkFleetBot release (npm package) as static files. There is no separate “build” step.

After upgrading ThinkFleetBot:
- Re-run `thinkfleetbot browser extension install` to refresh the installed files under your ThinkFleetBot state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

ThinkFleetBot ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want ThinkFleetBot to control.
...
- `ON`: attached; ThinkFleetBot can drive that tab.
...
Debugging: `thinkfleetbot sandbox explain`
...
`thinkfleetbot browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `thinkfleetbot browser extension install` first to copy the extension to a stable location under your ThinkFleetBot state directory.
