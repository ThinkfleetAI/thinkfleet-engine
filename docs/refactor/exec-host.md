...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.thinkfleetbot/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.thinkfleetbot/node.json` (node id + pairing token).
...
Path: `~/.thinkfleetbot/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.thinkfleetbot/exec-approvals.sock",
...
### IPC
- Unix socket at `~/.thinkfleetbot/exec-approvals.sock` (0600).
