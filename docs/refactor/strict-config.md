...
- Plugin manifests (`thinkfleetbot.plugin.json`) are required for all plugins.

## Plugin schema enforcement
- Each plugin provides a strict JSON Schema for its config (inline in the manifest).
- Plugin load flow:
  1) Resolve plugin manifest + schema (`thinkfleetbot.plugin.json`).
...
- If config invalid:
  ...
  - Instruct: `thinkfleetbot doctor --fix`.
...
Everything else must hard-fail with: “Config invalid. Run `thinkfleetbot doctor --fix`.”
