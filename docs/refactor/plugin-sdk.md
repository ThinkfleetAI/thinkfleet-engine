...
Delivery:
- Publish as `@thinkfleetbot/plugin-sdk` (or export from core under `thinkfleetbot/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)
Scope: everything that touches core runtime behavior.
Accessed via `ThinkFleetBotPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: ThinkFleetBotConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: ThinkFleetBotConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: ThinkFleetBotConfig, agentId?: string): RegExp[];
      matchesMentionPatterns(text: string, regexes: RegExp[]): boolean;
    };
    groups: {
      resolveGroupPolicy(cfg: ThinkFleetBotConfig, channel: string, accountId: string, groupId: string): {
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: ThinkFleetBotConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: ThinkFleetBotConfig): string;
  };
};
```
...
### Phase 0: scaffolding
- Introduce `@thinkfleetbot/plugin-sdk`.
- Add `api.runtime` to `ThinkFleetBotPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `thinkfleetbotRuntime: ">=2026.2.0"`).
