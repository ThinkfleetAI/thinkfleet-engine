...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `thinkfleetbot sandbox explain`.
...
Legacy `agent.*` configs are migrated by `thinkfleetbot doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ```exec
   docker ps --filter "label=thinkfleetbot.sandbox=1"
   ```
...
4. **Monitor logs:**
   ```exec
   tail -f "${THINKFLEETBOT_STATE_DIR:-$HOME/.thinkfleetbot}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
