...
Broadcast groups are evaluated after channel allowlists and group activation rules. In WhatsApp groups, this means broadcasts happen when ThinkFleetBot would normally reply (for example: on mention, depending on your group settings).
...
**Result:** When ThinkFleetBot would reply in this chat, it will run all three agents.
...
Workspace: /Users/pascal/thinkfleet-alfred/
...
Workspace: /Users/pascal/thinkfleet-baerbel/
Tools: read only
```
...
**Debug:**
```bash
tail -f ~/.thinkfleetbot/logs/gateway.log | grep broadcast
```
