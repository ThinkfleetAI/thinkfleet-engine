...
- Jobs persist under `~/.thinkfleetbot/cron/` so restarts don’t lose schedules.
...
## Storage & history
- Job store: `~/.thinkfleetbot/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.thinkfleetbot/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
```json5
{
  cron: {
    ...
    store: "~/.thinkfleetbot/cron/jobs.json",
...
thinkfleetbot cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
thinkfleetbot cron edit <jobId> --agent ops
thinkfleetbot cron edit <jobId> --clear-agent
```
```
...
```bash
thinkfleetbot system event --mode now --text "Next heartbeat: check battery."
```

## Gateway API surface
- `cron.list`, `cron.status`, `cron.add`, `cron.update`, `cron.remove`
- `cron.run` (force or due), `cron.runs`
For immediate system events without a job, use [`thinkfleetbot system event`](/cli/system).

## Troubleshooting

### “Nothing runs”
- Check cron is enabled: `cron.enabled` and `THINKFLEETBOT_SKIP_CRON`.
