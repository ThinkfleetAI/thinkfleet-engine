---
summary: "Research notes: offline memory system for ThinkFleet workspaces (Markdown source-of-truth + derived index)"
read_when:
  - Designing workspace memory (~/thinkfleet) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep ThinkFleetBot integration
  - Adding offline recall + reflection (retain/recall/reflect)
---

# Workspace Memory v2 (offline): research notes

Target: ThinkFleet-style workspace (`agents.defaults.workspace`, default `~/thinkfleet`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/thinkfleet` as canonical human-readable memory.
...
- `memory.md` remains “small + core-ish”: the things you want ThinkFleet to see every session.
...
```
~/thinkfleet/.memory/index.sqlite
```
...
Recommendation: **deep integration in ThinkFleetBot**, but keep a separable core library.

### Why integrate into ThinkFleetBot?
- ThinkFleetBot already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `thinkfleetbot memory recall "…" --k 25 --since 30d`
  - `thinkfleetbot memory reflect --since 7d`
