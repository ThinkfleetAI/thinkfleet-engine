...
read_when:
  - You are building a ThinkFleetBot plugin
  - You need to ship a plugin config schema or debug plugin validation errors
---
# Plugin manifest (thinkfleetbot.plugin.json)

Every plugin **must** ship a `thinkfleetbot.plugin.json` file in the **plugin root**.
ThinkFleetBot uses this manifest to validate configuration **without executing plugin
