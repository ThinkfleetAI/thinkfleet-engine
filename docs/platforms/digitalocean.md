---
summary: "ThinkFleetBot on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up ThinkFleetBot on DigitalOcean
  - Looking for cheap VPS hosting for ThinkFleetBot
---

# ThinkFleetBot on DigitalOcean

## Goal

Run a persistent ThinkFleetBot Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
# Check service
systemctl --user status thinkfleetbot-gateway.service

# View logs
journalctl --user -u thinkfleetbot-gateway.service -f
```
...
### WhatsApp
```bash
thinkfleetbot channels login whatsapp
# Scan QR code
```
...
journalctl -u thinkfleetbot --no-pager -n 50
```
