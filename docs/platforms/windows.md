...
read_when:
  - Installing ThinkFleetBot on Windows
  - Looking for Windows companion app status
---
# Windows (WSL2)

ThinkFleetBot on Windows is recommended **via WSL2** (Ubuntu recommended). The
...
- Remote nodes must point at a **reachable** Gateway URL (not `127.0.0.1`); use
  `thinkfleetbot status --all` to confirm.
...
### 3) Install ThinkFleetBot (inside WSL)
