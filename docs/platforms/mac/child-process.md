...
service via the external `thinkfleetbot` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.thinkfleet.gateway`
  (or `ai.thinkfleet.<profile>` when using `--profile`/`THINKFLEETBOT_PROFILE`; legacy `com.thinkfleetbot.*` is supported).
