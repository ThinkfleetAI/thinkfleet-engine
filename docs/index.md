---
summary: "Top-level overview of ThinkFleetBot, features, and purpose"
read_when:
  - Introducing ThinkFleetBot to newcomers
---
# ThinkFleetBot 🦞
...
- **Guided setup (recommended):** [Wizard](/start/wizard) (`thinkfleetbot onboard`)
- **Open the dashboard (local Gateway):** http://127.0.0.1:18789/ (or http://localhost:18789/)

If the Gateway is running on the same computer, that link opens the browser Control UI
immediately. If it fails, start the Gateway first: `thinkfleetbot gateway`.
...
WhatsApp / Telegram / Discord / iMessage (+ plugins)
  ...
  │                           │    /__thinkfleetbot__/canvas/ (Canvas host)
  └───────────┬───────────────┘
              │
              ├─ Pi agent (RPC)
              ├─ CLI (thinkfleetbot …)
              ├─ Chat UI (SwiftUI)
              ├─ macOS app (ThinkFleetBot.app)
              ├─ iOS node via Gateway WS + pairing
              └─ Android node via Gateway WS + pairing
```

Most operations flow through the **Gateway** (`thinkfleetbot gateway`), a single long-running process that owns channel connections and the WebSocket control plane.
...
- **Loopback-first**: Gateway WS defaults to `ws://127.0.0.1:18789`.
  ...
  - For Tailnet access, run `thinkfleetbot gateway --bind tailnet --token ...` (token is required for non-loopback binds).
- **Nodes**: connect to the Gateway WebSocket (LAN/tailnet/SSH as needed); legacy TCP bridge is deprecated/removed.
- **Canvas host**: HTTP file server on `canvasHost.port` (default `18793`), serving `/__thinkfleetbot__/canvas/` for node WebViews; see [Gateway configuration](/gateway/configuration) (`canvasHost`).
...
Switching between npm and git installs later is easy: install the other flavor and run `thinkfleetbot doctor` to update the gateway service entrypoint.
...
If you don’t have a global install yet, run the onboarding step via `pnpm thinkfleetbot ...` from the repo.
...
```bash
thinkfleetbot message send --target +15555550123 --message "Hello from ThinkFleetBot"
```

## Configuration (optional)

Config lives at `~/.thinkfleetbot/thinkfleetbot.json`.

- If you **do nothing**, ThinkFleetBot uses the bundled Pi binary in RPC mode with per-sender sessions.
...
**ThinkFleetBot = CLAW + TARDIS** — because every space lobster needs a time-and-space machine.
