...
Deploy ThinkFleetBot on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
<a href="https://render.com/deploy?repo=https://github.com/thinkfleetbot/thinkfleetbot" target="_blank" rel="noreferrer">Deploy to Render</a>
...
repository configures everything needed to run ThinkFleetBot:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: THINKFLEETBOT_STATE_DIR
        value: /data/.thinkfleetbot
      - key: THINKFLEETBOT_WORKSPACE_DIR
        value: /data/workspace
      - key: THINKFLEETBOT_GATEWAY_TOKEN
        generateValue: true  # auto-generates a secure token
    disk:
      name: thinkfleetbot-data
      mountPath: /data
      sizeGB: 1
```
...
The web dashboard is available at `https://<your-service>.onrender.com/thinkfleetbot`.
...
If you use the original ThinkFleetBot repository, Render will not auto-deploy your ThinkFleetBot. To update it, run a manual Blueprint sync from the dashboard.
...
For ThinkFleetBot, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any ThinkFleetBot host.
