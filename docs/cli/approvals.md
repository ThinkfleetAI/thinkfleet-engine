---
summary: "CLI reference for `thinkfleetbot approvals` (exec approvals for gateway or node hosts)"
read_when:
...
thinkfleetbot approvals get --gateway
```
...
thinkfleetbot approvals set --gateway --file ./exec-approvals.json
```
...
thinkfleetbot approvals allowlist remove "~/Projects/**/bin/rg"
```

## Notes

- `--node` uses the same resolver as `thinkfleetbot nodes` (id, name, ip, or id prefix).
- `--agent` defaults to `"*"`, which applies to all agents.
- The node host must advertise `system.execApprovals.get/set` (macOS app or headless node host).
- Approvals files are stored per host at `~/.thinkfleetbot/exec-approvals.json`.
