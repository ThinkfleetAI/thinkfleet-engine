...
ThinkFleetBot features that can generate provider usage or paid API calls.
...
- `thinkfleetbot status --usage` and `thinkfleetbot channels list` show provider **usage windows**
...
ThinkFleetBot can pick up credentials from:
...
- `thinkfleetbot status --usage`
- `thinkfleetbot models status --json`
...
### 8) Model scan / probe
`thinkfleetbot models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
