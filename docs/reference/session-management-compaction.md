...
This document explains how ThinkFleetBot manages sessions end-to-end:
...
ThinkFleetBot is designed around a single **Gateway process** that owns session state.
...
ThinkFleetBot persists sessions in two layers:
...
- Transcripts: `~/.thinkfleetbot/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

ThinkFleetBot resolves these via `src/config/sessions.ts`.
...
ThinkFleetBot intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (ThinkFleetBot consumes the events, but Pi decides when to compact).
...
ThinkFleetBot also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, ThinkFleetBot bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, ThinkFleetBot leaves it alone.
...
- `thinkfleetbot status` (CLI)
- `thinkfleetbot sessions` / `sessions --json`
...
ThinkFleetBot supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:
- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- ThinkFleetBot strips/suppresses this in the delivery layer.

As of `2026.1.10`, ThinkFleetBot also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
ThinkFleetBot uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but ThinkFleetBot’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `thinkfleetbot status`.
