...
```bash
thinkfleetbot plugins install @thinkfleetbot/nextcloud-talk
```

Local checkout (when running from a git repo):
```bash
thinkfleetbot plugins install ./extensions/nextcloud-talk
```
...
2) On your Nextcloud server, create a bot:
   ```bash
   ./occ talk:bot:install "ThinkFleetBot" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `thinkfleetbot pairing list nextcloud-talk`
  - `thinkfleetbot pairing approve nextcloud-talk <CODE>`
