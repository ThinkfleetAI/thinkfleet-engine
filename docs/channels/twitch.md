...
read_when:
  - Setting up Twitch chat integration for ThinkFleetBot
---
# Twitch (plugin)

Twitch chat support via IRC connection. ThinkFleetBot connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
thinkfleetbot plugins install @thinkfleetbot/twitch
```

Local checkout (when running from a git repo):

```bash
thinkfleetbot plugins install ./extensions/twitch
```
...
4) Configure the token:
   - Env: `THINKFLEETBOT_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "thinkfleetbot",              // Bot's Twitch account
      accessToken: "oauth:abc123...",    // OAuth Access Token (or use THINKFLEETBOT_TWITCH_ACCESS_TOKEN env var)
...
```bash
THINKFLEETBOT_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
