...
3) Set the token for ThinkFleetBot:
...
## Goals
- Talk to ThinkFleetBot via Discord DMs or guild channels.
...
3. Configure ThinkFleetBot with `channels.discord.token` (or `DISCORD_BOT_TOKEN` as a fallback).
...
7. Direct chats: secure by default via `channels.discord.dm.policy` (default: `"pairing"`). Unknown senders get a pairing code (expires after 1 hour); approve via `thinkfleetbot pairing approve discord <code>`.
...
This is the “Discord Developer Portal” setup for running ThinkFleetBot in a server (guild) channel like `#help`.
...
### 2) Enable the gateway intents ThinkFleetBot needs
...
Discord uses numeric ids everywhere; ThinkFleetBot config prefers ids.
...
### Troubleshooting
- First: run `thinkfleetbot doctor` and `thinkfleetbot channels status --probe` (actionable warnings + quick audits).
...
- On startup, ThinkFleetBot resolves channel/user names in allowlists to IDs (when the bot can search members)
  and logs the mapping; unresolved entries are kept as typed.

Native command notes:
- The registered commands mirror ThinkFleetBot’s chat commands.
...
- Slash commands may still be visible in Discord UI to users who aren’t allowlisted; ThinkFleetBot enforces allowlists on execution and replies “not authorized”.
...
- If the bot is stuck or rate limited, restart the gateway (`thinkfleetbot gateway --force`) after confirming no other processes own the Discord session.
