...
- If Telegram rejects the HTML payload, ThinkFleetBot retries the same message as plain text.

## Commands (native + custom)
ThinkFleetBot registers native commands (like `/status`, `/reset`, `/model`) with Telegram’s bot menu on startup.
...
Notes:
- Custom commands are **menu entries only**; ThinkFleetBot does not implement them unless you handle them elsewhere.
...
**Privacy note:** `@userinfobot` is a third-party bot. If you prefer, add the bot to the group, send a message, and use `thinkfleetbot logs --follow` to read `chat.id`, or use the Bot API `getUpdates`.

## Config writes
By default, Telegram is allowed to write config updates triggered by channel events or `/config set|unset`.

This happens when:
- A group is upgraded to a supergroup and Telegram emits `migrate_to_chat_id` (chat ID changes). ThinkFleetBot can migrate `channels.telegram.groups` automatically.
...
Telegram forum topics include a `message_thread_id` per message. ThinkFleetBot:
...
Private chats can include `message_thread_id` in some edge cases. ThinkFleetBot keeps the DM session key unchanged, but still uses the thread id for replies/draft streaming when it is present.
...
- Approve via:
  ...
  - `thinkfleetbot pairing approve telegram <CODE>`
...
2) Run `thinkfleetbot logs --follow` and look for `from.id`.
...
ThinkFleetBot defaults to audio files for backward compatibility.
...
ThinkFleetBot supports receiving and sending Telegram stickers with intelligent caching.

### Receiving stickers

When a user sends a sticker, ThinkFleetBot handles it based on the sticker type:
...
Stickers are processed through the AI's vision capabilities to generate descriptions. Since the same stickers are often sent repeatedly, ThinkFleetBot caches these descriptions to avoid redundant API calls.
...
**Cache location:** `~/.thinkfleetbot/telegram/sticker-cache.json`
...
ThinkFleetBot uses Bot API `sendMessageDraft` (not real messages) and then sends the
...
**How reactions work:**
Telegram reactions arrive as **separate `message_reaction` events**, not as properties in message payloads. When a user adds a reaction, ThinkFleetBot:
...
**Requirements:**
- Telegram bots must explicitly request `message_reaction` in `allowed_updates` (configured automatically by ThinkFleetBot)
...
- Example: `thinkfleetbot message send --channel telegram --target 123456789 --message "hi"`.
...
- `thinkfleetbot channels status` shows a warning when config expects unmentioned group messages.
- `thinkfleetbot channels status --probe` can additionally check membership for explicit numeric group IDs (it can’t audit wildcard `"*"` rules).
...
- Check gateway logs: `thinkfleetbot logs --follow` (look for "skipping group message")
...
- Upgrade to a ThinkFleetBot build that normalizes abort signals, or run the gateway on Node 20 until you can upgrade.
