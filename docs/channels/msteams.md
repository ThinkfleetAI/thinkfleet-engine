...
```bash
thinkfleetbot plugins install @thinkfleetbot/msteams
```

Local checkout (when running from a git repo):
```bash
thinkfleetbot plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
ThinkFleetBot will offer the local install path automatically.
...
3) Configure ThinkFleetBot with those credentials.
...
## Goals
- Talk to ThinkFleetBot via Teams DMs, group chats, or channels.
...
- On startup, ThinkFleetBot resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.thinkfleetbot/thinkfleetbot.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring ThinkFleetBot, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field | Value |
   |-------|-------|
   | **Bot handle** | Your bot name, e.g., `thinkfleetbot-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `thinkfleetbot plugins install @thinkfleetbot/msteams`
   - From a local checkout: `thinkfleetbot plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "ThinkFleetBot in Teams", "full": "ThinkFleetBot in Teams" },
...
ThinkFleetBot handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, ThinkFleetBot only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/ThinkFleetBotShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)
ThinkFleetBot sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `thinkfleetbot message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.thinkfleetbot/msteams-polls.json`.
...
**CLI:**
```bash
thinkfleetbot message send --channel msteams \
...
thinkfleetbot message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
thinkfleetbot message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
thinkfleetbot message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
thinkfleetbot message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
