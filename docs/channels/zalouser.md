...
read_when:
  - Setting up Zalo Personal for ThinkFleetBot
...
- Install via CLI: `thinkfleetbot plugins install @thinkfleetbot/zalouser`
- Or from a source checkout: `thinkfleetbot plugins install ./extensions/zalouser`
...
Approve via:
- `thinkfleetbot pairing list zalouser`
- `thinkfleetbot pairing approve zalouser <code>`
...
- On startup, ThinkFleetBot resolves group/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
...
- Re-login: `thinkfleetbot channels logout --channel zalouser && thinkfleetbot channels login --channel zalouser`
