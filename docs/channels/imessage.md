...
3) Configure ThinkFleetBot with `channels.imessage.cliPath` and `channels.imessage.dbPath`.
...
- Full Disk Access for ThinkFleetBot + `imsg` (Messages DB access).
...
2) Create a macOS user (example: `thinkfleethome`) and sign into it.
...
If you want iMessage on another Mac, set `channels.imessage.cliPath` to a wrapper that runs `imsg` on the remote macOS host over SSH. ThinkFleetBot only needs stdio.
...
**Remote attachments:** When `cliPath` points to a remote host via SSH, attachment paths in the Messages database reference files on the remote machine. ThinkFleetBot can automatically fetch these over SCP by setting `channels.imessage.remoteHost`:
...
If `remoteHost` is not set, ThinkFleetBot attempts to auto-detect it by parsing the SSH command in your wrapper script. Explicit configuration is recommended for reliability.
...
│ - thinkfleetbot gateway           │          SCP (attachments)        │ - Messages signed in     │
...
```json5
{
  channels: {
    imessage: {
      enabled: true,
      cliPath: "~/.thinkfleetbot/scripts/imsg-ssh",
...
Example wrapper (`~/.thinkfleetbot/scripts/imsg-ssh`):
```bash
#!/usr/bin/env bash
...
- Approve via:
  - `thinkfleetbot pairing list imessage`
  - `thinkfleetbot pairing approve imessage <CODE>`
...
If you explicitly configure a `chat_id` under `channels.imessage.groups`, ThinkFleetBot treats that thread as a “group” for:
