...
Tlon is a decentralized messenger built on Urbit. ThinkFleetBot connects to your Urbit ship and can
...
```bash
thinkfleetbot plugins install @thinkfleetbot/tlon
```

Local checkout (when running from a git repo):

```bash
thinkfleetbot plugins install ./extensions/tlon
```
...
Use these with `thinkfleetbot message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, ThinkFleetBot replies in-thread.
