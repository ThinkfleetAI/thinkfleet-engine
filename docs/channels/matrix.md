...
Matrix is an open, decentralized messaging protocol. ThinkFleetBot connects as a Matrix **user**
...
```bash
thinkfleetbot plugins install @thinkfleetbot/matrix
```

Local checkout (when running from a git repo):

```bash
thinkfleetbot plugins install ./extensions/matrix
```
...
1) Install the Matrix plugin:
   - From npm: `thinkfleetbot plugins install @thinkfleetbot/matrix`
   - From a local checkout: `thinkfleetbot plugins install ./extensions/matrix`
...
3) Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: ThinkFleetBot calls the same
     login endpoint, stores the access token in `~/.thinkfleetbot/credentials/matrix/credentials.json`,
...
- On first connection, ThinkFleetBot requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  ThinkFleetBot logs a warning.
...
`~/.thinkfleetbot/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `thinkfleetbot pairing list matrix`
  - `thinkfleetbot pairing approve matrix <CODE>`
...
- On startup, ThinkFleetBot resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
