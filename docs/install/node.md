...
read_when:
  - "You installed ThinkFleetBot but `thinkfleetbot` is “command not found”"
...
ThinkFleetBot’s runtime baseline is **Node 22+**.

If you can run `npm install -g thinkfleetbot@latest` but later see `thinkfleetbot: command not found`, it’s almost always a **PATH** issue: the directory where npm puts global binaries isn’t on your shell’s PATH.
