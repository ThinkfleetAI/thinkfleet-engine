...
### `thinkfleetbot: command not found`
...
You can also set `THINKFLEETBOT_VERBOSE=1` instead of the flag.
...
### `docs.thinkfleet.dev` shows an SSL error (Comcast/Xfinity)

Some Comcast/Xfinity connections block `docs.thinkfleet.dev` via Xfinity Advanced Security.
Disable Advanced Security or add `docs.thinkfleet.dev` to the allowlist, then retry.
...
- Check the allowlist: `thinkfleetbot config get agents.defaults.models`
...
If you can, include the relevant log tail from `thinkfleetbot logs --follow`.
