...
## Goals
- Send media with optional captions via `thinkfleetbot message send --media`.
- Allow auto-replies from the web inbox to include media alongside text.
- Keep per-type limits sane and predictable.

## CLI Surface
- `thinkfleetbot message send --media <path-or-url> [--message <caption>]`
...
- When media is present, the web sender resolves local paths or URLs using the same pipeline as `thinkfleetbot message send`.
- Multiple media entries are sent sequentially if provided.

## Inbound Media to Commands (Pi)
- When inbound web messages include media, ThinkFleetBot downloads to a temp file and exposes templating variables:
