---
summary: "Quick troubleshooting guide for common ThinkFleetBot failures"
read_when:
  - Investigating runtime issues or failures
---
# Troubleshooting 🔧

When ThinkFleetBot misbehaves, here's how to fix it.
...
|---|---|---|
| `thinkfleetbot status` | Local summary: OS + update, gateway reachability/mode, service, agents/sessions, provider config state | First check, quick overview |
| `thinkfleetbot status --all` | Full local diagnosis (read-only, pasteable, safe-ish) incl. log tail | When you need to share a debug report |
| `thinkfleetbot status --deep` | Runs gateway health checks (incl. provider probes; requires reachable gateway) | When “configured” doesn’t mean “working” |
| `thinkfleetbot gateway probe` | Gateway discovery + reachability (local + remote targets) | When you suspect you’re probing the wrong gateway |
| `thinkfleetbot channels status --probe` | Asks the running gateway for channel status (and optionally probes) | When gateway is reachable but channels misbehave |
| `thinkfleetbot gateway status` | Supervisor state (launchd/systemd/schtasks), runtime PID/exit, last gateway error | When the service “looks loaded” but nothing runs |
| `thinkfleetbot logs --follow` | Live logs (best signal for runtime issues) | When you need the actual failure reason |

**Sharing output:** prefer `thinkfleetbot status --all` (it redacts tokens). If you paste `thinkfleetbot status`, consider setting `THINKFLEETBOT_SHOW_SECRETS=0` first (token previews).
...
- Or paste a setup-token on the **gateway host**:
  ```bash
  thinkfleetbot models auth setup-token --provider anthropic
  ```
...
**Logs:**
- Preferred: `thinkfleetbot logs --follow`
- File logs (always): `/tmp/thinkfleetbot/thinkfleetbot-YYYY-MM-DD.log` (or your configured `logging.file`)
- macOS LaunchAgent (if installed): `$THINKFLEETBOT_STATE_DIR/logs/gateway.log` and `gateway.err.log`
- Linux systemd (if installed): `journalctl --user -u thinkfleetbot-gateway[-<profile>].service -n 200 --no-pager`
...
- Or set it directly:
  ```bash
  thinkfleetbot config set gateway.mode local
  ```

**If you meant to run a remote Gateway instead:**
- Set a remote URL and keep `gateway.mode=remote`:
  ```bash
  thinkfleetbot config set gateway.mode remote
  thinkfleetbot config set gateway.remote.url "wss://gateway.example.com"
  ```

**Ad-hoc/dev only:** pass `--allow-unconfigured` to start the gateway without
`gateway.mode=local`.

**No config file yet?** Run `thinkfleetbot setup` to create a starter config, then rerun
the gateway.
...
variables like `DISPLAY` should live in `~/.thinkfleetbot/.env` (loaded early by the
gateway).
...
service was installed with Bun or a version-managed Node path, run `thinkfleetbot doctor`
...
- then run `thinkfleetbot sandbox recreate --agent <id>` (or `--all`)
...
**Check:**
- `gateway.mode` must be `local` for `thinkfleetbot gateway` and the service.
- If you set `gateway.mode=remote`, the **CLI defaults** to a remote URL. The service can still be running locally, but your CLI may be probing the wrong place. Use `thinkfleetbot gateway status` to see the service’s resolved port + probe target (or pass `--url`).
- `thinkfleetbot gateway status` and `thinkfleetbot doctor` surface the **last gateway error** from logs when the service looks running but the port is closed.
- Non-loopback binds (`lan`/`tailnet`/`custom`, or `auto` when loopback is unavailable) require auth:
  `gateway.auth.token` (or `THINKFLEETBOT_GATEWAY_TOKEN`).
- `gateway.remote.token` is for remote CLI calls only; it does **not** enable local auth.
- `gateway.token` is ignored; use `gateway.auth.token`.

**If `thinkfleetbot gateway status` shows a config mismatch**
...
- Fix: rerun `thinkfleetbot gateway install --force` from the same `--profile` / `THINKFLEETBOT_STATE_DIR` you want the service to use.

**If `thinkfleetbot gateway status` reports service config issues**
- The supervisor config (launchd/systemd/schtasks) is missing current defaults.
- Fix: run `thinkfleetbot doctor` to update it (or `thinkfleetbot gateway install --force` for a full rewrite).
...
- Fix: set `gateway.auth.mode` + `gateway.auth.token` (or export `THINKFLEETBOT_GATEWAY_TOKEN`) and restart the service.

**If `thinkfleetbot gateway status` says `bind=tailnet` but no tailnet interface was found**
...
If you upgraded from older installs, you might still have `~/thinkfleetbot` on disk.
...
Symptoms: `pwd` or file tools show `~/.thinkfleetbot/sandboxes/...` even though you
...
ThinkFleetBot intentionally rejects **older/insecure models** (especially those more
...
- If you’re unsure which models are available, run `thinkfleetbot models list` or
  `thinkfleetbot models scan` and choose a supported one.
...
grep -n "agents\\|groupChat\\|mentionPatterns\\|channels\\.whatsapp\\.groups\\|channels\\.telegram\\.groups\\|channels\\.imessage\\.groups\\|channels\\.discord\\.guilds" \
  "${THINKFLEETBOT_CONFIG_PATH:-$HOME/.thinkfleetbot/thinkfleetbot.json}"
```
...
tail -f "$(ls -t /tmp/thinkfleetbot/thinkfleetbot-*.log | head -1)" | grep "blocked\\|skip\\|unauthorized"
```
...
```bash
thinkfleetbot pairing list <channel>
```
...
```bash
thinkfleetbot logs --follow | grep "pairing request"
```
...
- ❌ `@thinkfleet` + image
- ✅ `@thinkfleet check this` + image

### Session Not Resuming

**Check 1:** Is the session file there?
```bash
ls -la ~/.thinkfleetbot/agents/<agentId>/sessions/
```
...
thinkfleetbot logs --limit 200 | grep "connection\\|disconnect\\|logout"
```
...
```bash
thinkfleetbot gateway --verbose
```

If you’re logged out / unlinked:

```bash
thinkfleetbot channels logout
trash "${THINKFLEETBOT_STATE_DIR:-$HOME/.thinkfleetbot}/credentials" # if logout can't cleanly remove everything
thinkfleetbot channels login --verbose       # re-scan QR
```
...
```bash
grep "media\\|fetch\\|download" "$(ls -t /tmp/thinkfleetbot/thinkfleetbot-*.log | head -1)" | tail -20
```

### High Memory Usage

ThinkFleetBot keeps conversation history in memory.
...
ThinkFleetBot now refuses to start when the config contains unknown keys, malformed values, or invalid types.
This is intentional for safety.

Fix it with Doctor:
```bash
thinkfleetbot doctor
thinkfleetbot doctor --fix
```

Notes:
- `thinkfleetbot doctor` reports every invalid entry.
- `thinkfleetbot doctor --fix` applies migrations/repairs and rewrites the config.
- Diagnostic commands like `thinkfleetbot logs`, `thinkfleetbot health`, `thinkfleetbot status`, `thinkfleetbot gateway status`, and `thinkfleetbot gateway probe` still run even if the config is invalid.

### “All models failed” — what should I check first?

- **Credentials** present for the provider(s) being tried (auth profiles + env vars).
- **Model routing**: confirm `agents.defaults.model.primary` and fallbacks are models you can access.
- **Gateway logs** in `/tmp/thinkfleetbot/…` for the exact provider error.
- **Model status**: use `/model status` (chat) or `thinkfleetbot models status` (CLI).
...
```bash
thinkfleetbot channels login
```

### Build errors on `main` — what’s the standard fix path?

1) `git pull origin main && pnpm install`
2) `thinkfleetbot doctor`
...
```bash
curl -fsSL https://thinkfleet.dev/install.sh | bash -s -- --install-method git --no-onboard
```
...
5) Run `thinkfleetbot channels status --probe` for audit hints.
...
endpoint accepts a strict subset of JSON Schema. ThinkFleetBot scrubs/normalizes tool
schemas in current `main`, but the fix is not in the last release yet (as of
January 13, 2026).

Fix checklist:
1) **Update ThinkFleetBot**:
...
```bash
tccutil reset All ai.thinkfleet.mac.debug
```

**Fix 2: Force New Bundle ID**
If resetting doesn't work, change the `BUNDLE_ID` in [`scripts/package-mac-app.sh`](https://github.com/thinkfleetbot/thinkfleet-engine/blob/main/scripts/package-mac-app.sh) (e.g., add a `.test` suffix) and rebuild. This forces macOS to treat it as a new app.
...
# Or: launchctl bootout gui/$UID/ai.thinkfleet.gateway (replace with ai.thinkfleet.<profile>; legacy com.thinkfleetbot.* still works)
```
...
Ensure the global `thinkfleetbot` CLI is installed and matches the app version:
```bash
thinkfleetbot --version
npm install -g thinkfleetbot@<version>
```
...
#   ${THINKFLEETBOT_CONFIG_PATH:-$HOME/.thinkfleetbot/thinkfleetbot.json} -> { logging: { level: "trace" } }
#
# Then run verbose commands to mirror debug output to stdout:
thinkfleetbot gateway --verbose
thinkfleetbot channels login --verbose
```
...
| Log | Location |
|-----|----------|
| Gateway file logs (structured) | `/tmp/thinkfleetbot/thinkfleetbot-YYYY-MM-DD.log` (or `logging.file`) |
| Gateway service logs (supervisor) | macOS: `$THINKFLEETBOT_STATE_DIR/logs/gateway.log` + `gateway.err.log` (default: `~/.thinkfleetbot/logs/...`; profiles use `~/.thinkfleetbot-<profile>/logs/...`)<br />Linux: `journalctl --user -u thinkfleetbot-gateway[-<profile>].service -n 200 --no-pager`<br />Windows: `schtasks /Query /TN "ThinkFleetBot Gateway (<profile>)" /V /FO LIST` |
| Session files | `$THINKFLEETBOT_STATE_DIR/agents/<agentId>/sessions/` |
| Media cache | `$THINKFLEETBOT_STATE_DIR/media/` |
| Credentials | `$THINKFLEETBOT_STATE_DIR/credentials/` |
...
thinkfleetbot gateway status --deep
...
tail -20 /tmp/thinkfleetbot/thinkfleetbot-*.log
```
...
trash "${THINKFLEETBOT_STATE_DIR:-$HOME/.thinkfleetbot}"
thinkfleetbot channels login         # re-pair WhatsApp
thinkfleetbot gateway restart           # or: thinkfleetbot gateway
```
...
1. Check logs first: `/tmp/thinkfleetbot/` (default: `thinkfleetbot-YYYY-MM-DD.log`, or your configured `logging.file`)
