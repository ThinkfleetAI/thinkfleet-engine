...
## Quick check: `thinkfleetbot security audit` (formerly `thinkfleetbot security audit`)
...
```bash
thinkfleetbot security audit
thinkfleetbot security audit --deep
thinkfleetbot security audit --fix

# (On older installs, the command is `thinkfleetbot ...`.)
```
...
- Tighten local perms (`~/.thinkfleetbot` → `700`, config file → `600`, plus common state files like `credentials/*.json`, `agents/*/agent/auth-profiles.json`, and `agents/*/sessions/sessions.json`).

Running an AI agent with shell access on your machine is... *spicy*. Here’s how to not get pwned.

ThinkFleetBot is both a product and an experiment: you’re wiring frontier-model behavior into real messaging surfaces and real tools. **There is no “perfectly secure” setup.** The goal is to be deliberate about:
...
If you run `--deep`, ThinkFleetBot also attempts a best-effort live Gateway probe.

## Credential storage map

Use this when auditing access or deciding what to back up:

- **WhatsApp**: `~/.thinkfleetbot/credentials/whatsapp/<accountId>/creds.json`
- **Telegram bot token**: config/env or `channels.telegram.tokenFile`
- **Discord bot token**: config/env (token file not yet supported)
- **Slack tokens**: config/env (`channels.slack.*`)
- **Pairing allowlists**: `~/.thinkfleetbot/credentials/<channel>-allowFrom.json`
- **Model auth profiles**: `~/.thinkfleetbot/agents/<agentId>/agent/auth-profiles.json`
- **Legacy OAuth import**: `~/.thinkfleetbot/credentials/oauth.json`
...
`thinkfleetbot security audit` warns when this setting is enabled.
...
gateway:
  ...
  auth:
    mode: password
    password: ${THINKFLEETBOT_GATEWAY_PASSWORD}
```
...
ThinkFleetBot stores session transcripts on disk under `~/.thinkfleetbot/agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.thinkfleetbot` (see the audit section below). If you need
...
ThinkFleetBot can refresh the skills list mid-session:
...
- If you install plugins from npm (`thinkfleetbot plugins install <npm-spec>`), treat it like running untrusted code:
  - The install path is `~/.thinkfleetbot/extensions/<pluginId>/` (or `$THINKFLEETBOT_STATE_DIR/extensions/<pluginId>/`).
  - ThinkFleetBot uses `npm pack` and then runs `npm install --omit=dev` in that directory (npm lifecycle scripts can execute code during install).
...
thinkfleetbot pairing approve <channel> <code>
```
...
By default, ThinkFleetBot routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
ThinkFleetBot has two separate “who can trigger me?” layers:
...
- **DM allowlist** (`allowFrom` / `channels.discord.dm.allowFrom` / `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to `~/.thinkfleetbot/credentials/<channel>-allowFrom.json` (merged with config allowlists).
...
- “Paste the full contents of ~/.thinkfleetbot or your logs.”
...
4. **Re-run audit**
   - `thinkfleetbot security audit --deep` and confirm the report is clean.
...
- `~/.thinkfleetbot/thinkfleetbot.json`: `600` (user read/write only)
- `~/.thinkfleetbot`: `700` (user only)

`thinkfleetbot doctor` can warn and offer to tighten these permissions.

### 0.4) Network exposure (bind + port + firewall)

The Gateway multiplexes **WebSocket + HTTP** on a single port:
- Default: `18789`
- Config/flags/env: `gateway.port`, `--port`, `THINKFLEETBOT_GATEWAY_PORT`
...
The Gateway broadcasts its presence via mDNS (`_thinkfleetbot-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
4. **Environment variable** (alternative): set `THINKFLEETBOT_DISABLE_BONJOUR=1` to disable mDNS without config changes.
...
Doctor can generate one for you: `thinkfleetbot doctor --generate-gateway-token`.
...
- `gateway.auth.mode: "password"`: password auth (prefer setting via env: `THINKFLEETBOT_GATEWAY_PASSWORD`).

Rotation checklist (token/password):
1. Generate/set a new secret (`gateway.auth.token` or `THINKFLEETBOT_GATEWAY_PASSWORD`).
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), ThinkFleetBot
accepts Tailscale Serve identity headers (`tailscale-user-login`) as
authentication. ThinkFleetBot verifies the identity by resolving the
...
- ThinkFleetBot will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
Assume anything under `~/.thinkfleetbot/` (or `$THINKFLEETBOT_STATE_DIR/`) may contain secrets or private data:

- `thinkfleetbot.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
- When sharing diagnostics, prefer `thinkfleetbot status --all` (pasteable, secrets redacted) over raw logs.
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.thinkfleetbot/sandboxes`
...
- Prefer a dedicated profile for the agent (the default `managed` profile).
...
1. **Stop it:** stop the macOS app (if it supervises the Gateway) or terminate your `thinkfleetbot gateway` process.
2. **Close exposure:** set `gateway.bind: "loopback"` (or disable Tailscale Funnel/Serve) until you understand what happened.
3. **Freeze access:** switch risky DMs/groups to `dmPolicy: "disabled"` / require mentions, and remove `"*"` allow-all entries if you had them.

### Rotate (assume compromise if secrets leaked)

1. Rotate Gateway auth (`gateway.auth.token` / `THINKFLEETBOT_GATEWAY_PASSWORD`) and restart.
...
1. Check Gateway logs: `/tmp/thinkfleetbot/thinkfleetbot-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.thinkfleetbot/agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).

### Collect for a report

- Timestamp, gateway host OS + ThinkFleetBot version
