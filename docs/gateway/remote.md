...
- Use the macOS app’s **Remote over SSH** mode (Settings → General → “ThinkFleetBot runs”).
...
With the tunnel up:
- `thinkfleetbot health` and `thinkfleetbot status --deep` now reach the remote gateway via `ws://127.0.0.1:18789`.
- `thinkfleetbot gateway {status,health,send,agent,call}` can also target the forwarded URL via `--url` when needed.

Note: replace `18789` with your configured `gateway.port` (or `--port`/`THINKFLEETBOT_GATEWAY_PORT`).
