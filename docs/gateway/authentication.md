...
ThinkFleetBot supports OAuth and API keys for model providers. For Anthropic
...
2) Put it on the **gateway host** (the machine running `thinkfleetbot gateway`).
...
`~/.thinkfleetbot/.env` so the daemon can read it:
...
Then paste it into ThinkFleetBot:
...
```bash
thinkfleetbot models auth paste-token --provider anthropic
thinkfleetbot models auth paste-token --provider openrouter
```
