...
- During heartbeat runs, ThinkFleetBot treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `thinkfleetbot sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, ThinkFleetBot skips the heartbeat run entirely (no model call).
...
```bash
thinkfleetbot system event --text "Check for urgent follow-ups" --mode now
```
