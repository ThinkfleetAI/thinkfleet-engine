---
summary: "All configuration options for ~/.thinkfleetbot/thinkfleetbot.json with examples"
read_when:
  - Adding or modifying config fields
---
# Configuration 🔧

ThinkFleetBot reads an optional **JSON5** config from `~/.thinkfleetbot/thinkfleetbot.json` (comments + trailing commas allowed).

If the file is missing, ThinkFleetBot uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/thinkfleet`). You usually only need a config to:
...
ThinkFleetBot only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:
- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `thinkfleetbot doctor`, `thinkfleetbot logs`, `thinkfleetbot health`, `thinkfleetbot status`, `thinkfleetbot service`, `thinkfleetbot help`).
- Run `thinkfleetbot doctor` to see the exact issues.
- Run `thinkfleetbot doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `thinkfleetbot config set`. Keep a backup of `~/.thinkfleetbot/thinkfleetbot.json`.
...
```bash
thinkfleetbot gateway call config.get --params '{}' # capture payload.hash
thinkfleetbot gateway call config.apply --params '{
...
```bash
thinkfleetbot gateway call config.get --params '{}' # capture payload.hash
thinkfleetbot gateway call config.patch --params '{
...
{
  agents: {
    defaults: { workspace: "~/thinkfleet" },
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["@thinkfleet", "reisponde"] }
      }
    ]
  },
  channels: {
...
```json5
// ~/.thinkfleetbot/thinkfleetbot.json
{
  gateway: { port: 18789 },
...
```json5
// ~/.thinkfleetbot/agents.json5
{
  defaults: { sandbox: { mode: "all", scope: "session" } },
  list: [
    { id: "main", workspace: "~/thinkfleet" }
  ]
}
```
...
{ "$include": "/etc/thinkfleetbot/base.json5" } // absolute
...
```json5
// ~/.thinkfleetbot/clients/mueller/agents.json5
[
...
```json5
// ~/.thinkfleetbot/clients/mueller/broadcast.json5
{
...
ThinkFleetBot reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, ThinkFleetBot runs your login shell and imports only the missing expected keys (never overrides).
...
Env var equivalent:
- `THINKFLEETBOT_LOAD_SHELL_ENV=1`
- `THINKFLEETBOT_SHELL_ENV_TIMEOUT_MS=15000`
...
```json5
{
  ...
  gateway: {
    auth: {
      token: "${THINKFLEETBOT_GATEWAY_TOKEN}"
    }
  }
}
```
...
ThinkFleetBot stores **per-agent** auth profiles (OAuth + API keys) in:
- `<agentDir>/auth-profiles.json` (default: `~/.thinkfleetbot/agents/<agentId>/agent/auth-profiles.json`)

See also: [/concepts/oauth](/concepts/oauth)

Legacy OAuth imports:
- `~/.thinkfleetbot/credentials/oauth.json` (or `$THINKFLEETBOT_STATE_DIR/credentials/oauth.json`)

The embedded Pi agent maintains a runtime cache at:
- `<agentDir>/auth.json` (managed automatically; don’t edit manually)

Legacy agent dir (pre multi-agent):
- `~/.thinkfleetbot/agent/*` (migrated by `thinkfleetbot doctor` into `~/.thinkfleetbot/agents/<defaultAgentId>/agent/*`)

Overrides:
- OAuth dir (legacy import only): `THINKFLEETBOT_OAUTH_DIR`
- Agent dir (default agent root override): `THINKFLEETBOT_AGENT_DIR` (preferred), `PI_CODING_AGENT_DIR` (legacy)

On first use, ThinkFleetBot imports `oauth.json` entries into `auth-profiles.json`.
...
If set, ThinkFleetBot derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/thinkfleetbot/thinkfleetbot-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/thinkfleetbot/thinkfleetbot.log`.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // Optional override. Default: ~/.thinkfleetbot/credentials/whatsapp/biz
          // authDir: "~/.thinkfleetbot/credentials/whatsapp/biz",
        }
      }
    }
  }
...
- The legacy single-account Baileys auth dir is migrated by `thinkfleetbot doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [
      { id: "main", groupChat: { mentionPatterns: ["@thinkfleet", "thinkfleetbot", "managed"] } }
    ]
  }
}
```
    ...
    list: [
      {
        ...
        groupChat: {
          ...
          mentionPatterns: ["reisponde", "@thinkfleet"]
        }
      }
    ]
  }
...
- `agents.list[]`: per-agent overrides.
  ...
  - `workspace`: default `~/thinkfleet-<agentId>` (for `main`, falls back to `agents.defaults.workspace`).
  - `agentDir`: default `~/.thinkfleetbot/agents/<agentId>/agent`.
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/thinkfleet-home" },
      { id: "work", workspace: "~/thinkfleet-work" }
    ]
  },
  bindings: [
...
- `commands.config: true` enables `/config` (reads/writes `thinkfleetbot.json`).
...
ThinkFleetBot starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["thinkfleet-dm"]          // optional group DM allowlist
      },
      guilds: {
...
ThinkFleetBot starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `thinkfleetbot plugins install @thinkfleetbot/mattermost` (or `./extensions/mattermost` from a git checkout).
...
ThinkFleetBot starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
ThinkFleetBot spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, ThinkFleetBot
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/thinkfleetbot" } }
}
```
...
When a file exceeds this limit, ThinkFleetBot logs a warning and injects a truncated
...
message envelopes). If unset, ThinkFleetBot uses the host timezone at runtime.
...
`[thinkfleetbot]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[thinkfleetbot]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[thinkfleetbot]"`, ThinkFleetBot will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, ThinkFleetBot generates audio
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.thinkfleetbot/settings/tts.json",
      elevenlabs: {
...
`voiceAliases` lets Talk directives use friendly names (e.g. `"voice":"ThinkFleet"`).
...
```json5
{
  talk: {
    ...
    voiceAliases: {
      ThinkFleet: "EXAVITQu4vr4xnSDxMaL",
...
ThinkFleetBot also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, ThinkFleetBot currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `thinkfleetbot configure --section web`, or use `BRAVE_API_KEY` env var)
...
- `group:nodes`: `nodes`
- `group:thinkfleetbot`: all built-in ThinkFleetBot tools (excludes provider plugins)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.thinkfleetbot/sandboxes`
    ...
    defaults: {
      sandbox: {
        ...
        docker: {
          ...
          apparmorProfile: "thinkfleetbot-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
        ...
        browser: {
          enabled: false,
          image: "thinkfleetbot-sandbox-browser:bookworm-slim",
          containerPrefix: "thinkfleetbot-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
ThinkFleetBot uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.thinkfleetbot/agents/<agentId>/agent/models.json` or by defining the same schema inside your
ThinkFleetBot config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, ThinkFleetBot writes/merges a `models.json` into
`~/.thinkfleetbot/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. ThinkFleetBot uses
...
- Shortcut: `thinkfleetbot onboard --auth-choice opencode-zen`.
...
Shortcut: `thinkfleetbot onboard --auth-choice zai-api-key`.
...
Notes:
- Set `MOONSHOT_API_KEY` in the environment or use `thinkfleetbot onboard --auth-choice moonshot-api-key`.
...
Notes:
- Set `KIMICODE_API_KEY` in the environment or use `thinkfleetbot onboard --auth-choice kimi-code-api-key`.
...
Notes:
- Set `SYNTHETIC_API_KEY` or use `thinkfleetbot onboard --auth-choice synthetic-api-key`.
...
Notes:
- Set `MINIMAX_API_KEY` environment variable or use `thinkfleetbot onboard --auth-choice minimax-api`.
...
- Override the agent config root with `THINKFLEETBOT_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.thinkfleetbot/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.thinkfleetbot/agents/<agentId>/sessions/sessions.json
    // You can override with {agentId} templating:
    store: "~/.thinkfleetbot/agents/{agentId}/sessions/sessions.json",
...
- `resetByType`: per-session overrides for `dm`, `group`, and `thread`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, ThinkFleetBot stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.thinkfleetbot/skills` (workspace skills
...
from `~/.thinkfleetbot/extensions`, `<workspace>/.thinkfleetbot/extensions`, plus any
...
ThinkFleetBot can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for the managed browser and expose a small loopback control service.
...
- Note: the control server is started by the running gateway (ThinkFleetBot.app menubar, or `thinkfleetbot gateway`).
...
```json5
{
  browser: {
    ...
    profiles: {
      managed: { cdpPort: 18800, color: "#FF4500" },
...
- Examples: `"/ui"`, `"/thinkfleetbot"`, `"/apps/thinkfleetbot"`.
...
- When a connection comes from one of these IPs, ThinkFleetBot uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:
- `thinkfleetbot gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `THINKFLEETBOT_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- `gateway.auth.password` can be set here, or via `THINKFLEETBOT_GATEWAY_PASSWORD` (recommended).
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  (`tailscale-user-login`) to satisfy auth when the request arrives on loopback
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. ThinkFleetBot
...
macOS app behavior:
- ThinkFleetBot.app watches `~/.thinkfleetbot/thinkfleetbot.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.thinkfleetbot/thinkfleetbot.json` (or `THINKFLEETBOT_CONFIG_PATH`) and applies changes automatically.
...
Files watched:
- `~/.thinkfleetbot/thinkfleetbot.json` (or `THINKFLEETBOT_CONFIG_PATH`)
...
- `THINKFLEETBOT_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):
- `thinkfleetbot --dev …` → uses `~/.thinkfleetbot-dev` + shifts ports from base `19001`
- `thinkfleetbot --profile <name> …` → uses `~/.thinkfleetbot-<name>` (port via config/env/flags)
...
Example:
```bash
THINKFLEETBOT_CONFIG_PATH=~/.thinkfleetbot/a.json \
THINKFLEETBOT_STATE_DIR=~/.thinkfleetbot-a \
thinkfleetbot gateway --port 19001
```
...
- `x-thinkfleetbot-token: <token>` **or**
- `?token=<token>`
...
Gmail helper config (used by `thinkfleetbot webhooks gmail setup` / `run`):
...
- Set `THINKFLEETBOT_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, ThinkFleetBot defaults `serve.path` to `/` so
...
Default root: `~/thinkfleet/canvas`  
Default port: `18793` (chosen to avoid the managed browser CDP port `18792`)  
The server listens on the **gateway bind host** (LAN or Tailnet) so nodes can reach it.

The server:
- serves files under `canvasHost.root`
- injects a tiny live-reload client into served HTML
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__thinkfleetbot/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__thinkfleetbot__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
- env: `THINKFLEETBOT_SKIP_CANVAS_HOST=1`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.thinkfleetbot/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.thinkfleetbot/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.thinkfleetbot/bridge/tls/bridge-key.pem"
    }
  }
}
```
...
Controls LAN mDNS discovery broadcasts (`_thinkfleetbot-gw._tcp`).
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_thinkfleetbot-bridge._tcp` under `~/.thinkfleetbot/dns/` using the standard discovery domain `thinkfleetbot.internal.`

To make iOS/Android discover across networks (Vienna ⇄ London), pair this with:
- a DNS server on the gateway host serving `thinkfleetbot.internal.` (CoreDNS is recommended)
- Tailscale **split DNS** so clients resolve `thinkfleetbot.internal` via that server
