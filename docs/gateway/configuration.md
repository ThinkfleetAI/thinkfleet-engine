...
If the file is missing, ThinkFleetBot uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/clawd`). You usually only need a config to:
...
```json5
{
  ...
  agents: {
    list: [
      { id: "main", groupChat: { mentionPatterns: ["@clawd", "thinkfleetbot", "clawd"] } }
    ]
  }
}
```
...
ThinkFleetBot can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for clawd and expose a small loopback control service.
