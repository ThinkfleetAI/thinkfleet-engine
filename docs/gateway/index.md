...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__thinkfleetbot__/canvas/` from `~/clawd/canvas`. Disable with `canvasHost.enabled=false` or `THINKFLEETBOT_SKIP_CANVAS_HOST=1`.
