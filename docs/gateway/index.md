...
- Replaces the legacy `gateway` command. CLI entry point: `thinkfleetbot gateway`.
- Runs until stopped; exits non-zero on fatal errors so the supervisor restarts it.

## How to run (local)
```bash
thinkfleetbot gateway --port 18789
# for full debug/trace logs in stdio:
thinkfleetbot gateway --port 18789 --verbose
# if the port is busy, terminate listeners then start:
thinkfleetbot gateway --force
# dev loop (auto-reload on TS changes):
pnpm gateway:watch
```
- Config hot reload watches `~/.thinkfleetbot/thinkfleetbot.json` (or `THINKFLEETBOT_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__thinkfleetbot__/canvas/` from `~/thinkfleet/canvas`. Disable with `canvasHost.enabled=false` or `THINKFLEETBOT_SKIP_CANVAS_HOST=1`.
...
- Gateway auth is required by default: set `gateway.auth.token` (or `THINKFLEETBOT_GATEWAY_TOKEN`) or `gateway.auth.password`. Clients must send `connect.params.auth.token/password` unless using Tailscale Serve identity.
- The wizard now generates a token by default, even on loopback.
- Port precedence: `--port` > `THINKFLEETBOT_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- `THINKFLEETBOT_SERVICE_MARKER=thinkfleetbot`
- `THINKFLEETBOT_SERVICE_KIND=gateway`
- `THINKFLEETBOT_SERVICE_VERSION=<version>`
...
```bash
thinkfleetbot --dev setup
thinkfleetbot --dev gateway --allow-unconfigured
# then target the dev instance:
thinkfleetbot --dev status
thinkfleetbot --dev health
```

Defaults (can be overridden via env/flags/config):
- `THINKFLEETBOT_STATE_DIR=~/.thinkfleetbot-dev`
- `THINKFLEETBOT_CONFIG_PATH=~/.thinkfleetbot-dev/thinkfleetbot.json`
- `THINKFLEETBOT_GATEWAY_PORT=19001` (Gateway WS + HTTP)
- browser control service port = `19003` (derived: `gateway.port+2`, loopback only)
- `canvasHost.port=19005` (derived: `gateway.port+4`)
- `agents.defaults.workspace` default becomes `~/thinkfleet-dev` when you run `setup`/`onboard` under `--dev`.

Derived ports (rules of thumb):
- Base port = `gateway.port` (or `THINKFLEETBOT_GATEWAY_PORT` / `--port`)
- browser control service port = base + 2 (loopback only)
- `canvasHost.port = base + 4` (or `THINKFLEETBOT_CANVAS_HOST_PORT` / config override)
- Browser profile CDP ports auto-allocate from `browser.controlPort + 9 .. + 108` (persisted per profile).

Checklist per instance:
- unique `gateway.port`
- unique `THINKFLEETBOT_CONFIG_PATH`
- unique `THINKFLEETBOT_STATE_DIR`
- unique `agents.defaults.workspace`
- separate WhatsApp numbers (if using WA)

Service install per profile:
```bash
thinkfleetbot --profile main gateway install
thinkfleetbot --profile rescue gateway install
```

Example:
```bash
THINKFLEETBOT_CONFIG_PATH=~/.thinkfleetbot/a.json THINKFLEETBOT_STATE_DIR=~/.thinkfleetbot-a thinkfleetbot gateway --port 19001
THINKFLEETBOT_CONFIG_PATH=~/.thinkfleetbot/b.json THINKFLEETBOT_STATE_DIR=~/.thinkfleetbot-b thinkfleetbot gateway --port 19002
```
...
## Methods (initial set)
- `health` — full health snapshot (same shape as `thinkfleetbot health --json`).
...
- If other gateway-like services are detected, the CLI warns unless they are ThinkFleetBot profile services.
  ...
  - Cleanup: `thinkfleetbot gateway uninstall` (current service) and `thinkfleetbot doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `thinkfleetbot gateway install --force` to reinstall (profile/env/path changes).
...
`thinkfleetbot gateway install` writes the user unit. `thinkfleetbot doctor` audits the
unit and can update it to match the current recommended defaults.

Create `~/.config/systemd/user/thinkfleetbot-gateway[-<profile>].service`:
```
[Unit]
Description=ThinkFleetBot Gateway (profile: <profile>, v<version>)
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/local/bin/thinkfleetbot gateway --port 18789
Restart=always
RestartSec=5
Environment=THINKFLEETBOT_GATEWAY_TOKEN=
...
```
systemctl --user enable --now thinkfleetbot-gateway[-<profile>].service
```

**Alternative (system service)** - for always-on or multi-user servers, you can
install a systemd **system** unit instead of a user unit (no lingering needed).
Create `/etc/systemd/system/thinkfleetbot-gateway[-<profile>].service` (copy the unit above,
switch `WantedBy=multi-user.target`, set `User=` + `WorkingDirectory=`), then:
```
sudo systemctl daemon-reload
sudo systemctl enable --now thinkfleetbot-gateway[-<profile>].service
```
...
## CLI helpers
- `thinkfleetbot gateway health|status` — request health/status over the Gateway WS.
- `thinkfleetbot message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `thinkfleetbot agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `thinkfleetbot gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `thinkfleetbot gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance
- Retire uses of `thinkfleetbot gateway` and the legacy TCP control port.
