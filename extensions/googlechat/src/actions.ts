...
function listEnabledAccounts(cfg: ThinkFleetBotConfig) {
  return listEnabledGoogleChatAccounts(cfg).filter(
    (account) => account.enabled && account.credentialSource !== "none",
  );
}

function isReactionsEnabled(accounts: ReturnType<typeof listEnabledAccounts>, cfg: ThinkFleetBotConfig) {
...
export const googlechatMessageActions: ChannelMessageActionAdapter = {
  listActions: ({ cfg }) => {
    const accounts = listEnabledAccounts(cfg as ThinkFleetBotConfig);
    if (accounts.length === 0) return [];
    const actions = new Set<ChannelMessageActionName>([]);
    actions.add("send");
    if (isReactionsEnabled(accounts, cfg as ThinkFleetBotConfig)) {
      actions.add("react");
