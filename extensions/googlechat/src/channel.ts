import {
  ...
  type ChannelPlugin,
  type ThinkFleetBotConfig,
} from "thinkfleetbot/plugin-sdk";
import { GoogleChatConfigSchema } from "thinkfleetbot/plugin-sdk";
...
export const googlechatDock: ChannelDock = {
  ...
  config: {
    resolveAllowFrom: ({ cfg, accountId }) =>
      (resolveGoogleChatAccount({ cfg: cfg as ThinkFleetBotConfig, accountId }).config.dm?.allowFrom ??
        []
...
export const googlechatPlugin: ChannelPlugin<ResolvedGoogleChatAccount> = {
  ...
  pairing: {
    ...
    notifyApproval: async ({ cfg, id }) => {
      const account = resolveGoogleChatAccount({ cfg: cfg as ThinkFleetBotConfig });
  ...
  config: {
    listAccountIds: (cfg) => listGoogleChatAccountIds(cfg as ThinkFleetBotConfig),
    resolveAccount: (cfg, accountId) =>
      resolveGoogleChatAccount({ cfg: cfg as ThinkFleetBotConfig, accountId }),
    defaultAccountId: (cfg) => resolveDefaultGoogleChatAccountId(cfg as ThinkFleetBotConfig),
  ...
  security: {
    resolveDmPolicy: ({ cfg, accountId, account }) => {
      ...
      const useAccountPath = Boolean(
        (cfg as ThinkFleetBotConfig).channels?.["googlechat"]?.accounts?.[resolvedAccountId],
      );
    ...
    applyAccountConfig: ({ cfg, accountId, input }) => {
      ...
      const next =
        accountId !== DEFAULT_ACCOUNT_ID
          ? migrateBaseNameToDefaultAccount({
              cfg: namedConfig as ThinkFleetBotConfig,
              channelKey: "googlechat",
            })
          : namedConfig;
