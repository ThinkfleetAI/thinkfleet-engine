...
metadata: {"thinkfleetbot":{"emoji":"🪶","homepage":"https://www.prose.md"}}
---
...
- **Task tool** in the upstream spec == ThinkFleetBot `sessions_spawn`
- **File I/O** == ThinkFleetBot `read`/`write`
- **Remote fetch** == ThinkFleetBot `web_fetch` (or `exec` with curl when POST is required)
