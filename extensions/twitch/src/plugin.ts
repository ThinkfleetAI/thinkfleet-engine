/**
 * Twitch channel plugin for ThinkFleetBot.
 *
...
import { buildChannelConfigSchema } from "thinkfleetbot/plugin-sdk";
 ...
 * for ThinkFleetBot. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: ThinkFleetBotConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: ThinkFleetBotConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: ThinkFleetBotConfig): boolean => {
