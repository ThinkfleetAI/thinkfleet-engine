...
export class TwitchClientManager {
  ...
  async getClient(
    ...
    if (!tokenResolution.token) {
      this.logger.error(
        `Missing Twitch token for account ${account.username} (set channels.twitch.accounts.${account.username}.token or THINKFLEETBOT_TWITCH_ACCESS_TOKEN for default)`,
      );
