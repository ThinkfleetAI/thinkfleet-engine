 ...
 * The THINKFLEETBOT_TWITCH_ACCESS_TOKEN env var is only used for the default account.
 *
 * Token resolution priority:
 * 1. Account access token from merged config (accounts.{id} or base-level for default)
 * 2. Environment variable: THINKFLEETBOT_TWITCH_ACCESS_TOKEN (default account only)
 */
 ...
 *
 * @param cfg - ThinkFleetBot config
...
export function resolveTwitchToken(
  ...
  const envToken = allowEnv
    ? normalizeTwitchToken(opts.envToken ?? process.env.THINKFLEETBOT_TWITCH_ACCESS_TOKEN)
    : undefined;
  if (envToken) {
