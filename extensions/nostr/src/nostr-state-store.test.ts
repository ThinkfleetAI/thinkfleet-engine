...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.THINKFLEETBOT_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleetbot-nostr-"));
  process.env.THINKFLEETBOT_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const override = env.THINKFLEETBOT_STATE_DIR?.trim();
        if (override) return override;
        return path.join(homedir(), ".thinkfleetbot");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) delete process.env.THINKFLEETBOT_STATE_DIR;
    else process.env.THINKFLEETBOT_STATE_DIR = previous;
