...
- This plugin integrates Lobster with ThinkFleetBot *without core changes*.
...
- ThinkFleetBot provides an HTTP endpoint: `POST /tools/invoke`.
- The request is gated by **gateway auth** (e.g. `Authorization: Bearer …` when token auth is enabled).
- The invoked tool is gated by **tool policy** (global + per-agent + provider + group policy). If the tool is not allowed, ThinkFleetBot returns `404 Tool not available`.
