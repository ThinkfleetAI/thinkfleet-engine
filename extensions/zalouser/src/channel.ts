...
function resolveZalouserGroupToolPolicy(
  ...
  const account = resolveZalouserAccountSync({
    cfg: params.cfg as ThinkFleetBotConfig,
...
export const zalouserDock: ChannelDock = {
  ...
  config: {
    resolveAllowFrom: ({ cfg, accountId }) =>
      (resolveZalouserAccountSync({ cfg: cfg as ThinkFleetBotConfig, accountId }).config.allowFrom ?? []).map(
        (entry) => String(entry),
      ),
...
export const zalouserPlugin: ChannelPlugin<ResolvedZalouserAccount> = {
  ...
  config: {
    listAccountIds: (cfg) => listZalouserAccountIds(cfg as ThinkFleetBotConfig),
    resolveAccount: (cfg, accountId) =>
      resolveZalouserAccountSync({ cfg: cfg as ThinkFleetBotConfig, accountId }),
    defaultAccountId: (cfg) => resolveDefaultZalouserAccountId(cfg as ThinkFleetBotConfig),
  ...
  security: {
    resolveDmPolicy: ({ cfg, accountId, account }) => {
      ...
      const useAccountPath = Boolean(
        (cfg as ThinkFleetBotConfig).channels?.zalouser?.accounts?.[resolvedAccountId],
      );
  ...
  directory: {
    self: async ({ cfg, accountId, runtime }) => {
      ...
      const account = resolveZalouserAccountSync({ cfg: cfg as ThinkFleetBotConfig, accountId });
  ...
  pairing: {
    ...
    notifyApproval: async ({ cfg, id }) => {
      const account = resolveZalouserAccountSync({ cfg: cfg as ThinkFleetBotConfig });
  ...
  auth: {
    login: async ({ cfg, accountId, runtime }) => {
      ...
      if (!ok) {
        throw new Error(
          "Missing dependency: `zca` not found in PATH. See docs.thinkfleet.dev/channels/zalouser",
        );
      }
      runtime.log(
  ...
  gateway: {
    startAccount: async (ctx) => {
      ...
      return monitorZalouserProvider({
        account,
        config: ctx.cfg as ThinkFleetBotConfig,
        runtime: ctx.runtime,
