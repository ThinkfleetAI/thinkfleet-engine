...
ThinkFleetBot extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
thinkfleetbot channels login --channel zalouser              # Login via QR
thinkfleetbot channels login --channel zalouser --account work
...
```bash
thinkfleetbot directory self --channel zalouser
thinkfleetbot directory peers list --channel zalouser --query "name"
thinkfleetbot directory groups list --channel zalouser --query "work"
thinkfleetbot directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
thinkfleetbot message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
thinkfleetbot message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `thinkfleetbot channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
thinkfleetbot channels login --channel zalouser --account work
thinkfleetbot message send --channel zalouser --account work --target <id> --message "Hello"
