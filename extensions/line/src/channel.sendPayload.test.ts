...
function createRuntime(): { runtime: PluginRuntime; mocks: LineRuntimeMocks } {
  ...
  const resolveLineAccount = vi.fn(({ cfg, accountId }: { cfg: ThinkFleetBotConfig; accountId?: string }) => {
...
describe("linePlugin outbound.sendPayload", () => {
  it("sends flex message without dropping text", async () => {
    ...
    const cfg = { channels: { line: {} } } as ThinkFleetBotConfig;
  ...
  it("uses configured text chunk limit for payloads", async () => {
    const { runtime, mocks } = createRuntime();
    setLineRuntime(runtime);
    const cfg = { channels: { line: { textChunkLimit: 123 } } } as ThinkFleetBotConfig;
