...
type AgentEntry = NonNullable<NonNullable<ThinkFleetBotConfig["agents"]>["list"]>[number];

function listAgents(cfg: ThinkFleetBotConfig): AgentEntry[] {
  const list = cfg.agents?.list;
  if (!Array.isArray(list)) return [];
  return list.filter((entry): entry is AgentEntry => Boolean(entry && typeof entry === "object"));
}

function resolveAgentEntry(cfg: ThinkFleetBotConfig, agentId: string): AgentEntry | undefined {
  const id = normalizeAgentId(agentId);
  return listAgents(cfg).find((entry) => normalizeAgentId(entry.id) === id);
}

export function resolveIdentityName(cfg: ThinkFleetBotConfig, agentId: string): string | undefined {
