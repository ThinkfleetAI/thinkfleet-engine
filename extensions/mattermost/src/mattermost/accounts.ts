...
export function listMattermostAccountIds(cfg: ThinkFleetBotConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultMattermostAccountId(cfg: ThinkFleetBotConfig): string {
...
export function listEnabledMattermostAccounts(cfg: ThinkFleetBotConfig): ResolvedMattermostAccount[] {
