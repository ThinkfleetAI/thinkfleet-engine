import type { ChannelAccountSnapshot, ChannelPlugin, ThinkFleetBotConfig } from "thinkfleetbot/plugin-sdk";
import {
...
export const bluebubblesPlugin: ChannelPlugin<ResolvedBlueBubblesAccount> = {
  ...
  config: {
    listAccountIds: (cfg) => listBlueBubblesAccountIds(cfg as ThinkFleetBotConfig),
    resolveAccount: (cfg, accountId) =>
      resolveBlueBubblesAccount({ cfg: cfg as ThinkFleetBotConfig, accountId }),
    defaultAccountId: (cfg) => resolveDefaultBlueBubblesAccountId(cfg as ThinkFleetBotConfig),
    ...
    resolveAllowFrom: ({ cfg, accountId }) =>
      (resolveBlueBubblesAccount({ cfg: cfg as ThinkFleetBotConfig, accountId }).config.allowFrom ??
        []).map(
        (entry) => String(entry),
      ),
  ...
  security: {
    resolveDmPolicy: ({ cfg, accountId, account }) => {
      ...
      const useAccountPath = Boolean(
        (cfg as ThinkFleetBotConfig).channels?.bluebubbles?.accounts?.[resolvedAccountId],
      );
