...
export const bluebubblesMessageActions: ChannelMessageActionAdapter = {
  listActions: ({ cfg }) => {
    const account = resolveBlueBubblesAccount({ cfg: cfg as ThinkFleetBotConfig });
    if (!account.enabled || !account.configured) return [];
    const gate = createActionGate((cfg as ThinkFleetBotConfig).channels?.bluebubbles?.actions);
  ...
  handleAction: async ({ action, params, cfg, accountId, toolContext }) => {
    ...
    const opts = { cfg: cfg as ThinkFleetBotConfig, accountId: accountId ?? undefined };
