...
// NOTE: This extension is intended to be bundled with ThinkFleetBot.
// When running from source (tests/dev), ThinkFleetBot internals live under src/.
...
export function createLlmTaskTool(api: ThinkFleetBotPluginApi) {
  return {
    name: "llm-task",
    description:
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleetbot-llm-task-"));
