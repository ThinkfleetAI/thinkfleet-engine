...
`clawd.invoke --each`) without adding new ThinkFleetBot code per workflow.
...
This extension depends on ThinkFleetBot internal modules (the embedded agent runner).
It is intended to ship as a **bundled** ThinkFleetBot extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.thinkfleetbot/extensions` as a standalone plugin directory.
