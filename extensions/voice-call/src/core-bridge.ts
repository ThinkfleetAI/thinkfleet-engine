...
function resolveThinkFleetBotRoot(): string {
  if (coreRootCache) return coreRootCache;
  const override = process.env.THINKFLEETBOT_ROOT?.trim() || process.env.THINKFLEETBOT_ROOT?.trim();
  if (override) {
  ...
  for (const start of candidates) {
    for (const name of ["thinkfleetbot", "thinkfleetbot"]) {
  ...
  throw new Error(
    "Unable to resolve core root. Set THINKFLEETBOT_ROOT (or legacy THINKFLEETBOT_ROOT) to the package root.",
  );
}

async function importCoreModule<T>(relativePath: string): Promise<T> {
  const root = resolveThinkFleetBotRoot();
