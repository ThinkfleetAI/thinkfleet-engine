...
const liveEnabled = HAS_OPENAI_KEY && process.env.THINKFLEETBOT_LIVE_TEST === "1";
...
describe("memory plugin e2e", () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleetbot-memory-test-"));
...
describeLive("memory plugin live tests", () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "thinkfleetbot-memory-live-"));
    dbPath = path.join(tmpDir, "lancedb");
  });
