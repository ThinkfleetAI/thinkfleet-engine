...
# Paths
THINKFLEET_WORKSPACE_DEFAULT="$HOME/.thinkfleetbot/workspace"
THINKFLEET_WORKSPACE="${THINKFLEET_WORKSPACE:-$THINKFLEET_WORKSPACE_DEFAULT}"
SDK_REPO="${VEIL_SDK_REPO:-$THINKFLEET_WORKSPACE/repos/veildotcash-sdk}"
...
need_bankr_config() {
  if [[ ! -f "$BANKR_CONFIG" ]]; then
    ...
    echo "Create it with your Bankr API key (starts with bk_)." >&2
    exit 1
  fi
  need_bin jq
  need_bin curl
