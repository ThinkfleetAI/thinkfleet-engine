# ThinkFleetBot ACP Bridge

This document describes how the ThinkFleetBot ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`thinkfleetbot acp` exposes an ACP agent over stdio and forwards prompts to a running
ThinkFleetBot Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
In Zed, open the Agent panel and select “ThinkFleetBot ACP” to start a thread.

## Execution Model

- ACP client spawns `thinkfleetbot acp` and speaks ACP messages over stdio.
...
thinkfleetbot acp --reset-session
```
...
`thinkfleetbot acp` resolves the Gateway URL and auth from CLI flags or config:
